<!-- PR 은 한국어로, 다섯 살 아이에게 설명하듯 쓴다 (규칙: CLAUDE.md). 제목도 쉬운 한국어로 — 속도 숫자는 무엇을 어디서 잰 것인지 붙여서.
     글로만 말고 비유와 그림으로 — ```mermaid 그림, 글자 그림, 전/후 표, HTML(<table>·<details>). 그림 파일은 커밋하지 않는다.
     접지 않은 부분은 한 화면(2,000자 안팎) 안에. 아래 네 칸을 채운다.
     측정했으면 자세한 측정 기록은 본문이 아니라 PR 첫 댓글로 단다 — 다시 잴 수 있을 만큼 자세하게 (CLAUDE.md). -->
...
- [ ] 아니다 — 이 절은 지운다.
- [ ] 그렇다 — **같은 플릿 부팅에서 `bench/onepass.py` 로 C=1 두 판·C=4 한 판 측정한 숫자**를 여기 붙인다. 그 숫자가 나온 전체 기록(명령·판마다 숫자·로그 위치)은 측정 댓글에.
