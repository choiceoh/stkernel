...
- **엔진 노화 (08-09 발견)**: 고유 60K 프롬프트 벤치가 쌓이면 벤치당 ~−15 tok/s
  열화 (프리픽스 캐시 풀 포화 → 축출 churn; 20벤치에 −15%까지 관측), 재기동으로
  전량 회복. **구성 A/B는 반드시 프레시 엔진 + 동일 나이 표본** — 이를 어겨
  클럭캡 판정이 두 번 오염됐다(2000 "역효과"→교란, 1800 복귀 "미회복"→교란).
  실사용은 프리픽스 재사용(히트)이 있어 벤치만큼 병적이지 않지만, 장수 엔진의
  cache-full churn은 관찰 항목.
