...
**남은 최소 시도**: 부팅 기본값 그대로, **청크를 한 번도 강제하지 않고**, 스윕도 없이,
컨텍스트만 20K/12K/8K 로 바꿔 창 셋만 뜬다(꼬리가 크기를 준다). 그래도 안 되면 토치
프로파일러를 포기하고 `probes/moe_prof.py` 계열 CUDA-event 프로브로 `moe_expert` 가설을
직접 때린다.
