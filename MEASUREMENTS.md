...
대신 이 실험이 노출한 것: **동일 커널(RING_LL)의 per-call이 재기동을 거듭할수록
단조 악화** — 67.6µs(정착 엔진) → 87.0(재기동 N+1) → 96.1(N+3). 이것은
[hybrid-1.6 eval]의 **부트 사이클 침식** 서명이다(처방 = 플릿 재부팅 + 정착 대기).
따라서 "LL128 −4~5%", "Simple 지연지배" 류의 셀 판정은 **전부 침식과 교락 — 무효.**
