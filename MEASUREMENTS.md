...
### GLM rank checkpoint read/copy pipeline — 기본값 유지 (2026-09-08, PR #472)

네 노드 플릿 `rankpipe0908v3`, 소스 `82a97db`, 동일 이미지·환경에서 warm
B/A/A/B 부팅 **216 / 238 / 234 / 219초**. 기준 평균 **217.5초**, 후보
**236초**로 **18.5초 / 8.5% 느려졌다**. PRIME 407초는 캐시 생성이며 제외했다.
`VLLM_GLM53_RANK_CACHE_PIPELINE=0`을 유지한다. 캐시 파일을 두 개의 64 MiB
고정 버퍼에 읽고 검증한 뒤 CUDA 복사와 겹치는 후보는 성능 게이트를 통과하지 못했다.

헤드 모델 적재 **80.0 -> 95.95초**, 가장 느린 랭크 복원 **50.902 -> 65.992초**.
헤드 GPU 복사 대기는 약 **1.25 -> 0.06초**로 줄었지만, 직접 읽기 **41–43초**와
해시 **22초**가 같은 읽기 스레드에서 직렬화됐다. 기존 mmap 읽기/페이지 폴트/해시의
합계는 약 **41초**다. 중첩 타이머를 합산하거나 합성 CUDA 검사 시간을 성능으로
사용하지 않는다. 이 구현의 기각이며 모든 프리페치·읽기/해시 병렬화의 한계는 아니다.

모든 다섯 부팅 첫 텍스트·이미지·영상 및 품질 **6/6**, 한국어 손상 **0/4** 통과.
실제 GB10에서 지연 CUDA 복사·alias·offset·tail 포함 **268,435,517바이트**를
0/1/0/1 네 번 정확히 복원했고, 뒤쪽 손상 청크를 게시하지 않고 이전 복사를
완료했다. warm 부팅마다 네 랭크 hit, FP8 **976 hits / misses·errors 0**,
W4 SHA256 **258 누적 hits/rank**, 소스/Ninja 변경 0. 외부·health 이전 POST 0.
최저 가용 호스트 RAM **7.51 GiB**, swap 증가 0. 운영 복구 과정과 전체 검증·원본은
[실험 기록](measurements/glm53_rank_pipeline_20260908/README.md)에 보존한다.
