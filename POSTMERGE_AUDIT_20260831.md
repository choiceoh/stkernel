...
PR #138 후속 검토에서 최신 main의 #139까지 병합해 충돌·재합성을 다시 확인했고,
...
| 7 | `build/<profile>/` 스냅샷 ↔ module 소스 | **이격 6파일** — GLM53 3개(fp8_lm_head/tilelang 2개), 후속 parity 검사에서 DSV4 3개(oneshot CU/shim, moe gate)도 옛 코드로 남은 것을 확인 | 두 profile 재생성 + 39파일 byte parity 회귀 게이트 |
...
| 11 | `tests/test_logic.py` | 1079 checks green — UE8M0 섹션은 torch 없는 호스트에서 소리내어 스킵(#125 설계대로) | snapshot/probe 계약 회귀 게이트 추가 |
...
후속으로 #139를 최신 main에서 병합한 뒤 다시 compose했고, #139의 BIGFUSE 두 번째
필드와 이 PR의 ONEPASS/FP8 가드를 모두 보존한 상태로 18개 GLM53 build 파일이 각
...
재합성했다. `tests/test_logic.py`가 두 profile 39파일의 manifest/inventory/bytes를
