...
| `mac.sh` | Mac: `~/.venvs/stkernel`(`PYTHON_VERSION` 3.12, torch 의 macOS 휠)을 만들고 zsh 의 PATH 맨 앞에 둔다. 다른 Python 으로 만든 venv 는 옆으로 옮겨 두고 다시 만든다. macOS 용 triton 은 없어서 Triton 시험은 `stk-test` 컨테이너에서 돈다 |
