...
| `st_bracket.sh` · `st_judge.py` | **ST 엔진 브래킷** — 커밋 sha 하나가 팔, 프로덕션 형상, 부팅당 onepass 두 판(콜드·웜), `fleet.sh st-pair/st-chain/st-hold` 로 큐에 건다 | 팔은 #770 이후의 sha 여야 한다(릴리스의 런처가 티켓 리스를 검증) · 판정은 웜끼리, 기준선 부팅이 둘 미만이면 바닥 없음 |
