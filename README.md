...
| 파일 | 하는 일 |
|---|---|
| `node.sh` | 노드 하나를 목록에 맞춘다: `~/.local/bin`(uv·gh·mergiraf·wt·node·codex·claude, 아키텍처에 맞는 릴리스), 시스템 python3(3.12) 사용자 영역의 torch(CUDA 13.0)·triton·`PY_PACKAGES`, graphify, git 전역 설정, `~/stkernel` 체크아웃. 없거나 버전이 다른 것만 설치하고(바꾸는 파일은 `~/.local/bin/.pre-devenv/` 로 옮겨 둔다), 이미 있는 git 설정과 추적 파일이 바뀐 체크아웃은 건드리지 않는다. 판정은 GPU 를 가린 `dev_doctor.py --strict` |
| `sync.sh` | 모든 노드에 `node.sh` 를 한꺼번에 적용하고 노드마다 판정을 찍는다(`--verify` 는 CPU 시험 몇 개까지). 닿지 않는 노드(Windows 가 잠든 `ost-97x`)는 건너뛴다. 로그는 `~/.local/state/devenv-sync/` |
