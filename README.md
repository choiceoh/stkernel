...
- GLM 컴파일 캐시: 실행 파일 내용·설치 경로·원본 계약·이미지 ID가 같으면
  문서/벤치 커밋 재배포 후에도 head의 캐시를 재사용한다. 첫 적용은 한 번 비우며,
  이후 판단은 `[compile-cache]` 로그에 남는다. `.overlay-sha`는 플릿의 배포 SHA를
  계속 기록하고, `.compile-overlay.json`이 별도로 재사용 근거를 보관한다.
  캐시 삭제/기록 실패는 다음 컨테이너 시작 전에 중단한다.
- **롤백**: 런처의 해당 `-v ...:ro` 마운트 한 줄 제거 → 재기동. 오버레이가 없으면 이미지 원본 그대로.

## bench/ — 측정 도구와 함정

에이전트의 공유 플릿 실험은 [비동기 실험 제출 안내](bench/EXPERIMENTS.md)를 따른다.
`fleet.sh submit`은 커밋을 별도 체크아웃에 고정하고 중복 요청을 합친다. 실험 예약에는
짧은 구문·오버레이 구성 검사만 적용한다. GPU 하나면 되는 ST 검사는 플릿(스파크 넷)을 잡지 않고
srv4 한 대에서 프로덕션 옆에 돈다 — 큐의 단일 GPU 레인(`holder-single`). 증거는 여유 메모리(`--test`
부팅과 같은 16 GiB 바닥)이고, 플릿 부팅과는 한 박스를 나누지 않는다. 세션은 복구 부팅 없이 즉시 반납하고,
플릿이 5분 동안 유휴 상태일 때 중앙 타이머가 검증된 기존 버전으로 자동 복구한다.
전체 CPU 릴리스 검증과 복구 버전 갱신은 명시적으로 실행한다. 실험에 별도로 지정한 CPU
선행 검증은 그대로 적용하며, `inbox`/`result`로 결과를 받는 동안 작업을 계속할 수 있다.
**큐에서 기다리는 동안 CPU 작업이 자동으로 돈다**(`bench/fleet_waitwork.py`): 기본은
step_sim 재검증 — onepass 원장의 최근 기록 3개로 비용 상수를 다시 폴딩해 예측 표를
다시 낸다(`~/glm53-logs/wait-sim/`). GO 가 와도 끝나지 않았으면 두고 가고(컨트롤러 CPU,
플릿와 무관), 대기가 포기하는 끝남에서만 끊는다. `FLEET_WAIT_WORK='<cmd>'` 로 바꾸고
`off` 로 끈다.
플릿 점유 기록은 **리스 하나**(`engine/base/fleet_lease.py`, srv2 의 `glm53-logs/st-fleet.lock`)이고
큐가 그 주인이다: GO 때 큐가 `queue/<session>` 으로 잡아 페이로드(런처·프로브 러너)는 검증만 하고,
...
deploy-watch 는 배포 뒤 큐의 체크아웃 `~/fleet-controller` 를 배포된 sha 로 옮기고 D17 프로브 티켓 하나를 건다(웜 표본이
없는 채로 남으면 뒤 사이클에서 다시, sha 당 3장까지). 대기 중 체크아웃이 움직인 티켓은 멈추지 않고 큐가 같은 명령을 현재
리비전에 다시 준비해 자리를 지킨다(`FLEET_AUTO_REPIN=0` 이면 §95 의 멈춤).
프로덕션 복구의 유예는 큐의 기록에서 나온다(`fleet_pace.py`: 최근 6시간 티켓 간격의 p75, 5–20분; `fleet.sh window s 30` 이 창을 연다),
아무도 안 쓴 프로덕션은 정숙 게이트를 바로 연다(`served=0` 또는 `st:idle_seconds`), `run --replaces OLD` 는 옛 티켓의 자리를 잇는다.

**플릿 없이 쓰는 측정 경로** — 4박스를 잡지 못한 상태(누가 쥐고 있거나, 노트북에서
작업 중이거나)에서도 스텝/레이턴시 숫자를 얻는 세 도구가 아래 표 끝에 있다:
`step_sim.py`(엔진 스텝 루프의 숙주 비용 — 장치 시간 0), `step_peek.py`(살아있는 부팅의
`/metrics` 관측 — 요청을 보내지 않는다), `step_replay.py`(저장된 링·jsonl 증거의 재계산과
...
EOS·출력 길이 제한으로 잘린 커밋 기준이며 보너스 토큰은 제외한다. 빈 창, 카운터 리셋,
엔진/K 변경, 합계와 분포가 맞지 않는 구형 비동기 기록은 수치를 내지 않는다.
평균 수락률 하나만으로 마지막 위치의 누적 수락률을 복원하지 않는다.
...
| `st_bracket.sh` · `st_judge.py` | **ST 엔진 브래킷** — 커밋 sha 하나가 팔, 프로덕션 형상, 부팅당 onepass 두 판(콜드·웜), `fleet.sh st-pair/st-chain/st-hold` 로 큐에 건다; `st-probe` 는 부팅 없이 라이브 문에서 한 판(프로브 레인, 리스 없음 — 배포된 커밋의 웜 표본). 기준선은 최소로: 도입된 후보의 표본이 그대로 다음 기준선(정체성은 엔진 트리), 기준선 부팅이 하나면 바닥은 기록에서 빌린다 | 팔은 #770 이후의 sha 여야 한다(릴리스의 런처가 티켓 리스를 검증) · 판정은 웜끼리, 기준선 부팅이 둘 미만이면 바닥 없음 |
