...
`fleet.sh submit`은 커밋을 별도 체크아웃에 고정하고 중복 요청을 합친다. CPU 검증을
선행 조건으로 연결한 뒤 `inbox`/`result`로 증거를 받으면, 대기 중에도 원래 작업 폴더에서
개선을 계속할 수 있다. CPU 검증이 실패하거나 일부 생략되면 의존하는 GPU 실험은 시작하지 않는다.
