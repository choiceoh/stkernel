...
리맵하고, 원격 슬롯은 dummy expert(scale 0)에 둔다. 디코드 그래프는 미리 잡은
스크래치에 리맵하고, 프리필(`tokens * top_k > 640`)은 dummy 슬롯을 빼서 GEMM
하지 않는다. vLLM 의 EP all-reduce 가 랭크를 합친다. `ENABLE_EP=1` 이 `--enable-expert-parallel` 이다. EP 를 끄면
