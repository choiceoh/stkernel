...
- 대기 항목: **`MAX_NUM_BATCHED` 4096→2048 A/B** — 그들 실측 8192→2048에서
  ...
  이므로 한 번에 하나씩.
- 대기 항목: **`SPEC_TOKENS` 5→3 A/B** — aiden 레시피 스레드 #677(woooflet,
  2× GB10, eldritch.final + Triton MLA 조합): k=3에서 수용률 66.3%·평균
  수용 길이 2.99, **k=4/k=5 둘 다 k=3보다 느림**. 스택 구성이 달라
  직접 이식은 불가하지만, 수용 길이가 ~3에서 포화하면 k=5는 드래프트
  낭비 + noncausal 폭·디코드 threshold(1+2N)만 키운다. bench-dec의 새
  수용률 열이 정확히 이 판정용 — k=5 베이스라인 수용률부터 측정.
