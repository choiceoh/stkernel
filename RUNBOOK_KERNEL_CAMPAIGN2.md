...
| 7 | 준비 커널 통합 (prep_fused) | **채택 · 기본값** | — | `PREP_FUSED=1` · 레버 체인 16.39 → **17.59 step/s**(+7.3%), 32차 |
...
| 21 | MK-GEMM v2 (비상주 레인) | 프로브 먼저 | — | `MK_GEMM2` 프로필 미선언(= 모듈 기본 0) · exact PASS, 규칙 확정 뒤 m ≤ 16 전 형상 v2 ≥ v1 — **노출 프로브·스탬프 대기**(30차 §4~5). 상한 스텝 −2.5 ms |
