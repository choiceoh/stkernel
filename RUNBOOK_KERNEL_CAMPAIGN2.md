...
| 23 | b12x 정적(디코드) MoE 커널 v2/v3/v4 | **v3 `w` 채택 · 프로필 기본값(35차 부록)** · v4 `u` 브래킷 대기(38차) | ✅ | `VLLM_GLM53_B12X_STATIC_V2=w`(프로덕션, 디코드 창 +3~5%). 38차 프로브(U=40, 같은 창): v4 `u` 648.3 vs v3 671.7 µs(−3.5%, 스톡 대 +9.8%), `k` 분할 −1~2%, `e` 짝수 웨이브 기각(합계 대역폭은 스트림 수에 준선형). 다음 = base(`w`)→cand(`u`)→base 풀 레그 브래킷, 기대 디코드 창 +1.5~2% |
