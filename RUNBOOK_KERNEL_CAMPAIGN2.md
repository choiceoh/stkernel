...
| 23 | b12x 정적(디코드) MoE 커널 v2 | 프로브 먼저 · **선언 선행** | — | `VLLM_GLM53_B12X_STATIC_V2=""`(프로필, 기본 스톡). 스텝의 47~58%(층당 ~720 µs = 197 GB/s, 읽기 참조 245)인 커널 자체를 접수: gate+up 단일 스테이지·FC2 전용 스테이지·아이템 경계 배리어 제거·32행 A 박스(모듈 `b12x_zero_weight_micro/moe_static_kernel_v2.py`). 상한 층당 720 → 590~620 µs = 스텝 −4~6 ms(−7~10%). `probes/b12x_dram_pattern_bench.py`(TMA 접근 패턴 vs 선형) + `probes/b12x_static_probe.py`(스톡 vs v2, 수치 게이트, 스탬프) → 이기면 프로필 스펙 → 브래킷(프리필 동반; dynamic 백엔드는 불변이어야) |
