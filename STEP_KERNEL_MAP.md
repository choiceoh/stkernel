...
> 살아 있는 참조 — **스텝이 무슨 커널로 이루어지는지. 스텝이 바뀌면 다시 센다.** 여기가 틀리면 그건 버그다.
...
**ST 엔진 (2026-09-14)**: 위 세 트레이스는 vLLM 포크 시절이다. ST 엔진 main `de8bfff6` 의 C=1 K=7 반복은 커널 호출
약 1,359 회, 프로파일러 하 54.0 ms(없으면 약 47 ms)다. 도구가 달라(도어 `/v1/engine/profile`, 커널별 합) 이 표의 칸에
앉히지 않고 보충 분해 7 에 둔다.
...
**왜 시간이 제자리인가 — 30차(2026-09-05)가 답했다.** 같은 두 트레이스를 **발사
위치별**로 다시 읽으니 원인은 형상이 아니라 스트림이었다. b12x MoE(smem 90,112 B) ·
`mk_gemm`(69,648 B) · deep_gemm(92,672 B) 중 어느 둘도 한 SM(102,400 B)에 같이 앉지
못하므로 aux 스트림의 공유 전문가 쌍은 MoE 와 **직렬**이다. 스톡 deep_gemm 은 블록이
독립이라 MoE 블록이 하나씩 물러나는 꼬리에 끼어 노출이 0 이었지만(09-01 트레이스의
down 36 µs), **상주** `mk_gemm` 은 publish 배리어가 마지막 블록을 기다리느라 MoE 뒤로
밀린다 — 공유 down 이 트레이스 135 µs(오프라인 단독 18.4). 임계경로 노출은 스텝당
**1,177 µs**(MoE 시작 지연 15 + MoE 뒤 down 13, 층당 28 µs). 상주 커널의 값은 발사
제거인데, **MoE 옆에서는 그 상주성이 비용이 된다.**

**그리고 v2 레인이 그 비용을 걷어냈다(30차 §12, 2026-09-05).** 같은 185 발이
13.86 → **7.78 ms**, 공유 전문가 쌍의 노출이 1.16 → **0.15 ms** 로 떨어졌다 — 상주성을
버린 게 아니라 MoE 옆에서 자리를 다투지 않도록 고친 결과다. 프로덕션 부팅에서는 그
레인이 147 발 3.52 ms 로 잡힌다(KDA 레인이 in/o_proj 68 발을 삼켜서; 보충 분해 6).
...
### 오늘의 기본값은 이 트레이스보다 한 발 앞서 있다 (→ 보충 분해 6 이 확인)
...
| `VLLM_GLM53_PREP_FUSED=1` (EXP-7) | 09-04 밤 | **유휴 8.18 ms** 가 표적이었다 — 준비 체인이 발사 하나로 접힌다 | 32차: 기준 16.39 → **+6~9%** |

다음 캡처에서 **먼저 확인할 네 줄**이었다: (1) `mk_mla_kernel` 11 발/스텝이 있는가
(없으면 MLA 는 또 공허 무장이다), (2) cutlass bf16 201 → ~171 인가, (3) mk_gemm 185 →
~215 인가, (4) `_glm53_prep_fused_kernel` 이 스텝 앵커로 잡히고 **유휴가 8.18 ms 아래로**
내려갔는가.

**넷 다 09-05 프로덕션 캡처가 답했다(보충 분해 6)**: mk_mla 11 발 있음 · cutlass
201 → **103**(예상보다 더 내려갔다 — 드래프터 30 발이 W4 로 갔고 남은 22 발은 MLA 층의
q_b·wq_b) · mk_gemm 185 → **147**(KDA 레인이 68 발을 삼켰다; ~215 라는 내 예상은 KDA
미무장을 가정한 값이라 빗나갔다) · 유휴 8.18 → **1.20 ms**. 예상 셋 중 둘이 방향은
맞고 크기가 틀렸다 — **다른 노브가 같은 칸을 동시에 움직이면 한 노브의 예상은 빗나간다.**
...
## 보충 분해 7 (2026-09-14 — ST 엔진 main de8bfff6, 도어 프로파일)

ST 엔진(CUDA 13.2.1, #956 뒤), Red Hat 랭크, C=1 K=7(검증 8행), 약 70 토큰 프롬프트의 긴 그리디 생성. 도어의
`POST /v1/engine/profile {"steps": 32}` 두 창(32 버스트 × 약 3.9 반복)의 rank 0 커널 표를 반복당으로 나눴다.
벽시계는 프로파일러 하 54.0 ms, 없는 옆 창 46.8~47.8 ms. 커널 시간 합 54.89 ms(스트림이 겹쳐 벽시계를 넘는다),
호출 1,359 회/반복.

| 범주 | ms/반복 | 호출/반복 | 비중 | 보충 분해 5·6 (vLLM 포크) |
|---|---|---|---|---|
| MoE 전문가 (`MoEStaticKernelV5`) | 25.13 | 40 | 45.8% | 32.52 / 42 (09-04), 29.4 / 42 (09-05) |
| dense GEMM (mk_gemm2·input CTA·input pack·cutlass) | 13.65 | 356 | 24.9% | mk_gemm2 3.52 / 147 + bf16 3.34 / 103 (09-05, KDA in/o_proj 는 KDA 레인 안) |
| TP 통신 (publish·moe_packets·consumer) | 4.43 | 147 | 8.1% | `k_oneshot` AR 5.28 / 102 (09-04) |
| mHC (`mk_mhc_packets` 80회 4.04) | 4.42 | 93 | 8.0% | MK MHC 2.03 / 89 (09-04) |
| LM head·드래프터 fc (deep_gemm fp8×fp4, 형상 추정) | 2.21 | 3 | 4.0% | lm_head 둘 1.66 / 2 (09-04) |
| MLA / DSA (torch topk 0.85 포함) | 1.57 | 168 | 2.9% | MLA 0.41 / 22 + 인덱서 0.39 / 44 (09-04) |
| ST 글루 커널 | 1.41 | 206 | 2.6% | MoE 글루 1.20 / 136 (09-04) |
| KDA | 1.25 | 98 | 2.3% | 1.33 / 102 (09-04) |
| norm / elementwise / 복사 | 0.83 | 249 | 1.5% | elementwise 0.68 / 318 (09-05) |

오른쪽 열은 다른 스택·다른 도구의 사진이라 칸끼리 빼지 않는다(범주 경계도 다르다). 읽히는 것: 프로파일러 하 스텝
전체는 09-05 프로덕션(55.8 ms)과 비슷하고, MoE 는 가벼워졌고, mHC 는 호출당 23 µs → 50 µs 로 무거워 보인다 —
`mk_mhc_packets` 가 TP 패킷 소비를 포함하는지는 확인하지 않았다. 같은 프로세스에서 세 번째 프로파일부터 표가 비어
C=4 분해는 없다. 원시 표·창별 수치·재현은 `measurements/st_decode_profile_20260914/README.md`.
