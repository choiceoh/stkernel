...
from glm53_offline_checks import check_holder, pinned, remote
...
def verify_gate(candidate, directory, repo):
    ...
    if (not complete.get('ended') or complete.get('exit_code') != 0 or complete.get('error')
            ...
            or not gate.get('ended') or gate.get('command') != ['bash', 'probes/run_glm53_moe_overlap_tp4_check.sh']):
    ...
    if b'MOE_OVERLAP_ALL_GATES_PASS' not in log.splitlines():
        raise RuntimeError('four-rank GPU gate log incomplete')
    pinned(frozen, revision)
    frozen = Path(frozen)
    # The probe mounts the entire generated overlay. Preserve every byte,
    # including model integration and the stock expert implementation; a
    # passing transport-only fixture cannot cover an untested runtime edit.
    manifest = (frozen/'build/glm53/manifest.tsv').read_bytes()
    if manifest != (repo/'build/glm53/manifest.tsv').read_bytes():
        raise RuntimeError('GPU-validated overlay manifest changed')
    hashes, provenance = {}, {}
    for line in manifest.decode().splitlines():
        if not line or line.startswith('#'):continue
        name, _, _ = line.split('\t')
        path = 'build/glm53/'+name
    ...
    for path in ('profiles/glm53.env', 'launchers/start-glm53-nvfp4-tp4.sh',
                 'probes/glm53_moe_overlap_check.py', 'probes/run_glm53_moe_overlap_tp4_check.sh'):
        old, new = (frozen/path).read_bytes(), (repo/path).read_bytes()
    ...
    for line in log.splitlines():
        ...
        if isinstance(record, dict) and record.get('verdict') == 'MOE_OVERLAP_GPU_PASS':reports.append(record)
    ...
    cases = [(n, skew) for n in (4096, 4143, 6912, 8192) for skew in (False, True)]
    for report in reports:
        if report.get('provenance') != provenance:
            raise RuntimeError('GPU report source fingerprint mismatch')
        results = report.get('results', [])
        if [(r.get('rows'), r.get('skew')) for r in results] != cases:
            raise RuntimeError('GPU gate routing/length coverage incomplete')
        for row in results:
            if row.get('overlap_admitted') is not (row['rows'] >= 6144):
                raise RuntimeError('GPU report candidate routing missing')
            if any(row.get(phase, {}).get('bad_rows') != 0 for phase in ('eager', 'changed')):
