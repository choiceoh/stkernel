...
CANDIDATES = {
    'moe-m64': ('VLLM_GLM53_B12X_PREFILL_M64', 'GLM53_MOE_PREFILL_M64_LAUNCHED',
        'a1622f17fc4be5d0d5325130f533270427e16886', '/home/choiceoh/stkernel-moe-m64-check3-0908'),
}
...
def verify_gate(candidate, directory, repo):
    ...
    for report in reports:
        ...
        for row in results:
            if row.get('m64_admitted') is not (row['rows'] >= 6144):
                raise RuntimeError('GPU report candidate routing missing')
            phases = ['eager', 'stock_control', 'changed_control', 'local_moe', 'local_control']
            if row['rows'] == 8192:phases += ['graph', 'graph_changed']
            checks = [row.get(phase) for phase in phases]
            changed = row.get('changed')
            if not isinstance(changed, list) or len(changed) != 3:
                raise RuntimeError('GPU changed-input reuse coverage incomplete')
            checks += changed
