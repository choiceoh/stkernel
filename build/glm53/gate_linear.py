...
# deneb fork (vLLM #54048, opt-in): the fused bf16 x bf16 -> fp32 router GEMM is
# just torch.mm's out_dtype epilogue -- plain cuBLAS, no NVVM or CuteDSL
# codegen -- so it is arch-agnostic.  Upstream gates it on
# allow_specialized_router_gemm, which is Hopper + SM100 only, so family-120
# (consumer sm_120 / SoC sm_121: GB10 / DGX Spark) loses the one tier it can
# actually run and falls all the way to bf16 F.linear plus a standalone
# bf16->fp32 copy, bf16-rounding the router logits on the way.
#
# This is off by default here, unlike upstream, for one local reason: the
# DenebGateLinear tier in moe_gate_sm121.py refuses to arm unless every
# accelerated tier above the F.linear one is off, so switching this on disarms
# the measured M <= 32 fused-gate path (1.71 -> 1.18 ms/step, C=1 +3.5% on
# DSv4).  The two want composing -- cuBLAS for prefill's M > 32, the Triton
# split-K for decode -- and that composition needs a bracket run first.
# Numerics: enabling this drops a bf16 round trip, so prefill router logits move
# strictly closer to the fp32 reference but are bit-different from baseline.
_CUBLAS_ROUTER_GEMM_F120 = (
    os.environ.get("VLLM_GLM53_ROUTER_CUBLAS_F120", "0").strip().lower()
    in ("1", "true", "yes", "on")
)
...
class GateLinear(ReplicatedLinear):
    ...
    def __init__(
        ...
        self.allow_cublas_router_gemm = (
            (
                self.allow_specialized_router_gemm
                or _allow_cublas_router_gemm_family120(self._router_gemm_no_bias)
