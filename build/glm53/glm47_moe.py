...
def glm47_moe_config(thinking: bool = True) -> ParserEngineConfig:
    ...
    return ParserEngineConfig(
        ...
        # After the initial reasoning block, these may be literal text or tool
        # arguments. Do not restart reasoning or auto-drop them in off mode.
        preserve_tokens=frozenset({THINK_START, THINK_END}),
        # Content is user data, including indentation and inter-call spacing.
...
class Glm47MoeParser(ParserEngine):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._skip_for_none = False

    def _check_skip_tool_parsing(self, request) -> None:
        # The stock engine suppresses tool events AFTER consuming their text.
        # Skip recognition instead, so literal XML survives even without tools.
        # Restore our own flag when the parser is reused for another request;
        # leave the reasoning adapter's separate skip context intact.
        if self._skip_for_none:
    ...
    def _events_to_delta(self, events, finished=False):
        ...
        # single pass never drains that buffer, and chunk boundaries can reorder
        # or trim the text. GLM can emit content alongside tool-call deltas, so
        # collect every text event in order while retaining the engine's typed
        # argument conversion, reasoning handling, IDs, and tool delta logic.
        content = self._deferred_content + "".join(
            e.value for e in events if e.type == EventType.TEXT_CHUNK
        )
        self._deferred_content = ""
        delta = super()._events_to_delta(
            [e for e in events if e.type != EventType.TEXT_CHUNK], finished=finished
        )
        if content and not self._content_has_nonws:
            if content.strip():
                self._content_has_nonws = True
            elif not finished:
                # Keep a whitespace prefix until we know whether real content
                # follows. DelegatingParser canonicalizes whitespace-only
                # content accompanying tools to None in non-streaming replies.
    ...
    def extract_content_ids(self, input_ids: list[int]) -> list[int]:
        if not self.thinking_enabled:
            return input_ids
        # DelegatingParser calls this on the delta that ends reasoning. A later
        # literal </think> inside arguments must not discard the first call's
        # token IDs while its text is still handed to the tool parser.
        tool_start = self.vocab.get(TOOL_CALL_START)
        for index, token_id in enumerate(input_ids):
            if token_id == tool_start:
                return input_ids[index:]
            if token_id == self._reasoning_end_token_id:
