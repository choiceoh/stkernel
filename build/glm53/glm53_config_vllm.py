...
# deneb fork (glm53_async_dflash): the async-scheduling allowlist below is by
# method NAME. "dspark" is on it and DSparkSpeculator subclasses
# DFlashSpeculator, inheriting propose() -- the flow async scheduling touches --
# while "dflash" itself is not, so every dflash boot silently loses async
# scheduling (the V2 runner has no method-specific async branch; the scheduler
# fills [-1] draft placeholders and the worker writes the real ids). Whitelist
# dflash only when the operator asks; the default is the stock verdict.
def _deneb_dflash_async_ok(method) -> bool:
    if method != "dflash" or os.environ.get("VLLM_GLM53_ASYNC_DFLASH", "").strip() != "1":
        return False
    logger.warning_once(
        ...
        if self.scheduler_config.async_scheduling:
            ...
            if self.speculative_config is not None:
                if (
                    ...
                    and self.speculative_config.method != "dspark"
                    # deneb fork (glm53_async_dflash)
                    and not _deneb_dflash_async_ok(self.speculative_config.method)
                ):
                    raise ValueError(
