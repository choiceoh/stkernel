...
needed by the index/tail caches, then calls the same kpool op.
"""
...
def _glm53_cache_only_indexer_forward(
    ...
    # wk_weights_proj is a bf16, bias-free, replicated merged linear. Only its
    # first head_dim rows are K; the trailing n_head rows feed sparse scoring
    # and are dead under dense MLA. Avoid materializing that trailing output.
    k = F.linear(hidden_states, k_weight[: self.head_dim])
    ...
    gate_score = F.linear(hidden_states, gate)
