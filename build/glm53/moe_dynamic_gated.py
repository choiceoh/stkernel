...
class MoEGatedDynamicKernel:
    ...
    def __call__(
        ...
        # stkernel overlay (39차, static v2 cell t): tile-major expert weights
        # arrive 4-D -- (rows, K_in, K/K_in, E), K_in the stride-1 mode --
        # so every TMA box is one contiguous run. The two K modes group into
        # one hierarchical K that this kernel's tiles divide (tile_k 128 into
        # K_in 512 / 128); the scale-factor layouts and the gate tile count
        # use the flat (rows, K, E) shape the SF storage is laid out for.
        if cutlass.const_expr(len(b_w13.shape) == 4):
            w13_flat_shape = (
                b_w13.shape[0], b_w13.shape[1] * b_w13.shape[2], b_w13.shape[3]
            )
            b_w13 = cute.group_modes(b_w13, 1, 3)
        else:
            w13_flat_shape = b_w13.shape
        if cutlass.const_expr(len(b_down.shape) == 4):
            down_flat_shape = (
                b_down.shape[0], b_down.shape[1] * b_down.shape[2], b_down.shape[3]
            )
            b_down = cute.group_modes(b_down, 1, 3)
        else:
            down_flat_shape = b_down.shape
        ...
        sfb_w13_layout = blockscaled_utils.tile_atom_to_shape_SF(
            w13_flat_shape, self.sf_vec_size
        )
        ...
        sfb_down_layout = blockscaled_utils.tile_atom_to_shape_SF(
            down_flat_shape, self.sf_vec_size
        )
        ...
        gate_tile_cnt_static = w13_flat_shape[0] // self.tile_shape_mnk[1] // 2
