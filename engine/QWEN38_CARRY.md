...
| Q9 | QSA 커널 GB10 발사 기하 스윕: norm_rope·압축·저장·확장·점수, 어텐션 split 프로필(지금은 상류 GB300 프로필로 디코드마다 64 split) | #554 #556 #641 #658 #737 | `qsa.py` | measure | split 1 이면 −13 발사 | gpu | 일 | PR #1208: 측정 도구(`probes/engine_qwen38_qsa_geometry.py`, 레인 `qwen38_qsa_geometry`) + 프로브 훅 `qsa._SPLIT_PROFILE_OVERRIDE`·`_SCORE_PROFILE_OVERRIDE`·`_INPUT_WARPS_OVERRIDE`, `qsa_select._WIDEST_OVERRIDE`·`_WARPS_OVERRIDE`(기본 None, 서빙은 안 건드림). 서빙 층이 실제로 쏘는 발사만 잰다 — norm_rope·압축·저장·확장은 Q1–Q5 가 `qsa_index_keys`/`qsa_inputs`/FROM_BLOCKS 로 접어 더는 발사되지 않음. 팔: attention(캡처 N=2..32, eager 4,096행, covered 2,048행) 타일 폭×split×warps, 점수 타일 폭×프로그램당 타일×warps(4K/32K/256K 버킷), 선택의 한 발사 vs torch 형태를 버킷의 실제 열 수(1,152…65,664)에서(=GB10 의 WIDEST), 입력 두 발사 warps. 게이트: attention 은 fp32 오라클 밴드(split 은 반올림 위치를 옮겨 바이트는 요구 못 함), 점수·입력은 오늘 규칙과 바이트 동일. 짓다가 확인: `tl.dot` 의 shared memory 한계로 attention 타일은 64 열, 점수 타일은 256 열까지만 컴파일; 131K 버킷(32,832 열)은 이미 WIDEST 밖. 판정: CPU 인터프리터 + RTX 5050 스모크. **GB10 스윕 미실행** — 머지 뒤 레인 티켓, 규칙(`_split_profile`·`_score_profile`·WIDEST)은 그 기록 뒤에만 바꾼다 |
...
| Q13 | K/V 를 한 영역의 레코드로 두는 캐시 배치 | #641 | `caches.py` | measure | 0 발사 | gpu | 시간 | PR #1208 의 `records` 팔이 측정 도구: 같은 K/V 바이트를 오늘의 블록 배치(층의 K 행들 뒤에 V 행들 — 한 위치의 key 와 value 가 768 행 떨어짐)와 레코드 배치(위치마다 key·value 인접)로 두고 sparse attention 을 캡처 N=2/8/32 와 eager 4,096행에서 비교. 커널은 넘겨받은 stride 로 읽으므로 발사·바이트 동일(게이트), 주소만 다르다 — 이기면 바꿀 곳은 `caches.layout` 의 오프셋과 `Qwen38Caches` 의 두 `as_strided` 뿐. **GB10 미실행**(Q9 와 같은 레인 티켓) |

## 3. 게이트 잔차 — mHC 계열 아이디어

| ID | 내용 | GLM 출처 | 대상 | 종류 | 크기 | 판정 | 비용 | 상태 |
|---|---|---|---|---|---|---|---|---|
| H1 | down+inject GEMM 을 `leave_norm` 안으로 접기(디코드 ≤16 행) | MK_SEG_MHC | `kernels/gated_residual.py` | kernel | 사이트당 −1, −100 발사 | gpu | 일 | **기각(GB10 실측)** — H2 와 같은 구조(down+inject GEMM 15.0 µs 도 대역폭 한계치, 접으면 Triton matvec 이 행마다 다시 읽음). H2 의 실측과 프로브 참조. **GB10 실측(2026-09-19, 단일 GPU 레인 `qwen38-hc-headroom-0919b`): headroom 2.0–2.8 µs**(레인 34–43 µs 중 두 GEMM 31–40 µs, 1·2·4·6·8 행) — 레인의 Triton 발사를 전부 없애도 스텝 100 사이트에 0.2–0.3 ms, 35.3 ms 의 1% 미만. CUDA 세그먼트로 갈 값도 없다(`measurements/qwen38_hc_headroom_20260919`) |
| H2 | gates + up GEMM + `mix_mean` 한 발사 | MK_SEG_MHC | `gated_residual.py:mix` | kernel | 사이트당 −2, −200 발사 | gpu | 일 | **기각(GB10 실측)** — 짓기 전에 실측(`probes/engine_qwen38_hc_mix_fused.py`, RTX 5050·sm_120, CUDA 그래프 재생): 캡처 스텝의 행 수(2·4·6·8)에서 사이트 28 µs 중 두 cuBLAS GEMM 이 15.0 + 12.8 µs — 각자 가중치 6.5 MB 를 모든 행에 한 번 읽는 대역폭 한계치이고, H2 가 없애는 Triton 두 발사는 그래프 안에서 합쳐 1 µs 미만. 한 발사 프로토타입은 출력이 레인과 바이트 동일하지만 34 / 55 / 92 µs(2 / 4 / 8 행): Triton matvec 은 행마다 가중치를 다시 읽는다(1 행에서만 이김: 43 → 25 µs, cuBLAS 의 gemv 경로가 느려서). 그 카드에서 사이트를 움직이는 것은 발사 수가 아니라 바이트 — H6. 가중치를 모든 행에 한 번만 읽는 두 번째 프로토타입은 더 느렸다(브로드캐스트 합 40–107 µs, `tl.dot` 은 행과 무관하게 188 µs): Triton 이 만든 GEMM 은 이 모양에서 cuBLAS 에 못 미친다 — **Triton 으로 짓는 접기는 어느 카드에서도 기각.** 남은 질문은 GB10 의 발사 고정비다: 그 카드는 발사당 약 1 µs 였지만 GLM 원장은 캡처 그래프 안 15–25 µs(제거 가능 약 5)를 적는다. 프로브의 `headroom`(레인 − 두 GEMM)이 이 사이트의 그 숫자이고, GB10 에서 수십 µs 가 나오면 접기는 `MK_SEG_MHC` 식 CUDA 세그먼트로 갈 값이 있다. **GB10 실측(2026-09-19, 단일 GPU 레인 `qwen38-hc-headroom-0919b`): headroom 2.0–2.8 µs**(레인 34–43 µs 중 두 GEMM 31–40 µs, 1·2·4·6·8 행) — 레인의 Triton 발사를 전부 없애도 스텝 100 사이트에 0.2–0.3 ms, 35.3 ms 의 1% 미만. CUDA 세그먼트로 갈 값도 없다(`measurements/qwen38_hc_headroom_20260919`) |
...
| H6 | `--hc-fp8` 레인 실측(믹서 가중치 읽기가 스텝당 1.32 GB) | — | `net.py:_prepare_hc_fp8` | measure | 바이트 −40%, 발사 +300 | gpu | 시간 | 열림 — H2 의 실측이 가리키는 레버: 디코드 사이트는 거의 전부 두 믹서 GEMM 의 바이트(사이트당 13 MB, 스텝 100 사이트에 1.32 GB → GB10 273 GB/s 로 나누면 약 4.8 ms, 산수이지 실측이 아님). FP8 은 그 절반. **GB10 에서 잰 값(2026-09-19, `measurements/qwen38_hc_headroom_20260919`): 사이트의 두 GEMM 31–35 µs(2–8 행) → 스텝 100 사이트에 3.1–3.5 ms, 35.3 ms 스텝의 9–10%** — 이 레인이 겨냥할 수 있는 상한의 크기 |
