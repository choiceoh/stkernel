...
| S3 | 부팅 자기 보정(GPTQ W4/FP8·헤드) 배선 | #650 #659 #661 #673 #779 | `profiles/qwen38/fleet.py`, `calibration.py` | kernel | 품질 | gpu | 일 | 배선·통계 검증 완료: 타깃 193곳, 패딩·출처 표식·실제 prefill 수집·자동 저장. GB10 83검사 통과. 실서비스 보정 후 TP4 품질은 미판정([기록](../measurements/qwen38_precision_port_20260919/README.md)) |
| S4 | compact EP 프리필 FP32 누적 + FC1/FC2 활성 스케일 탐색 | #1139 #1127 #1157 | `b12x/moe_dispatch.py`, `_moe_dynamic/generic.py`, `qwen38/lanes.py` | kernel | 정밀도 | gpu | 일 | 구현·GB10 셀 통과. 다섯 부분합의 독립 합산과 바이트 동일, SSE −67.75%; as2 합성 MLP SSE −13~21%. TP4 품질·수용률·속도는 미판정([기록](../measurements/qwen38_moe_precision_20260919/README.md)) |
| S5 | unit-offset 채널 스무딩 | #1151 #1154 | `dense/smoothing.py`, `qwen38/net.py` | 검토 | 정밀도 | cpu | 일 | 직접 norm fold는 합성 비양자화 투영에 RMSE 2.258%를 더해 적용하지 않음. 반올림된 입력을 packer에서 2의 거듭제곱으로 나누는 대안은 합성 정상 범위에서 정확; 실가중치 재양자화·비용은 미판정([검토](../measurements/qwen38_moe_precision_20260919/README.md)) |
