...
그 대화를 모든 랭크에서 버렸다. 통째 기록(토큰 리스트 전부, 10만 토큰이면 3.4 MiB)은 파킹까지 포함해 가장 최근 8개만 둔다
(`Runner.PARKED_RECORDS_KEPT`; 2026-09-19 까지는 파킹이 자르지 않아 이 프로세스가 파킹한 대화를 모든 랭크가 전부 들고 있었다). 입장과
힌트 재확인은 파킹된 대화의 **다이제스트**만 읽는다 — context·pending(입장이 다음 턴 크기를 잰다)과 길이·끝 두 토큰·그림·마지막 토큰
앞까지의 해시(`_stale_hint` → `_offer_digest`). 파킹과 부팅 때의 한 번 읽기(`hold_parked`)가 모든 랭크에 남기고, 들고 있지 않은 기록은
재개가 블록과 함께 티어 스레드에서 읽는다: 루프는 기록을 디스크에서 읽지 않는다(D10). 디코드는
...
- 파킹의 단위는 **대화**이지 행이 아니다. `Runner.park_begin(row, key)`는 유휴 행의 모델 호스트 기록(`Model.park`: 토큰 이력·문맥,
  ...
  슬롯은 통째가 아니라 **이어 쓰는 데 필요한 바이트만** 간다(`Model.park_bytes` → `StateRings.live_bytes`, `kv_tier.Segments`): KDA·GDN
  링은 초안을 되돌리려고 층마다 상태를 K+1 개 두지만 다음 스텝은 `(context-1) % (K+1)` 하나만 읽고 나머지는 읽기 전에 쓴다(prefix 경계
  복원이 이미 기대는 규칙). GLM-5.3 한 대화의 슬롯 299,932,672 B(286 MiB) 중 50,371,584 B(48.0 MiB), Qwen3.8(K=3)은 114,645,248 B 중
  29,710,400 B 만 쓰고 읽는다. 재개는 슬롯을 먼저 0으로 지우고(`resume_bytes`, `open` 이 새 행에 하듯 — Qwen3.8 은 PLE id 링도 빈 칸으로)
  그 바이트를 되돌린다; 2026-09-19 전에 통째로 파킹된 대화는 manifest 의 `extra` 크기로 알아보고 통째로 읽는다.
