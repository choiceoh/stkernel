...
UMA 입장 검사는 공간이 부족하면 지정된 모델 보관 경로의 `.safetensors`와 다운로드 임시 파일에서 깨끗한 파일 캐시를 반환한다. 파일 내용·진행 중인 쓰기·다운로드 프로세스는 바꾸지 않는다. 최종 입장 조건은 아레나 + 작업 공간 상한 9GiB + OS 여유(SIGTERM 선 + 1GiB) + 접두사 티어 호스트 캐시이며, 준비 중에도 실제 여유와 할당 최고치를 검사한다. 작업 공간 상한은 2026-09-13 네 랭크 실측 최고치 7.48GiB(가장 큰 프리필 청크)에 여유를 더한 값이고, 더 쓰는 형상은 `--workspace-gib`(런처 `ST_WORKSPACE_GIB`)로 올린다.
