...
플릿(스파크 4대, 각 노드에 ST 이미지를 빌드한 뒤 노드당 컨테이너):
    ...
    STK_moe_static=t,r,sf6 bash launchers/start-st-glm53.sh   # 선언된 D11 노브는 STK_* 로 부팅에 들어간다(미선언·만료 = 사망; kernels/README.md)
...
- 파킹의 단위는 **대화**이지 행이 아니다. `Runner.park(row, key)`는 유휴 행의 블록과 상태 슬롯 바이트, 모델의 호스트
  ...
  티어가 되돌렸다). 쓰기 실패는 행을 유휴·상주로 남기고, 읽기 실패는 받은 행·슬롯·블록을 반납하고 디스크 사본을 보존한다.
  읽기 완료 후 디스크 사본 정리에 실패하면 복귀한 메모리는 유지한다. 보존 대화 수의 상한은 `max_seqs`가 아니라 디스크다.
...
가장 오래된 유휴 대화를 정리한다. 티어가 있으면 끝난 턴은 그 자리에서 파킹되어 행·슬롯이 곧바로 비고,
이어가기 요청은 파킹 기록의 문맥으로 전체 예산을 확인한 뒤 빈 행으로 복원한다. 티어가 꽉 차면 가장 오래
전에 파킹된 대화부터 잊고, 잊을 것이 없으면 그 턴의 대화는 보존하지 않는다. 대화 ID는 부팅을 넘겨 유효하다:
새 서버의 요청 번호는 티어에 남은 가장 큰 대화 ID 위에서 시작하고, 엔진이 죽어도 파킹된 대화는 디스크에 남는다.
알 수 없거나 실행 중·정리된 대화는 기존 요청을 건드리지 않고 409로 응답한다. 누적 요청 수와 보존 대화 수는
KV 행 수에 제한되지 않는다.
