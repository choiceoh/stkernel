...
- 디코드는 `BlockPool.reserve_many`로 배치 전체의 증가분을 먼저 검증·예약한다. 자원이 부족하면
  어느 행의 토큰 수도 바뀌지 않는다. 블록 풀과 러너 상태는 러너를 소유한 스레드에서 갱신한다.
...
EOS나 생성 한도에 도달하면 그 스텝에서 KV와 상태 슬롯을 반납한다. `draft_slots=0`인
계약을 사용한다. DFlash2 연결 전까지 이 런타임은 명시적인 eager greedy 검증 경로다.
...
이는 2개 실제 층(KDA+dense, DSA+NVFP4 MoE)의 실행·캐시 계약 검사다. 서빙 검사에서도
전문가 레인은 명시적으로 참조 구현을 사용한다. b12x 전문가 바인딩, DFlash2, 그래프 기반 요청 실행,
전체 45층 onepass 품질·처리량·ITL 판정은 이 결과에 포함하지 않는다.
