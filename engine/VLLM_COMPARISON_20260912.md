...
**결론부터: 대부분 이미 맞거나 의도적으로 다르다. 실제로 가져올 것은 여섯이고, 그중 하나가 크다.**
(§1.5 구조화 출력과 §1.6 디토크나이저는 09-12 에 뒤이어 붙었다.)
...
## 1-c. 그래서 한 것 (구조화 출력, 2026-09-12 — 원장 45차 §31)
...
### 1.6 스트리밍 디토크나이저 — 러스트 디코드 스트림과 복구 둘 (조사 뒤 추가)
...
## 1-d. 블록 수명 — 핀을 없앴다 (2026-09-12, 원장 §32)
...
128K 경계 기준 ~1 GB 읽기가 45 MiB 로 줄어든다(`kv_tier.promote(block_ids=None)`; 그 읽기를 재 보면 5.4 GB/s 에 8.7 ms).
...
(45 MiB × 96 = 4.24 GiB, 개수가 아니라 `boot.PREFIX_SNAPSHOT_GIB` 가 선언한 바이트)이 있어야 이어서 프리필할 수 있다 — 모델이 강제하는 것이지 빈 곳이 아니다. 그래서 희소한 자원이 블록이
아니라 스냅샷이고, 블록 수명은 1-d 에서 vLLM 의 규칙 그대로 고쳤다.
