...
| 접두사 캐시(하이브리드) | **ST 앞섬(입자 768 vs 1,152)**, 제약은 스냅샷 96개 |
...
**판정: 입자는 ST 앞섬.** 어느 쪽이 이 워크로드에서 실제로 더 아끼는지는 아직 안 쟀다 — 우리 쪽 미지수는
스냅샷 96개가 긴 프롬프트에서 얼마나 자기 것을 버리느냐이고, 그 숫자는 이미 나가고 있다.
...
4. **스냅샷 예산** — 입자는 우리가 곱지만(768 vs 1,152), 스냅샷이 96개라 73,728 토큰이 넘는 프롬프트는
   자기 경계를 스스로 축출한다. 계량기(`st:prefix_snapshot_self_evicts_total`)는 이미 나가고 **읽기만 하면 된다**.
5. ~~**운영 훅** — 접두사 캐시를 통째로 버리는 문이 없다.~~ → **`POST /v1/prefix/reset` 으로 넣었다**(45차 §55).

> **정정(이 문서의 첫 판)**: 4번을 *"우리 공유 단위는 프리필 청크 6,912 토큰이라 그 미만 공유 접두사를 전부
> 놓친다"* 라고 적었는데 **틀렸다**. `PrefixCache` 의 모듈 독스트링이 *"The unit is a chunk"* 라고 되어 있었지만
> 그건 45차 §23 **이전** 설계였고, 코드(`chain()`)는 그때부터 블록으로 돌고 있었다. 상대가 아니라 **우리 쪽을
> 코드로 확인하라**는 규칙을 내가 어겼다. 독스트링은 같이 고쳤다.
