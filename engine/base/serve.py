...
class Server:
    ...
    def submit(self, ids, max_new: int, temperature: float):
        ...
        with self._lock:
            ...
            self.arrivals.put((request, list(ids), max_new, float(temperature), blocks))
    ...
    def _admit(self):
        while self._waiting and self._free_rows:
            request, ids, limit, temperature, promised = self._waiting[0]
            ...
            if promised > self.runner.kv.available - future:
                break
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                try:
                    ...
                    request, event = server.submit(ids, req.get("max_tokens", 64), req.get("temperature", 0.0))
                    event.wait()
                    out = server.take_result(request)
                    text = server.tok.decode(out) if server.tok is not None else None
                    self.reply(200, {"seq": request, "ids": out, "text": text, "prompt_tokens": len(ids),
