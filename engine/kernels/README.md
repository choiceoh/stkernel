...
- **프로필이 선언하는 만료 노브** (`engine/profiles/glm53/boot.declared`, `STK_*`; 미선언·만료는 부팅 사망):
  `STK_moe_static` — b12x 정적 레인 사양, 프로덕션 `t,r,sf6,batch,q0`(09-09 채택값과 TP 레시피, 09-15 C=2 16행 M16 reform `batch` 추가), 비교용 `stock` 후보도 유지.
...
- **프로브 훅으로 남긴 것**(env 가 아니라 인자·모듈 속성; 서빙은 안 건드림): MLA 분할 강제 `mla_decode(splits=)`, MLA 루프라인 모드
  ...
  `kda/ring._BV_OVERRIDE`·`kda/kda._BV_OVERRIDE`(새 셀 BV 스윕용, `probes/engine_qwen38_kda.py`), QSA 발사 기하 고정
  `qsa._SPLIT_PROFILE_OVERRIDE`·`_SCORE_PROFILE_OVERRIDE`·`_INPUT_WARPS_OVERRIDE`, `qsa_select._WIDEST_OVERRIDE`·`_WARPS_OVERRIDE`
  (GB10 기하 스윕용, `probes/engine_qwen38_qsa_geometry.py`), 백엔드·컷오버·MAC 사다리 `moe_dispatch._GLM53_B12X_*`(직접 대입),
...
전문가의 FP8 활성값)는 수식이 달라 글루가 아니다. 아직 확정되지 않은 변형(sink `None`)은 어댑터가 거부하므로 판정표도
그 층을 none 으로 두고 노트에 확정 뒤 서빙할 후보를 적는다 — establish 레시피가 먼저고, 확정하면 마법사를 다시 돌린다. MLA 무장은 `glue.arm()` 이 `mla.maybe_arm(check=glue.check)` 로 하고,
