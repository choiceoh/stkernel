...
Example (Wrapper API with CUDA Graph):
    >>> from engine.kernels.b12x import B12xMoEWrapper
    >>> moe = B12xMoEWrapper(
...
from flashinfer.trace.templates.moe import b12x_fused_moe_trace, b12x_moe_wrapper_run_trace
...
def b12x_fused_moe(
    ...
    *,
    ...
    from .moe_dispatch import launch_sm120_moe
...
class B12xMoEWrapper:
    ...
    def run(
        ...
        *,
        ...
        # deneb fork: an explicit out= makes the caller's buffer the scatter
        # target, so the kernel writes the result once instead of once into
        # the static workspace and once more through the caller's copy_. The
        # copy this replaces wrote the same address under capture, which is
        # what proves the address replays -- direct write is equivalent.
        if out is not None:
            if (
                out.shape[0] < num_tokens
                or out.shape[1] != self.hidden_size
                or out.dtype != self.output_dtype
                or out.device != x.device
                or not out.is_contiguous()
            ):
                raise ValueError(
                    f"out must be a contiguous [{num_tokens}+, "
                    f"{self.hidden_size}] {self.output_dtype} tensor on "
                    f"{x.device}; got {tuple(out.shape)} {out.dtype} on "
                    f"{out.device}"
                )
            moe_output = out[:num_tokens]
