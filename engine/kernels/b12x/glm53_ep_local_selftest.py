...
def _runtime(wrapper, device):
    ...
    from engine.kernels.b12x import moe_dynamic_ep_local as local
    from engine.kernels.b12x import glm53_ep_route_remap as remap
    from engine.kernels.b12x import moe_micro_kernel as micro
...
def _weights(torch, device):
    ...
    sf13 = mma_sf_view(torch.stack([swizzle_sf(row) for row in s13]), 2*n, h)
    sf2 = mma_sf_view(torch.stack([swizzle_sf(row) for row in s2]), h, n)
