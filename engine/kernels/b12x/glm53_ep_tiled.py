...
def validate_configuration(owner):
    if os.environ.get("ST_GLM53_TP_SF6_Q0", "0") == "1":
        raise ValueError(f"{KNOB}=1 requires ST_GLM53_TP_SF6_Q0=0 for the EP owner")
    if "sf6" not in {part.strip() for part in os.environ.get(
            "ST_GLM53_B12X_STATIC_V2", "").split(",")}:
        raise ValueError(f"{KNOB}=1 requires lossless sf6 in ST_GLM53_B12X_STATIC_V2")
