...
class MoEGatedDynamicKernelSF6Prefill(MoEGatedDynamicKernelSF6Words):
    def _setup_attributes(self, hidden_size):
        if (hidden_size != 4096 or self.tile_shape_mnk != (128,128,128)
            ...
            raise ValueError("long-prefill SF6 route cache requires exact unshared M128 GLM geometry")
        if not stock_contract_matches():
            raise RuntimeError("long-prefill SF6 route cache inherited source changed")
        ...
        if (cute.size_in_bytes(self.a_dtype,self.a_smem_layout_staged) < 3*288*4
            ...
            raise ValueError("long-prefill SF6 route cache startup aliases exceed shared backing")
    ...
    def _check_sf6_shapes(self, w13, down, packed1, packed2):
        ...
        if (w13.shape[0] != 1024 or cute.size(w13.shape[1]) != 4096
            ...
            raise ValueError("long-prefill SF6 route cache requires E288/H4096/I512")
