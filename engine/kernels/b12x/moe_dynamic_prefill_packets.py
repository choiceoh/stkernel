...
"""Long-prefill SF6 with a packet load into the existing four-row BF16 stage.

The route producer below is pinned to the ordinary long-prefill producer.
...
PREFILL_SOURCE_SHA256 = '57f366b34a0452e664a3d41aa91add4100388e9f20071d0859af626c1b958d47'


@lru_cache(maxsize=1)
def stock_contract_matches():
    from . import moe_dynamic_gated_sf6_prefill as base
    return (base.stock_contract_matches()
            and hashlib.sha256(Path(base.__file__).read_bytes()).hexdigest() == PREFILL_SOURCE_SHA256)


@cute.jit
def stage_packet_input(a_input, batch_base, staged_tokens, tidx, threads, stage_address):
    # a_input carries the real [N,H] extent and an explicit packet base
    # pointer. This variant never reads that pointer as a BF16 global row.
    # The launch ABI includes the packet variant in its compilation identity.
...
class MoEGatedDynamicKernelSF6Packets(MoEGatedDynamicKernelSF6Prefill):
    def _setup_attributes(self, hidden_size):
        if not stock_contract_matches():
            raise RuntimeError('packet FFN inherited long-prefill producer changed')
    ...
    def initialize_route_q0_and_publish(self, thread_info, route_inputs, route_outputs, routing_state, task_queue, resident_barriers, shared_addresses, launch_params: DynamicLaunchParams):
        ...
        while produce_active > Int32(0):
            ...
            if batch_base >= producer_limit:
                ...
                stage_packet_input(a_input, batch_base, staged_tokens, Int32(tidx),
                                   Int32(self.threads_per_cta), q0_input_stage_base_addr)
