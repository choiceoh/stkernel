"""Bound-shape decode projections and 128-wide indexer boundary fusion.
...
class IndexerPair:
    def __init__(self, wk, gate, *, storage=None):
        ...
        if storage is None:  # standalone qualification; the serving owner supplies arena storage
            storage = torch.empty((2 * self.head, self.hidden), dtype=wk.dtype, device=wk.device)
        if (storage.shape != (2 * self.head, self.hidden) or storage.dtype != wk.dtype
                or storage.device != wk.device or not storage.is_contiguous()):
            raise ValueError('indexer pair storage must match the bound pair shape, device and BF16 dtype')
        storage[:self.head].copy_(wk)
        storage[self.head:].copy_(gate)
        self.weight = storage
