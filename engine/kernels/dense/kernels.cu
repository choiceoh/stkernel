...
template <int RQ, bool LR, bool COMPACT = false, bool PACKED_INPUT = false>
...
void set_kernel_attrs() {
  ...
  MK_CHECK_CUDA(cudaFuncSetAttribute(
      mk_gemm2_kernel<2, false, false, true>, cudaFuncAttributeMaxDynamicSharedMemorySize, GEMM2_SMEM));
  MK_CHECK_CUDA(cudaFuncSetAttribute(
      mk_gemm2_kernel<4, false, false, true>, cudaFuncAttributeMaxDynamicSharedMemorySize, GEMM2_SMEM));
  MK_CHECK_CUDA(cudaFuncSetAttribute(
...
void mk_run_gemm_impl(torch::Tensor x, torch::Tensor wq4, torch::Tensor ws4,
  ...
  if (wide_input) {
    ...
    if (c2.m <= 16)
      mk_launch(mk_gemm2_kernel<2, false, false, true>, nblk * c2.ksr, GEMM2_SMEM, stream, c2);
    else
      mk_launch(mk_gemm2_kernel<4, false, false, true>, nblk * c2.ksr, GEMM2_SMEM, stream, c2);
  } else if (input_reuse) {
