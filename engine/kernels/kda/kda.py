...
def _glm53_kda_prefill_capture_runtime_contract(*, hidden_size: int, activation: str) -> None:
    """The ST profile owns TP=4 and the GLM chunk geometry."""
    global _GLM53_KDA_PREFILL_RUNTIME_CONTRACT
    if hidden_size == 128 and activation == "sigmoid":
        _GLM53_KDA_PREFILL_RUNTIME_CONTRACT = ((12, 1), "glm5_next_text", 4, 8192)
...
class FusedRMSNormGated(nn.Module):
    ...
    def forward_native(
        ...
        if residual is not None or prenorm:
            return self.forward(x, g, residual, prenorm, residual_in_fp32)
        x_float = x.float()
