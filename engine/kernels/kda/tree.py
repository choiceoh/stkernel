...
from engine.modules.tree_kda import Factors
...
@triton.jit
def _verify(Q, K, VEC, DEC, BET, INITIAL, PATH, DEPTH, UPDATE, OUT,
            N: tl.constexpr, H: tl.constexpr, KDIM: tl.constexpr, VDIM: tl.constexpr,
            WIDTH: tl.constexpr, BK: tl.constexpr, BV: tl.constexpr):
...
def verify(tree, q, k, v, g, beta, a, bias, initial, lower_bound):
    n, h, d = q.shape
    vd = v.shape[-1]
    if not q.is_cuda or d > 256 or vd > 256:
        raise ValueError("native KDA tree requires CUDA and head dimensions <= 256")
    paths = [tree.path(i)[:-1] for i in range(n)]
    width = max(1, max(map(len, paths)))
    indices = torch.tensor([p + (-1,) * (width - len(p)) for p in paths], dtype=torch.int32, device=q.device)
    depths = torch.tensor([len(p) for p in paths], dtype=torch.int32, device=q.device)
    ...
    _verify[(h, triton.cdiv(vd, 16))](qf, kf, v.contiguous(), decay, bet, initial, indices, depths, update, out,
                                    n, h, d, vd, width, triton.next_power_of_2(d), 16, num_warps=4)
