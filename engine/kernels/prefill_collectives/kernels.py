...
def _pack_rs_payload(X, Packed, Scales, N, LOCAL_N, PAYLOAD_BYTES,
                     BLOCK: tl.constexpr, Roundtrip=None):
    block = tl.program_id(0)
    local_blocks = LOCAL_N // BLOCK
    rank = block // local_blocks
    local_block = block % local_blocks
    ...
    if Roundtrip is not None:
        # The sender router must see the same BF16 values as the receiver's
        # unpack, including the intervening FP8 rounding. Emit only its shard.
        tl.store(Roundtrip + offsets, q.to(tl.float32)*scale, offsets < N)
    ...
    if local_block == local_blocks - 1:
        ...
        activation_end = ((scale_end+31)//32)*32
        tl.store(Scales + rank * (PAYLOAD_BYTES // 4) + scale_end + tail, 0.0,
                 mask=scale_end + tail < activation_end)
