"""Sparse attention over selected positions (module): the torch reference semantics the served kernels are held
to -- DSv4.1's sparse_attn_kernel with its sink term (-1 the only sentinel, the running max seeded at -1e30 so an
all-(-1) row is zero rather than NaN), GLM-5.3's sparse MLA in MQA form, Qwen3.8's sparse GQA. The attention
FEATURE (the family: forms, rotation, norms, gates, selections, the sink) is engine/modules/attention.py.
"""
