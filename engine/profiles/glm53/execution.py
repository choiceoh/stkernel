...
class ExecutionPlan:
    ...
    def __post_init__(self):
        if any(type(v) is not bool for v in (self.overlap, self.early_observe, self.direct_mhc, self.prefill_project_tiles, self.deferred_kda, self.terminal_mhc, self.prefill_indexer_shards, self.prefill_dense_prefix, self.prefill_absorb_tiles, self.decode_fastpaths, self.prefill_ffn_packets, self.decode_dsa_inputs, self.decode_indexer_gate, self.decode_absorb_tiles)):
    ...
    def active(self):
        return (self.overlap or self.early_observe or self.prefill_tiles != 1 or self.direct_mhc
                ...
                or self.prefill_indexer_shards or self.prefill_dense_prefix or self.prefill_absorb_tiles or self.decode_fastpaths or self.prefill_ffn_packets or self.decode_dsa_inputs or self.decode_indexer_gate or self.decode_absorb_tiles)
