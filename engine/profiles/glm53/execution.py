...
class ExecutionPlan:
    ...
    def active(self):
        return (self.overlap or self.early_observe or self.prefill_tiles != 1 or self.direct_mhc
                ...
                or self.prefill_indexer_shards or self.prefill_dense_prefix or self.prefill_absorb_tiles or self.decode_fastpaths or self.prefill_ffn_packets or self.decode_dsa_inputs or self.decode_indexer_gate, self.decode_absorb_tiles)
    ...
    def label(self):
        return (f"tp_overlap={int(self.overlap)},early_observe={int(self.early_observe)},"
                ...
                f"deferred_kda={int(self.deferred_kda)},compact_kda={int(self.compact_kda)},terminal_mhc={int(self.terminal_mhc)},"
