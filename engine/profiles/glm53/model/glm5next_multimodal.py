# [glm53 overlay: video placeholder per temporal group -- see _construct_video_placeholder]
...
from engine.base.comm import get_tensor_model_parallel_world_size
from vllm.distributed import parallel_state  # TODO: still vLLM
from vllm.distributed import utils  # TODO: still vLLM
from engine.profiles.glm53.shims import SiluAndMulWithClamp
from vllm.model_executor.layers.attention import MMEncoderAttention  # TODO: still vLLM
from vllm.model_executor.layers.conv import Conv2dLayer, Conv3dLayer  # TODO: still vLLM
from engine.modules.norm import RMSNorm
from engine.modules.linear import ColumnParallelLinear, MergedColumnParallelLinear, QKVParallelLinear, RowParallelLinear
from engine.modules.nvfp4_linear import QuantizationConfig
from engine.modules.rotary import get_rope
from vllm.model_executor.layers.rotary_embedding.common import ApplyRotaryEmb  # TODO: still vLLM
from vllm.model_executor.models.glm4_1v import Glm4vMultiModalProcessor, Glm4vProcessingInfo  # TODO: still vLLM
from engine.base.loader import AutoWeightsLoader, WeightsMapper
from vllm.model_executor.models.vision import get_vit_attn_backend, is_vit_use_data_parallel  # TODO: still vLLM
from vllm.models.common.ops import fused_q_kv_rmsnorm  # TODO: still vLLM
from vllm.multimodal.parse import ImageSize, MultiModalDataItems  # TODO: still vLLM
from vllm.v1.attention.backends.registry import AttentionBackendEnum  # TODO: still vLLM
...
class Glm5NextProcessingInfo(Glm4vProcessingInfo):
    ...
    # ------------------------------------------------------------------
    # [glm53-video] 39차: the inherited GLM-4.6V placeholder emits one image
    # block per SAMPLED FRAME (len(video_array) timestamps), but this
    # checkpoint's video processor (the training-side port above) merges
    # frames in pairs (temporal_patch_size=2) and hands the vision tower
    # video_grid_thw with T = sampled/2 groups. The prompt then carried 2x
    # the placeholders the encoder produced and the worker died with
    ...
    def _construct_video_placeholder(self, video_array, metadata, grid_thw):
        ...
        vp = hf_processor.video_processor
        merge_length = hf_processor.image_processor.merge_size ** 2
        tps = int(getattr(vp, "temporal_patch_size", 2) or 2)
        assert isinstance(grid_thw, torch.Tensor)
        T, H, W = (int(x) for x in grid_thw.tolist())
        num_tokens_per_group = int(H * W) // merge_length

        fps = float(metadata.get("fps") or 0.0) or 1.0
        total = int(metadata.get("total_num_frames") or len(video_array))
        if metadata.get("do_sample_frames", True):
            indices = glm_sample_frame_indices(
                total, fps, float(metadata.get("duration") or 0.0),
                target_fps=getattr(vp, "fps_interval", None),
                max_frame_count=getattr(vp, "max_frame_count_dynamic", None),
                temporal_patch_size=tps,
            )
        else:
            indices = [int(i) for i in metadata.get("frames_indices", range(len(video_array)))]
            if len(indices) % tps:
                indices.append(indices[-1])
        groups = [indices[i:i + tps] for i in range(0, len(indices), tps)]
        if len(groups) != T:
            # geometry disagreement: follow the encoder (T groups), spread stamps
            groups = [groups[min(g, len(groups) - 1)] if groups else [0] for g in range(T)]
        seconds = [grp[0] / fps for grp in groups]

        boi, eoi = hf_config.image_start_token_id, hf_config.image_end_token_id
        bov, eov = hf_config.video_start_token_id, hf_config.video_end_token_id
        frame_embed_token_id = self._get_video_frame_embed_token_id(hf_processor)
        placeholder = [bov]
        for sec in seconds:
            placeholder.append(boi)
            placeholder.extend([frame_embed_token_id] * num_tokens_per_group)
            placeholder.append(eoi)
            placeholder.extend(tokenizer.encode(f"{sec:.1f} seconds", add_special_tokens=False))
        placeholder.append(eov)
        return placeholder
