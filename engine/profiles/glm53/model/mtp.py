...
from engine.profiles.glm53.shims import fused_moe_make_expert_params_mapping
from engine.modules.norm import RMSNorm
from engine.modules.logits import LogitsProcessor
from engine.modules.logits import VocabParallelEmbedding
from engine.base.loader import default_weight_loader, maybe_remap_kv_scale_name
from engine.modules.logits import SharedHead
from engine.modules.moe import DeepseekV2MixtureOfExperts
...
class Glm5NextMultiTokenPredictorLayer(nn.Module):
    def __init__(self, vllm_config: VllmConfig, prefix: str) -> None:
        ...
        # deneb fork (37차, 2026-09-06): the checkpoint's MTP block keeps FP8
        # experts while the base model's are NVFP4, and the fleet's MoE backend
        # (flashinfer_b12x) refuses FP8 -- so the native-MTP boot died at this
        # line ("moe_backend='flashinfer_b12x' is not supported for FP8 MoE").
        # Build the MTP block under an FP8-capable backend (VLLM_GLM53_MTP_MOE_
        # BACKEND, default "auto") and restore the fleet's backend afterwards;
        # every base-model layer is already built by the time this runs.
        import os as _os
        _kc = vllm_config.kernel_config
        _saved_backend = _kc.moe_backend
        _kc.moe_backend = (_os.environ.get("VLLM_GLM53_MTP_MOE_BACKEND") or "auto").strip()
        try:
            self.mtp_block = self._build_mtp_block(vllm_config, config, layer_idx, prefix, topk_indices_buffer)
        finally:
            _kc.moe_backend = _saved_backend

    def _build_mtp_block(self, vllm_config, config, layer_idx, prefix, topk_indices_buffer):
