...
8192 and seven draft slots, where 2304 is the Mamba/KDA cache block size in
"align" mode (--block-size 2304, VLLM_GLM53 APC). MAX_BATCHED=9216 did not
move it -- seven tokens short. The self-check below IS that finding: if it
...
SPEC_K = 7            # facts.SPEC_K=7 (operator 2026-09-13): DFlash2 draft slots out of the token budget
...
def _selfcheck() -> None:
    ...
    assert chunk_for(9216) == 6912                           # seven tokens short: still 6,912
    assert chunk_for(9223) == 9216                           # N x 2304 + SPEC_K is the only way up
    assert chunk_for(10240) == 9216                          # what ST serves: four blocks (조사 19차)
    assert chunk_for(32768) == 32256                         # (32768-7)//2304 = 14 blocks
    assert chunk_for(16384) == 16128                         # (16384-7)//2304 = 7 blocks
    print("  glm53 shapes: 8192->6,912, 9216->6,912, 9223->9,216, 10240->9,216 -- the chunk law reproduces OK")
