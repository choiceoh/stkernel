...
class CompressorMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        if common_attn_metadata.max_query_len <= 1 and num_tokens == num_reqs:
            ...
            # deneb fork: port of upstream PR #47474 -- build via the shared
            # helper so the sparse MLA and sparse SWA builders reuse this instead
            # of each repeating the repeat_interleave + pin + H2D copy.
            token_to_req_indices = common_attn_metadata.token_to_req_indices(
                self.token_to_req_indices
            )
        # Fast paths above skip the helper, so publish their result too; the
        # compressor is then always the cache's owner (see seed_token_to_req_indices).
        common_attn_metadata.seed_token_to_req_indices(token_to_req_indices)
