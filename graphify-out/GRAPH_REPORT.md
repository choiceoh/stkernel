...
## Corpus Check
- 54 files · ~111,652 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1591 nodes · 4399 edges · 31 communities detected
- Extraction: 55% EXTRACTED · 45% INFERRED · 0% AMBIGUOUS · INFERRED: 1990 edges (avg confidence: 0.62)
...
Cohesion: 0.03
Nodes (145): carve(), _layout(), PagedSpec, Plan, What each layer kind needs cached, declared once, sized from the budget (base)., Bind the plan to the arena: one KV region, one slot region, two pools., A spec that names a key also names a dtype and a shape whose bytes are its decla, Split a KV budget: slots first (fixed by max_seqs), blocks with the rest.      B (+137 more)

### Community 1 - "Serving Diagnostics"
Cohesion: 0.03
Nodes (141): DiagnosticMetrics, Exception, The disk cannot take this conversation; forget one or drop this one., TierFull, Recorder, answer_budget(), _byte_level_chars(), cache_key() (+133 more)

### Community 2 - "Device Memory Arena"
Cohesion: 0.02
Nodes (74): Arena, expandable_segments(), Back every new caching-allocator segment with 20 MiB physical chunks     under o, Free the one allocation now, whatever still points at it, and say how many bytes, Give every block of `seq` back, TAIL FIRST -- the head of a prompt is what the n, Off-thread I/O. Poll `.done()`, then `.result()` to surface failures.          T, The pool this cache claims blocks in; the pool tells it when a claimed block has, boundary tokens -> hash of the prompt up to there, for every whole BLOCK (45차 §2 (+66 more)

### Community 3 - "Fleet Lease & Latency"
Cohesion: 0.03
Nodes (79): host_reclaim(), _meminfo(), prepare_allocation(), One allocation for persistent weights and caches (base).  D16 in code: weights (, Ask this node's host to return its clean file cache now, and wait for it to say, Drop clean pages of the supplied weight files, reclaim the rest of the     short, Hold `nbytes` of anonymous memory for an instant, then give it back.      MemAva, Return clean model-download pages without allocating or changing files.      The (+71 more)
...
Cohesion: 0.05
Nodes (66): Background, Host work a boot runs where it is already waiting (base): a thread started early, Host work started where the boot is already waiting, and joined where its result, Budget, Line, probe_box(), rank_files(), The box, declared -- not discovered (framework).  A budget is a list of claims o (+58 more)
...
Cohesion: 0.04
Nodes (44): A uint8 view of `nbytes`, or MemoryError -- never a second allocation (D3)., Region, _selfcheck(), Checkpoint, _LazyDtypes, Read exactly these tensors as coalesced ranges, one shard at a time.          Ea, {name: tensor} with every tensor a read-only view of its shard's mapped bytes (n, safetensors dtype name -> (the numpy element type the bytes are read as, the tor (+36 more)

### Community 8 - "Request Cache"
Cohesion: 0.04
Nodes (61): rank_weights(), (GiB, tensor count) read out of a safetensors header.      The header is a const, _float32(), _lsr(), mix(), mix_tensor(), Stateless uniforms: every draw is a function of what it is for, never of what ca, Logical shift right of an int64 tensor: torch shifts arithmetically, the mask dr (+53 more)
...
- **Why does `BlockPool` connect `Memory Plan` to `Device Memory Arena`, `Runtime Introspection`, `Fleet Lease & Latency`?**
...
- **Why does `numel()` connect `Fleet Lease & Latency` to `Memory Plan`, `Serving Diagnostics`, `Distributed Communication`, `Memory Budget Gates`, `Request Cache`, `Sampling Constants`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Are the 138 inferred relationships involving `TierFull` (e.g. with `CompressedSnapshots` and `Model`) actually correct?**
  _`TierFull` has 138 INFERRED edges - model-reasoned connections that need verification._
