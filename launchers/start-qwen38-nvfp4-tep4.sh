...
ct_load_profile "$_repo/profiles/qwen38.env" \
  ...
  PLE_SSD PLE_SSD_DIR KV_CACHE_MEMORY \
  FORCE_FP8_EMBED CUDAGRAPH_MODE SPEC_TOKENS ADAPTIVE_SPEC NGRAM_FIX \
  QSA_MAX_SPLITS ALL2ALL ASYNC AUTOTUNE LOAD_FORMAT FUSE SHARED_FUSE ONESHOT_AR
...
-e DENEB_PLE_SSD=${PLE_SSD:-0} -e DENEB_PLE_SSD_DIR=${PLE_SSD_DIR:-} -e VLLM_DSV4_ONESHOT_AR=${ONESHOT_AR:-0} \
