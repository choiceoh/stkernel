...
#
# Rank order is base/comm.NODES (srv2, srv1, srv3, srv4); the head (MASTER_ADDR) is srv2.
...
for r in "${!NODES[@]}"; do
  ip=${NODES[$r]}
  echo "== rank $r on $ip"
  rsync -a --delete -e "ssh $SSHOPT" --exclude __pycache__ "$REPO/engine" "$REPO/overlay" "$REPO/launchers" "choiceoh@$ip:$ENGINE_DIR/"
  ...
  node_sh "$ip" "docker rm -f $NAME >/dev/null 2>&1 || true; docker run -d --name $NAME --gpus all --restart no \
    ...
    --entrypoint /bin/bash $IMAGE -lc 'source /repo/launchers/lib/common-tp4.sh; eval \"\$CT_GID_PRELUDE\"; cd /repo && PYTHONPATH=/repo exec python3 engine/profiles/glm53/boot.py --port $PORT' >/dev/null && echo '$ip: started'"
done
