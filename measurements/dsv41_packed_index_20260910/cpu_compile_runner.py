#!/usr/bin/env python3
"""Normal fleet CPU payload: compile the packed indexer without GPU devices.
...
INPUTS = (
    ...
    "tests/test_dsv41_packed_index.py",
    "tests/test_dsv41_packed_reference_adapter.py",
    "measurements/dsv41_packed_index_20260910/cpu_compile_runner.py",
)
...
from dsv41_packed_index_triton import offline_compile
compiled = offline_compile(out / 'compiled')
assert len(compiled) == 4, 'expected H8/H32 dense and compact specializations'
assert not torch.cuda.is_initialized()
...
def main():
    ...
    name = "dsv41-packed-index-cpu-" + str(os.getpid())
