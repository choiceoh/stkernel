...
def sample(sources, depth):
    ...
    try:
        ...
        for name, source in sources.items():
            (fixture.repo / name).write_text(source)
        def cli(*args):
...
def main():
    ...
    parser.add_argument('--baseline',default='bb123cfa1fc47e2b9e87fde3b55eb4400412785d')
    ...
    files = ['bench/experiments.py','bench/experiment_resources.py',
             'bench/experiment_retirement.py','bench/cpu_unittest.py','bench/cpu_evidence.py']
    sources = dict(baseline={name:subprocess.check_output(
        ['git','-C',str(ROOT),'show',baseline+':'+name],text=True) for name in files},
        candidate={name:(ROOT/name).read_text() for name in files})
    result = dict(baseline_revision=baseline,python=sys.version,platform=platform.platform(),
                  ...
                  source_sha256={variant:{name:hashlib.sha256(value.encode()).hexdigest() for name,value in files.items()}
                                 for variant,files in sources.items()},
                  samples=[],summary={})
