...
PLAN = Path('/tmp/m8-fp8-cache-plan.json')
LIST = Path('/tmp/m8-fp8-cache-files.list')
CUTOFF = time.mktime(time.strptime('2026-09-07 07:01:00', '%Y-%m-%d %H:%M:%S'))
...
if sys.argv[1] == 'plan':
    ...
    data = dict(source=str(ROOT), cutoff='2026-09-07 07:01:00 KST', total_bytes=total, entries=entries)
