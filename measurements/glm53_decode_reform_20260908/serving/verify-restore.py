...
KNOB_NAMES = ('VLLM_GLM53_MK_INPUT_CTA', 'VLLM_GLM53_MK_INPUT_REUSE',
...
def main():
    ...
    knobs = {}
    for line in (args.restore_repo/'profiles/glm53.env').read_text().splitlines():
        key,sep,value=line.partition('=')
        if sep and key in KNOB_NAMES:
            knobs[key]=value.split('#',1)[0].strip()
    assert set(knobs)==set(KNOB_NAMES) and all(knobs.values())
    ...
    for rank,node in enumerate((2,1,3,4)):
        ...
        code = ('import json,subprocess; '
                ...
                f'knobs={{k:e.get(k) for k in {list(KNOB_NAMES)!r}}})))')
        def run(command):
            if node != 2:
