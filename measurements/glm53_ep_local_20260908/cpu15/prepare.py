#!/usr/bin/env python3
"""Prepare/submit CPU15 on the head only, after an explicit committed revision.
...
BASE = '881456a1f43fcf61de1bb5822b883dd2fd32e693'
BUNDLE = Path('/tmp/glm53-ep-local-0908-15.bundle')
REPORT = Path('/tmp/glm53-ep-local-cpu15-launch.json')
...
def remote(mode, revision, bundle_sha=''):
    ...
    value = run(['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=8', 'choiceoh@srv2', shlex.join(command)],
                'CPU15 remote ' + mode, stdin=subprocess.DEVNULL)
...
def main():
    ...
    run(['git', 'merge-base', '--is-ancestor', BASE, revision], 'CPU14 base ancestry', cwd=ROOT)
    ...
    if not info['source_exists']:
        if not BUNDLE.exists():
            run(['git', 'bundle', 'create', str(BUNDLE), 'HEAD', '^' + BASE], 'CPU15 bundle create', cwd=ROOT)
        heads = run(['git', 'bundle', 'list-heads', str(BUNDLE)], 'CPU15 bundle identity', cwd=ROOT).splitlines()
        if heads != [revision + ' HEAD']:
            raise RuntimeError('CPU15 bundle revision differs; refusing path replacement')
        digest = hashlib.sha256(BUNDLE.read_bytes()).hexdigest()
        if info['bundle_sha256'] is None:
            run(['scp', '-q', str(BUNDLE), 'choiceoh@srv2:' + str(BUNDLE)], 'CPU15 bundle transfer')
        elif info['bundle_sha256'] != digest:
            raise RuntimeError('remote CPU15 bundle differs; refusing path replacement')
