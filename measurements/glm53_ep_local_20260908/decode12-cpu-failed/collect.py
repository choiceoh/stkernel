#!/usr/bin/env python3
"""Read-only collection of the already completed CPU12 receipt; never runs tests."""
import base64
import hashlib
import io
import json
...
REVISION = 'cb3f5c754a7c4a65a89affd0c436f4ea570014b0'
SOURCE = '/home/choiceoh/stkernel-ep-onepass-0909-12'
WORKER = 'choiceoh@10.10.10.1'
EVIDENCE = '/home/choiceoh/glm53-ep-cpu-0909-12/evidence'
JOB = '/tmp/glm53-ep-decode-cpu-0909-12'
...
root=pathlib.Path('/home/choiceoh/glm53-ep-cpu-0909-12/evidence')
source=pathlib.Path('/home/choiceoh/stkernel-ep-onepass-0909-12')
...
source=pathlib.Path('/home/choiceoh/stkernel-ep-onepass-0909-12')
job=pathlib.Path('/tmp/glm53-ep-decode-cpu-0909-12')
def git(*args):return subprocess.check_output(['git','-C',str(source),*args],text=True).strip()
files={}
...
if (job/'evidence').exists():assert head_evidence==worker_data['files']
else:head_evidence=None
print(json.dumps({'time':time.time(),'revision':git('rev-parse','HEAD'),'status':git('status','--porcelain'),
 'shallow':git('rev-parse','--is-shallow-repository'),'alternates':(source/'.git/objects/info/alternates').exists(),
...
def main():
    ...
    capture = dict(schema=1, scope='read-only archive of completed CPU12',
