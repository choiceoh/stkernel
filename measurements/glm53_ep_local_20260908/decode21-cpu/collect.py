#!/usr/bin/env python3
"""Read-only collection of the already completed CPU21b receipt; never runs tests."""
import base64
import hashlib
import io
import json
...
WORKER = 'choiceoh@10.10.10.4'
EVIDENCE = '/home/choiceoh/glm53-ep-cpu-0909-21/evidence'
JOB = '/tmp/glm53-ep-decode-cpu-0909-21b'
...
root=pathlib.Path('/home/choiceoh/glm53-ep-cpu-0909-21/evidence')
...
source=pathlib.Path('/home/choiceoh/stkernel-ep-onepass-0909-21')
job=pathlib.Path('/tmp/glm53-ep-decode-cpu-0909-21b')
def git(*args):return subprocess.check_output(['git','-C',str(source),*args],text=True).strip()
files={}
...
worker=subprocess.run(['ssh','-o','BatchMode=yes','-o','ConnectTimeout=5','choiceoh@10.10.10.4',shlex.join(['python3','-B','-c',sys.argv[1]])],capture_output=True,text=True,timeout=30)
...
def main():
    ...
    capture = dict(schema=1, scope='read-only archive of completed CPU21b',
