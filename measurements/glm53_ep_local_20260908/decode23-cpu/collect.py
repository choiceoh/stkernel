#!/usr/bin/env python3
"""Read-only CPU23 collection on srv2/head; never compiles or submits work."""
...
OUT=ROOT/'measurements/glm53_ep_local_20260908/decode23-cpu'
REVISION='cbf1c7916247f946167c15ff74d92261588e8cea'
EXPECTED_TESTS=165
SOURCE='/home/choiceoh/stkernel-ep-onepass-0909-23'
JOB='/tmp/glm53-ep-decode-cpu-0909-23'
...
def command():return ['bash',SOURCE+'/bench/fleet.sh','run','--cpu','epdecodecpu0909v23','6','TP SF6 Q0 stock and candidate actual lowering; no devices, 4GiB and 2CPU','--','python3','-B',SOURCE+'/probes/run_glm53_ep_short_decode_cpu.py','--image',IMAGE,'--output',JOB+'/evidence','--capsule-root',CAPSULE,'--manifest-sha256',CAPSULE_SHA]
...
assert completion['returncode']==0 and completion['ended']>=completion['started']>=submission['created']
evidence={};total=0
...
def main():
 require(re.fullmatch('[0-9a-f]{40}',REVISION) and type(EXPECTED_TESTS)is int and EXPECTED_TESTS>0,'bind actual CPU23 revision and successful test count first')
 ...
 with tempfile.TemporaryDirectory(prefix='glm53-cpu23-artifact-check-') as temp:
  directory=Path(temp)
