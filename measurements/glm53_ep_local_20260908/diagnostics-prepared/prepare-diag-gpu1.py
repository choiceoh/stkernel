#!/usr/bin/env python3
"""Freeze and normally enqueue a CPU17-matched concentrated-case diagnostic.
...
CPU17 receipt. No CPU/GPU test runs here; the normal fleet wrapper owns GO-time
admission, sanitizer availability, resource/capsule/source checks and exact
service restoration. This is diagnostic-only; it cannot promote a default or
complete the full GPU gate. CPU17 must actually pass first; no CPU16 fallback.
"""
import argparse
import ast
import base64
...
BUNDLE = Path('/tmp/glm53-ep-local-diag-gpu-0908-1.bundle')
REPORT = Path('/tmp/glm53-ep-local-diag-gpu-0908-1-launch.json')
HOST = 'choiceoh@srv2'
...
def prepare():
    require(not JOB.exists(),'diagnostic GPU v1 job path already exists; never resubmit or overwrite')
    no_duplicate()
    ...
    if SOURCE.exists():
        frozen(revision)
        receipt_parent(revision,cpu['revision'])
    ...
    require(cpu==payload['cpu_state'] and approved==payload['scheduler_state'],
            'CPU17 or approved scheduler changed after initial inspection')
    ...
    if not SOURCE.exists():
        ...
        # Fetch the explicitly supplied, published receipt revision only.
        # Depth 2 is valid only for an immediate child of the CPU17 source;
        # both the local parent check and the frozen check below enforce it.
        # The immutable local bundle remains independently validated above.
        run(['git','init','--quiet',str(SOURCE)],'diagnostic GPU v1 fresh init')
    ...
    require((SOURCE/PROOF).read_bytes()==proof_bytes,'bundle CPU17 receipt differs')
    proof_and_capsule(SOURCE,SOURCE/PROOF,payload['cpu_summary'])
    # The production runner must itself consume CPU17, not a stale evidence path.
    import run_glm53_ep_local_offline as offline
    require(offline.CPU_EVIDENCE==Path(PROOF),'full runner does not select CPU17 evidence')
    ...
    compile(driver_bytes,'diagnostic-gpu-v1-driver.py','exec')
    no_duplicate()
    ...
    metadata=dict(source=str(SOURCE),revision=revision,source_clean=True,scheduler=str(SCHEDULER),
        ...
        command=diagnostic_command(revision),created=time.time(),session=SESSION,
        diagnostic_only=True,mode='diagnostic',diagnose_case='concentrated6912',
        performance_acceptance=False,full_gpu_acceptance=False,default_promotion=False,
...
def remote(mode, payload):
    ...
    return json.loads(run(['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=10', HOST,
                          shlex.join(command)], 'diagnostic GPU v1 remote ' + mode))
...
def transfer_bundle(digest):
    ...
    run(['scp', '-q', str(BUNDLE), HOST + ':' + incoming], 'diagnostic GPU v1 bundle upload')
...
def main():
    ...
    parser.add_argument('--revision', required=True, help='Exact clean committed HEAD including actual CPU17 PASS receipt')
    ...
    if (ROOT / PROOF).read_bytes() != committed:
        raise RuntimeError('current CPU17 receipt differs from committed HEAD')
    sys.dont_write_bytecode = True
    sys.path.insert(0, str(ROOT / 'probes'))
    from glm53_ep_local_evidence import validate_compile_evidence, CONTRACT_PATHS, compile_cases
    ...
    if set(proof['contracts']['files']) != set(CONTRACT_PATHS):
        raise RuntimeError('CPU17 contract file set differs from current source')
    summary = dict(tests_run=proof['contracts']['tests_run'], mounted_source_count=len(proof['mounted_sources']),
                   contract_source_count=len(proof['contracts']['files']), remap_compile_count=len(proof['remap_compilation']),
                   binding_runtime=validate_runtime_receipt(proof['binding_runtime']))
    if (type(summary['tests_run']) is not int or summary['tests_run'] != 146
            or summary['mounted_source_count'] != 13 or summary['contract_source_count'] != 29
            or summary['remap_compile_count'] != len(compile_cases()) or len(compile_cases()) != 24):
        raise RuntimeError('incomplete current CPU17 proof coverage')
    ...
    run(['git', 'merge-base', '--is-ancestor', base, args.revision], 'CPU17 source ancestry', cwd=ROOT)
    parents = run(['git', 'rev-list', '--parents', '-n', '1', args.revision], 'CPU17 receipt parent', cwd=ROOT).split()
    if parents != [args.revision, base]:
        raise RuntimeError('depth-2 diagnostic source requires the receipt commit to be the immediate single child of CPU17 source')
    if not BUNDLE.exists():
        run(['git', 'bundle', 'create', str(BUNDLE), 'HEAD', '^' + base], 'immutable diagnostic GPU v1 bundle create', cwd=ROOT)
    if run(['git', 'bundle', 'list-heads', str(BUNDLE)], 'bundle identity', cwd=ROOT).splitlines() != [args.revision + ' HEAD']:
        raise RuntimeError('existing diagnostic GPU v1 bundle differs; no replacement')
    digest = hashlib.sha256(BUNDLE.read_bytes()).hexdigest()
    if inspected['bundle_sha256'] is None:
        transfer_bundle(digest)
    elif inspected['bundle_sha256'] != digest:
        raise RuntimeError('existing remote diagnostic GPU v1 bundle differs; no replacement')
