#!/usr/bin/env python3
"""Prepare/submit CPU16 on the head only, after an explicit committed revision.
...
BASE = '30608530f0c908bc5f81db6bdb69fd70ccee9a24'
BUNDLE = Path('/tmp/glm53-ep-local-0908-16.bundle')
REPORT = Path('/tmp/glm53-ep-local-cpu16-launch.json')
...
source=pathlib.Path('/home/choiceoh/stkernel-ep-local-0908-16')
previous=pathlib.Path('/home/choiceoh/stkernel-ep-local-0908-15')
job=pathlib.Path('/tmp/glm53-ep-local-compile0908-16-head')
bundle=pathlib.Path('/tmp/glm53-ep-local-0908-16.bundle')
...
def frozen():
    ...
    if run(['git','-C',str(source),'rev-parse','HEAD'],'frozen revision')!=revision:
        raise RuntimeError('existing CPU16 source has another revision; refusing replacement')
    if run(['git','-C',str(source),'status','--porcelain'],'frozen cleanliness'):
        raise RuntimeError('existing CPU16 source is dirty; refusing replacement')
    return True
...
def existing_job():
    ...
    if not metadata.is_file():
        raise RuntimeError('CPU16 job path exists without submission metadata; inspect manually')
    data=json.loads(metadata.read_text())
    if data['revision']!=revision:
        raise RuntimeError('existing CPU16 job has another revision; refusing replacement')
...
def prepare():
    ...
    if not reusable:
        if not bundle_sha or not bundle.is_file() or hashlib.sha256(bundle.read_bytes()).hexdigest()!=bundle_sha:
            raise RuntimeError('CPU16 bundle is missing or changed')
        run(['git','-C',str(previous),'cat-file','-e','30608530f0c908bc5f81db6bdb69fd70ccee9a24^{commit}'],'CPU15 base prerequisite')
        run(['git','clone','--quiet','--shared','--no-checkout',str(previous),str(source)],'CPU16 clone')
        run(['git','-C',str(source),'fetch','--quiet',str(bundle),'HEAD'],'CPU16 bundle fetch')
        run(['git','-C',str(source),'checkout','--quiet','--detach','FETCH_HEAD'],'CPU16 checkout')
        run(['git','-C',str(source),'remote','set-url','origin','https://github.com/choiceoh/stkernel.git'],'CPU16 origin')
        frozen()
    free=available()
    if free<12*1024*1024:
    ...
    if run(['git','-C',str(scheduler),'status','--porcelain'],'scheduler cleanliness'):
        raise RuntimeError('normal scheduler is dirty; no CPU16 job started')
    ...
    command=['bash',str(scheduler/'bench/fleet.sh'),'run','--cpu','eplocalcpu0908v16head','6',
    ...
    metadata=dict(source=str(source),revision=revision,scheduler=str(scheduler),
                  ...
                  reason='CPU16 committed EP local kernel candidate, current CPU-only wrapper and source contracts; compare CPU15')
    raw=base64.b64decode(driver64,validate=True)
    compile(raw,'CPU16 driver','exec')
    job.mkdir()
...
def remote(mode, revision, bundle_sha=''):
    ...
    value = run(['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=8', 'choiceoh@srv2', shlex.join(command)],
                'CPU16 remote ' + mode, stdin=subprocess.DEVNULL)
...
def main():
    ...
    run(['git', 'merge-base', '--is-ancestor', BASE, revision], 'CPU15 base ancestry', cwd=ROOT)
    ...
    if not info['source_exists']:
        if not BUNDLE.exists():
            run(['git', 'bundle', 'create', str(BUNDLE), 'HEAD', '^' + BASE], 'CPU16 bundle create', cwd=ROOT)
        heads = run(['git', 'bundle', 'list-heads', str(BUNDLE)], 'CPU16 bundle identity', cwd=ROOT).splitlines()
        if heads != [revision + ' HEAD']:
            raise RuntimeError('CPU16 bundle revision differs; refusing path replacement')
        digest = hashlib.sha256(BUNDLE.read_bytes()).hexdigest()
        if info['bundle_sha256'] is None:
            run(['scp', '-q', str(BUNDLE), 'choiceoh@srv2:' + str(BUNDLE)], 'CPU16 bundle transfer')
        elif info['bundle_sha256'] != digest:
            raise RuntimeError('remote CPU16 bundle differs; refusing path replacement')
