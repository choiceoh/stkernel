...
REPORT = Path('/tmp/glm53-ep-local-gpu-0908-5-launch.json')
HOST = 'choiceoh@srv2'
...
from pathlib import Path
SOURCE=Path('/home/choiceoh/stkernel-ep-local-gpu-0908-5')
...
def prepare():
    ...
    if not SOURCE.exists():
        run(['git','clone','--quiet','--shared','--no-checkout',str(CPU_SOURCE),str(SOURCE)],'full GPU v5 clone')
        git(SOURCE,'fetch','--quiet',str(bundle),'HEAD')
        git(SOURCE,'checkout','--quiet','--detach','FETCH_HEAD')
        git(SOURCE,'remote','set-url','origin','https://github.com/choiceoh/stkernel.git')
    frozen(revision)
