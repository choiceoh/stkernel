...
JOB = '/tmp/glm53-ep-onepass-0909-11'
SOURCE = '/home/choiceoh/stkernel-ep-onepass-0909-11'
NODE = r'''
...
def current_arm_event(session, ticket, owner_pid):
    ...
    expected=['EPONEPASS11'+arm for arm in ('A0','B1','A','B2','B3')]
    ...
    return dict(arm=names[-1].removeprefix('EPONEPASS11'),started_at=born.timestamp(),
...
def main():
    ...
    if args.session != 'eplocalonepass0909v11': p.error('wrong onepass11 session')
    ...
    if args.revision!=REV or not re.fullmatch('[a-f0-9]{40}',REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        p.error('bind actual onepass11 revision, ticket and owner PID before capture')
    os.umask(0o077)
    local=Path('/tmp')/('glm53-onepass11-live-'+label)
