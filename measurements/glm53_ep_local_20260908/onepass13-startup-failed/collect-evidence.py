#!/usr/bin/env python3
"""Archive closed onepass13 A0 failure: read-only SSH; new archive writes only."""
import base64
import gzip
import hashlib
import json
...
OUT = ROOT/'measurements/glm53_ep_local_20260908/onepass13-startup-failed'
...
P=pathlib.Path
root=P('/home/choiceoh/stkernel-ep-onepass-0909-13')
job=P('/tmp/glm53-ep-onepass-0909-13')
session='eplocalonepass0909v13';ticket='17889155333785782'
log=P('/home/choiceoh/glm53-logs/fleet/run-logs/f65aef228e9dbbed9cba05d3dd7d73d5dda4e5455f4caa098e6f61f1806fca6e.log')
def identity():
...
paths += [P('/home/choiceoh/glm53-logs')/('boot-EPONEPASS13'+arm+'.log') for arm in ('A0','B1','A','B2','B3')]
for path in paths:
 ...
 if path.name.startswith('boot-') and path.name!='boot-EPONEPASS13A0.log':raise AssertionError('later arm unexpectedly started')
...
provided_raw=read('/tmp/glm53-onepass13-mounted-hashes.json');provided=json.loads(provided_raw)
assert all(mounts.get(k)==v for k,v in provided.items())
save('source/continuity-input-mounted-hashes.json',provided_raw,'/tmp/glm53-onepass13-mounted-hashes.json')
continuity_raw=read('/tmp/glm53-onepass13-continuity.json');continuity=json.loads(continuity_raw)
...
save('continuity.json',continuity_raw,'/tmp/glm53-onepass13-continuity.json')

stream_root=Path('/tmp/glm53-onepass13-streams')
...
failures={}
for node in NODES:
    path=Path('/tmp/glm53-onepass13-canary')/(node+'.json');raw=read(path);receipt=json.loads(raw)
...
for source,name in [('/tmp/glm53-onepass13-submit.json','submission/request.json'),('/tmp/glm53-onepass13-submit-receipt.txt','submission/receipt.json'),('/tmp/glm53_onepass13_observer.py','tools/passive-observer.py'),('/tmp/glm53_onepass13_snapshot.py','tools/snapshot-helper.py'),('/tmp/glm53_extract_onepass13_canary.py','tools/extract-canary.py'),('/tmp/glm53_onepass10_compare_canary.py','tools/compare-canary.py')]:save(name,read(source),source)
submission=json.loads(items['submission/receipt.json'])
assert (submission['session'],submission['ticket'],submission['pid'],submission['log_path'])==(SESSION,TICKET,3785782,remote['fleet_after']['log_path'])
summary=dict(schema=1,verdict='STARTUP_CANARY_FAIL',revision=REV,session=SESSION,ticket=TICKET,supervisor_pid=3785782,arm='A0',payload_returncode=1,returncode=1,own_holder_after=False,onepass_request_records=0,strict_arm_snapshots=0,later_arms_started=[],prefill_case_pass_count=24,canary_pass_count=0,canary_failure_count=4,same_old_weight_and_concentrated_input_identity=True,short6_initial_preparation_exact_variants=12,short6_changed_reached=False,failures=failures,performance_acceptance=False,adoption_acceptance=False,full_sanitizer_acceptance=False,scope='A0 startup self-test only. Normal onepass HTTP workload did not start. Source bytes are independently bound to frozen Git revision; no running-container snapshot/image attribution was completed.')
record('failure-summary.json',summary,'unchanged original receipts and closed normal fleet evidence')
readme='''# onepass13: A0 startup canary failed

Frozen source `a010504ee03842f30892a31d764b74cdbf3a5610`, normal fleet session `eplocalonepass0909v13`, ticket `17889155333785782`, supervisor `3785782`.
...
Initial T6 preparation bytes matched the reference for BF16, FP32 and FP16 weights on all four ranks. Changed T6 was not reached. Short6 numerical validation failed: rank0 at initial-C3 (2 bad rows, normalized peak 0.1264822185); ranks1/2/3 stopped on unstable stock controls (normalized peaks 0.1086956486, 0.0541871935, 0.0790513828). Original errors, controls and diagnostics remain in all four complete receipts.
