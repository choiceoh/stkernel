...
JOB = '/tmp/glm53-ep-onepass-0909-13'
...
def main():
    ...
    if args.session != 'eplocalonepass0909v13': p.error('wrong onepass13 session')
    ...
    if args.revision!=REV or not re.fullmatch('[a-f0-9]{40}',REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        p.error('bind actual onepass13 revision, ticket and owner PID before capture')
    os.umask(0o077)
    local=Path('/tmp')/('glm53-onepass13-live-'+label)
