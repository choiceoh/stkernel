...
def main():
    ...
    if len(REV)!=40 or any(c not in '0123456789abcdef' for c in REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        args.error('bind actual onepass14 revision, ticket and owner PID before observing')
    ...
    def snapshot(arm, head):
        ...
        event({'kind':'strict_snapshot_finished','arm':arm,'returncode':rc,'path':'/tmp/glm53-onepass14-live-'+arm+'-observer'})
