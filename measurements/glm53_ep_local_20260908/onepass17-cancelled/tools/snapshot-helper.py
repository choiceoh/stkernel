...
TICKET='17889188803995175'
OWNER_PID='3995175'

IMAGE = 'sha256:a3dd4c0f6cbb053097d65d10cd8ff8f6ae0cb9115cf0ff142e1cafe124c09211'
JOB = '/tmp/glm53-ep-onepass-0909-17'
SOURCE = '/home/choiceoh/stkernel-ep-onepass-0909-17'
NODE = r'''
...
def current_arm_event(session, ticket, owner_pid):
    ...
    expected=['EPONEPASS17'+arm for arm in ('A0','B1','A','B2','B3')]
    ...
    return dict(arm=names[-1].removeprefix('EPONEPASS17'),started_at=born.timestamp(),
...
def main():
    ...
    p.add_argument('--arm',choices=('A0','B1','A','B2','B3'),required=True)
    ...
    if args.session != 'eplocalonepass0909v17': p.error('wrong onepass17 session')
    ...
    config=dict(arm=args.arm,label=label,revision=args.revision,session=args.session,job=JOB,source=SOURCE,
                ...
                ep=args.arm in ('A0','A'),flags={
                    'VLLM_GLM53_EP_PREFILL_LOCAL':'1' if args.arm in ('A0','A') else '0',
                    'VLLM_B12X_EP_WARM_COMPACT':'1' if args.arm in ('A0','A') else '0',
                    'VLLM_B12X_EP_ZERO_WEIGHT_MICRO':'1' if args.arm in ('A0','A') else '0',
    ...
    if args.revision!=REV or not re.fullmatch('[a-f0-9]{40}',REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        p.error('bind actual onepass17 revision, ticket and owner PID before capture')
    os.umask(0o077)
    local=Path('/tmp')/('glm53-onepass17-live-'+label)
