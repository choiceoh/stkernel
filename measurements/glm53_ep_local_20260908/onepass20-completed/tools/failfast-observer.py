...
REMOTE = r'''
import datetime, hashlib, json, math, os, re, signal, stat, subprocess, time
from pathlib import Path
F=Path('/home/choiceoh/glm53-logs/fleet')
S=Path('/home/choiceoh/stkernel-ep-onepass-0909-20')
J=Path('/tmp/glm53-ep-onepass-0909-20')
SESSION='eplocalonepass0909v20'; PREFIX='EPONEPASS20'
...
def arm_event():
    ...
    names=[m[2].decode() for m in events];expected=[PREFIX+a for a in ('A0','B1','A','B2','B3')]
...
def run():
    ...
    while time.monotonic()<deadline:
        ...
        if event['arm']!=cfg['arm'] or event['arm'] not in ('A0','A'):
        ...
        receipt=dict(schema=1,session=SESSION,ticket=ticket,owner_pid=owner,owner_start_tick=cfg['owner_start_tick'],revision=revision,
                     ...
                     threshold_tok_s=cfg['threshold'],observed=meta,records_sha256=sha(before_records),completed_records=names,
...
def main():
    ...
    p.add_argument('--arm',choices=('A0','A'),default='A0')
