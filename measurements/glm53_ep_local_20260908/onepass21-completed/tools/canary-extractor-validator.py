...
candidate can use --occurrence 1 --output /tmp/glm53-onepass21-canary-A and
--compare-to /tmp/glm53-onepass21-canary for within-rank content continuity.
"""
...
REV='028f98167376f0a0857c20c7ec89a3505c1a000f'
CPU_SHA='b4bfcb1c13885939553a1e5c56fb65595a5a5f78c6e73001d3c3eee24396a725'
...
def expected_source(args):
 raw=plain(args.cpu_result);require(sha(raw)==CPU_SHA,'CPU21 result hash changed')
 cpu=json.loads(raw)
 require(cpu['verdict']=='PASS' and cpu['phase']=='complete' and cpu['binding_runtime_rechecked'] is True
         and cpu['cuda_initialized'] is False,'CPU21 incomplete')
 require(cpu['contracts']==dict(tests_run=151,failures=0,errors=0,skips=0),'CPU21 contract counts')
 require([x['arm'] for x in cpu['tp_sf6_passes']]==['stock','q0-cache'],'CPU21 TP compile pair missing')
 result={}
 for key,filename in FILES.items():
  ...
  require(len(found)==1 and found[0][1]==sha(data),'frozen source differs from CPU21: '+key)
...
def validate(receipt,expected):
 ...
 return dict(cases=4,control_comparisons=24,candidate_comparisons=24,q0_comparisons=24,
             weights_preserved=True,inputs_same_address=True,source_matches_cpu21=True,
...
def main():
 ...
 parser.add_argument('--streams',type=Path,default=Path('/tmp/glm53-onepass21-streams'))
 parser.add_argument('--output',type=Path,default=Path('/tmp/glm53-onepass21-canary'))
 parser.add_argument('--cpu-result',type=Path,default=ROOT/'measurements/glm53_ep_local_20260908/decode21-cpu/result.json')
 ...
 args=parser.parse_args();require(args.revision==REV and args.occurrence in (0,1),'wrong revision/occurrence')
 ...
 report=dict(schema=1,revision=args.revision,cpu21_result_sha256=CPU_SHA,occurrence=args.occurrence,
