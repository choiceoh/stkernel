...
SESSION='eplocalonepass0909v21'
TICKET='1788924164133801'
OWNER_PID='133801'
REV='028f98167376f0a0857c20c7ec89a3505c1a000f'
SOURCE='/home/choiceoh/stkernel-ep-onepass-0909-21'
ROOT=Path('/tmp/glm53-onepass21-streams')
SNAPSHOT=Path('/tmp/glm53_onepass21_snapshot.py')
...
def current_arm_event(session, ticket, owner_pid):
    ...
    expected=['EPONEPASS21'+arm for arm in ('A0','B1','A','B2','B3')]
    ...
    return dict(arm=names[-1].removeprefix('EPONEPASS21'),started_at=born.timestamp(),
...
if s['own'] and s['go'] and not s['terminal']:
    try:
        ...
        flags={k:env.get(k) for k in ('VLLM_GLM53_EP_PREFILL_LOCAL','VLLM_B12X_EP_WARM_COMPACT','VLLM_GLM53_SKIP_UNUSED_GRAPH_PROFILE','VLLM_B12X_EP_ZERO_WEIGHT_MICRO','VLLM_GLM53_TP_SF6_Q0')}
        s['head']={'id':c['Id'],'created_at':c['Created'],'started_at':c['State']['StartedAt'],'running':c['State']['Running'],'stable':stable,
                   ...
                   'tp_sf6_q0_pass':q0_pass,'tp_only':tp_only,'image_exact':image_exact,
...
def main():
    ...
    if len(REV)!=40 or any(c not in '0123456789abcdef' for c in REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        args.error('bind actual onepass21 revision, ticket and owner PID before observing')
    ...
    def snapshot(arm, head):
        ...
        event({'kind':'strict_snapshot_finished','arm':arm,'returncode':rc,'path':'/tmp/glm53-onepass21-live-'+arm+'-observer'})
    ...
    try:
        while not stopped.is_set() and time.time()<deadline:
            ...
            if seen_go and head and head['manifest_exact'] and head['stable'] and head['running'] and head['born_after_arm'] and not head['duplicate_env']:
                ...
                expected='1' if arm in ('A0','A') else '0'
                eligible=all(flags[key]=='0' for key in ('VLLM_GLM53_EP_PREFILL_LOCAL','VLLM_B12X_EP_WARM_COMPACT','VLLM_B12X_EP_ZERO_WEIGHT_MICRO'))
                ...
                eligible=eligible and(expected=='0' or head['tp_sf6_q0_pass'])
