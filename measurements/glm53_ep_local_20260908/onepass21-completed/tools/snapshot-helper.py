...
JOB = '/tmp/glm53-ep-onepass-0909-21'
...
summary={'node':cfg['node'],'id':c['Id'],'created_at':c['Created'],'started_at':c['State']['StartedAt'],'image':c['Image'],
         ...
         'readiness':{'graph_finished':graph,'candidate':cfg['candidate'],'tp_sf6_q0_pass_records':pass_records},
...
def main():
    ...
    if args.session != 'eplocalonepass0909v21': p.error('wrong onepass21 session')
    ...
    config=dict(arm=args.arm,label=label,revision=args.revision,session=args.session,job=JOB,source=SOURCE,
                ...
                ep=False,candidate=args.arm in ('A0','A'),flags={
                    'VLLM_GLM53_EP_PREFILL_LOCAL':'0',
                    'VLLM_B12X_EP_WARM_COMPACT':'0',
                    'VLLM_B12X_EP_ZERO_WEIGHT_MICRO':'0',
                    'VLLM_GLM53_TP_SF6_Q0':'1' if args.arm in ('A0','A') else '0',
    ...
    if args.revision!=REV or not re.fullmatch('[a-f0-9]{40}',REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        p.error('bind actual onepass21 revision, ticket and owner PID before capture')
    os.umask(0o077)
    local=Path('/tmp')/('glm53-onepass21-live-'+label)
