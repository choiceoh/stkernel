#!/usr/bin/env python3
"""Offline immutable TP Q0 receipt extraction; no CUDA, SSH or HTTP.

Default occurrence 0 is the first TP receipt in each observer stream. It is
NOT an arm/container proof; correlate strict snapshots separately. A later
candidate can use --occurrence 1 --output /tmp/glm53-onepass23-canary-A and
--compare-to /tmp/glm53-onepass23-canary for within-rank content continuity.
"""
...
REV='cbf1c7916247f946167c15ff74d92261588e8cea'
CPU_SHA='947cca0a91857cd326cc08d1ac5d1f4dcf21185287733750b481ca5b2ed2af25'
EXPECTED_TESTS=165
...
def expected_source(args):
 raw=plain(args.cpu_result);require(sha(raw)==CPU_SHA,'CPU23 result hash changed')
 cpu=json.loads(raw)
 require(cpu['verdict']=='PASS' and cpu['phase']=='complete' and cpu['binding_runtime_rechecked'] is True
         and cpu['cuda_initialized'] is False,'CPU23 incomplete')
 require(cpu['contracts']==dict(tests_run=EXPECTED_TESTS,failures=0,errors=0,skips=0),'CPU23 contract counts')
 require([x['arm'] for x in cpu['tp_sf6_passes']]==['stock','q0-cache'],'CPU23 TP compile pair missing')
 result={}
 for key,filename in FILES.items():
  ...
  require(len(found)==1 and found[0][1]==sha(data),'frozen source differs from CPU23: '+key)
...
def validate(receipt,expected):
 ...
 return dict(cases=4,control_comparisons=24,candidate_comparisons=24,q0_comparisons=24,
             weights_preserved=True,inputs_same_address=True,source_matches_cpu23=True,
...
def main():
 ...
 parser.add_argument('--streams',type=Path,default=Path('/tmp/glm53-onepass23-streams'))
 parser.add_argument('--output',type=Path,default=Path('/tmp/glm53-onepass23-canary'))
 parser.add_argument('--cpu-result',type=Path,default=ROOT/'measurements/glm53_ep_local_20260908/decode23-cpu/result.json')
 ...
 args=parser.parse_args();require(re.fullmatch('[0-9a-f]{40}',REV) and ishash(CPU_SHA) and type(EXPECTED_TESTS)is int and EXPECTED_TESTS>0,'bind actual CPU23 revision, result SHA and test count first');require(args.revision==REV and args.occurrence in (0,1),'wrong revision/occurrence')
 ...
 report=dict(schema=1,revision=args.revision,cpu23_result_sha256=CPU_SHA,occurrence=args.occurrence,
