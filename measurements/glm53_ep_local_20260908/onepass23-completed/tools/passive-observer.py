...
SESSION='eplocalonepass0909v23'
TICKET='1788926263262324'
OWNER_PID='262324'
REV='cbf1c7916247f946167c15ff74d92261588e8cea'
SOURCE='/home/choiceoh/stkernel-ep-onepass-0909-23'
ROOT=Path('/tmp/glm53-onepass23-streams')
SNAPSHOT=Path('/tmp/glm53_onepass23_snapshot.py')
...
def current_arm_event(session, ticket, owner_pid):
    ...
    expected=['EPONEPASS23'+arm for arm in ('A0','B1','A','B2','B3')]
    ...
    return dict(arm=names[-1].removeprefix('EPONEPASS23'),started_at=born.timestamp(),
...
def main():
    ...
    if len(REV)!=40 or any(c not in '0123456789abcdef' for c in REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        args.error('bind actual onepass23 revision, ticket and owner PID before observing')
    ...
    def snapshot(arm, head):
        ...
        event({'kind':'strict_snapshot_finished','arm':arm,'returncode':rc,'path':'/tmp/glm53-onepass23-live-'+arm+'-observer'})
