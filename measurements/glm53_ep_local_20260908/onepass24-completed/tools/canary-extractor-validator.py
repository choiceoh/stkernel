...
candidate can use --occurrence 1 --output /tmp/glm53-onepass24-canary-A and
--compare-to /tmp/glm53-onepass24-canary for within-rank content continuity.
"""
...
REV='82ac3c34173ae63b3dd0a42c49f8421097e96a1a'
...
def main():
 ...
 parser.add_argument('--streams',type=Path,default=Path('/tmp/glm53-onepass24-streams'))
 parser.add_argument('--output',type=Path,default=Path('/tmp/glm53-onepass24-canary'))
 ...
 args=parser.parse_args();require(re.fullmatch('[0-9a-f]{40}',REV) and ishash(CPU_SHA) and type(EXPECTED_TESTS)is int and EXPECTED_TESTS>0,'bind actual CPU24 revision, result SHA and test count first');require(args.revision==REV and args.occurrence in (0,1),'wrong revision/occurrence')
 ...
 report=dict(schema=1,revision=args.revision,cpu24_result_sha256=CPU_SHA,occurrence=args.occurrence,
