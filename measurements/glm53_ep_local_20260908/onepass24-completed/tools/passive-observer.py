...
SESSION='eplocalonepass0909v24'
TICKET='1788928530449824'
OWNER_PID='449824'
REV='82ac3c34173ae63b3dd0a42c49f8421097e96a1a'
SOURCE='/home/choiceoh/stkernel-ep-onepass-0909-24'
ROOT=Path('/tmp/glm53-onepass24-streams')
SNAPSHOT=Path('/tmp/glm53_onepass24_snapshot.py')
...
def current_arm_event(session, ticket, owner_pid):
    ...
    expected=['EPONEPASS24'+arm for arm in ('A0','B1','A','B2','B3')]
    ...
    return dict(arm=names[-1].removeprefix('EPONEPASS24'),started_at=born.timestamp(),
...
def main():
    ...
    if len(REV)!=40 or any(c not in '0123456789abcdef' for c in REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        args.error('bind actual onepass24 revision, ticket and owner PID before observing')
    ...
    def snapshot(arm, head):
        ...
        event({'kind':'strict_snapshot_finished','arm':arm,'returncode':rc,'path':'/tmp/glm53-onepass24-live-'+arm+'-observer'})
