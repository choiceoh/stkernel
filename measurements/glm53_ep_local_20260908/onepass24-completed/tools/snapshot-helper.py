...
JOB = '/tmp/glm53-ep-onepass-0909-24'
...
def main():
    ...
    if args.session != 'eplocalonepass0909v24': p.error('wrong onepass24 session')
    ...
    config=dict(arm=args.arm,label=label,revision=args.revision,session=args.session,job=JOB,source=SOURCE,
                ...
                ep=False,candidate=args.arm in ('A0','A'),mm_limit={'image':4,'video':0},flags={
    ...
    if args.revision!=REV or not re.fullmatch('[a-f0-9]{40}',REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0:
        p.error('bind actual onepass24 revision, ticket and owner PID before capture')
    os.umask(0o077)
    local=Path('/tmp')/('glm53-onepass24-live-'+label)
