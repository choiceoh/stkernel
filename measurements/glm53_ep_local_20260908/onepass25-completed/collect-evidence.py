#!/usr/bin/env python3
"""Prepare terminal GPU25 evidence. Running this later performs read-only captures
and writes one fresh archive; it never submits work or signals any process.
Only B0/B1/A/B2/B3 are valid, and only A enables TP Q0. CPU24 is reused verbatim.
"""
...
OUT=ROOT/'measurements/glm53_ep_local_20260908/onepass25-completed'
REV='055914aeb719c1769e05cdb863e43a88b2ee47af'; SESSION='eplocalonepass0909v25'; TICKET='1788930114553638'; PID=553638; START='43250203'
...
VALIDATOR_SHA='c697ea54b9828b37414c7c981c5b1bf819d0ac1af4efe59a0e04ba36060f705e'
IMAGE='sha256:a3dd4c0f6cbb053097d65d10cd8ff8f6ae0cb9115cf0ff142e1cafe124c09211'
ARMS=('B0','B1','A','B2','B3'); NODES=('local','10.10.10.1','10.10.10.3','10.10.10.4')
...
P=pathlib.Path; root=P('/home/choiceoh/stkernel-ep-onepass-0909-25'); job=P('/tmp/glm53-ep-onepass-0909-25'); F=P('/home/choiceoh/glm53-logs/fleet')
...
capture(log)
for arm in cfg['arms']: capture(P('/home/choiceoh/glm53-logs')/('boot-EPONEPASS25'+arm+'.log'))
...
def main():
 parser=argparse.ArgumentParser(description=__doc__); parser.add_argument('--leg',action='append',default=['/tmp/leg.589372','/tmp/leg.616221'],help='Known exact /tmp/leg.<pid>; missing originals stay missing')
 ...
 stream_root=Path('/tmp/glm53-onepass25-streams'); eraw=read(stream_root/'events.jsonl'); events=jsonlines(eraw)
 ...
 reuse_path=Path('/tmp/glm53-onepass25-cpu24-reuse.json'); need(read(reuse_path)==items['job/cpu24-reuse.json'],'local/remote CPU24 reuse receipt differs')
 terminal_log=gzip.decompress(items['fleet/terminal-run.log.gz'])
 begun=[v.decode().removeprefix('EPONEPASS25') for v in re.findall(rb'(?m)^== [0-9]{2}:[0-9]{2}:[0-9]{2} arm (EPONEPASS25[^ :\r\n]+):',terminal_log)]
 need(begun==list(ARMS[:len(begun)]),'canonical arm order differs')
 rows=jsonlines(items.get('job/onepass.jsonl',b'')); names=[r['name'].removeprefix('EPONEPASS25') for r in rows]
 need(names==list(ARMS[:len(names)]) and all(r['name']=='EPONEPASS25'+a for a,r in zip(names,rows)) and begun[:len(names)]==names,'completed records are not a canonical prefix')
 ...
 helper=Path('/tmp/glm53_extract_onepass25_canary.py'); helper_raw=read(helper); need(sha(helper_raw)==VALIDATOR_SHA,'reviewed pure canary validator changed')
 spec=importlib.util.spec_from_file_location('onepass25_archive_validator',helper); validator=importlib.util.module_from_spec(spec); spec.loader.exec_module(validator)
 ...
 for arm in ARMS:
  private=Path('/tmp/glm53-onepass25-live-'+arm+'-observer')
  ...
  need(identity['source']=='/home/choiceoh/stkernel-ep-onepass-0909-25' and set(identity['nodes'])==set(NODES),'snapshot source/nodes differ')
 ...
 canary_root=Path('/tmp/glm53-onepass25-canary-A'); canary_summary=dict(verdict='MISSING',candidate_started='A' in begun)
 ...
 for source,name in [('/tmp/glm53-onepass25-submit.json','submission/request.json'),('/tmp/glm53-onepass25-submit-receipt.txt','submission/receipt.json'),('/tmp/glm53_onepass25_observer.py','tools/passive-observer.py'),('/tmp/glm53_onepass25_snapshot.py','tools/snapshot-helper.py'),('/tmp/glm53_onepass25_progress.py','tools/progress-helper.py'),('/tmp/glm53_onepass25_failfast.py','tools/failfast-helper.py'),(str(helper),'tools/canary-extractor-validator.py')]: save(name,read(source),source)
 ...
 record('failure/failfast-parent-observation.json',dict(schema=1,session=SESSION,ticket=TICKET,revision=REV,owner_pid=PID,owner_start_tick=START,arm='A',exec_session=60633,action='REPORT_FIRST_RESULT_NOT_BELOW_THRESHOLD',signal_sent=False,rounded_first_fixed_decode_tok_s=69.22,threshold_tok_s=65,leg_path='/tmp/leg.616221',exit_code=0,raw_stdout_archived=False,scope='Parent reported the tool write_stdin session60633 stdout. This is a derived observation, not an original saved stdout file, and does not exclude any later cancellation.'),'parent message during A; no raw stdout file was created by report-only helper')
