#!/usr/bin/env python3
"""Offline TP Q0 receipt extraction for the sole candidate A of GPU27.

B has Q0 disabled and produces no TP candidate receipt. Occurrence0
...
def expected_source(args):
 ...
 require(reuse['schema']==1 and reuse['mode']=='ORIGINAL_CPU24_EXACT_SOURCE_REUSE' and reuse['new_revision']==REV
         and reuse['new_source']=='/home/choiceoh/stkernel-ep-onepass-0909-27'
...
def main():
 ...
 parser.add_argument('--streams',type=Path,default=Path('/tmp/glm53-onepass27-streams'))
 parser.add_argument('--output',type=Path,default=Path('/tmp/glm53-onepass27-canary-A'))
 parser.add_argument('--cpu-result',type=Path,default=ROOT/'measurements/glm53_ep_local_20260908/decode24-cpu/result.json')
 parser.add_argument('--cpu-reuse-receipt',type=Path,default=Path('/tmp/glm53-onepass27-cpu24-reuse.json'))
