#!/usr/bin/env python3
"""Prepared only; ownership supplied by required CLI arguments. Poll the owned candidate's first fixed1024 stdout; opt-in SIGTERM.
...
S=Path('/home/choiceoh/stkernel-ep-onepass-0909-27')
J=Path('/tmp/glm53-ep-onepass-0909-27')
SESSION='eplocalonepass0909v27'; PREFIX='EPONEPASS27'
...
def arm_event():
    ...
    names=[m[2].decode() for m in events];expected=[PREFIX+a for a in ('B','A')]
...
def records():
    ...
    need(names==[PREFIX+'B',PREFIX+'A'][:len(names)],'only this run B/A record prefix is allowed')
    return raw,names
...
def main():
    ...
    if not (args.revision==REV and len(args.revision)==40 and all(c in '0123456789abcdef' for c in args.revision) and args.ticket.isdecimal() and args.owner_start_tick.isdecimal() and args.owner_pid>1):p.error('exact immutable revision and actual reservation identity required')
