...
session='eplocalonepass0909v27';owner=int(owner_pid);ticket=ticket
fleet=Path('/home/choiceoh/glm53-logs/fleet');job=Path('/tmp/glm53-ep-onepass-0909-27')
pending=json.loads((fleet/'pending'/(hashlib.sha256(session.encode()).hexdigest()+'.json')).read_text())
assert pending['session']==session and str(pending['ticket'])==ticket and pending['pid']==owner and str(pending['start'])==owner_start_tick
...
if record.exists():
 for line in record.read_text().splitlines():
  r=json.loads(line)
  assert r.get('session')==session and r.get('git')==revision[:8]
  assert len(result['records'])<2 and r.get('name')==['EPONEPASS27B','EPONEPASS27A'][len(result['records'])], 'records must be this run B/A prefix'
...
"""
if not re.fullmatch('[0-9a-f]{40}',REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=1 or not OWNER_START_TICK.isdecimal():raise SystemExit('bind actual onepass27 revision, ticket, owner PID and start tick first')
REMOTE='revision='+repr(REV)+'\nsource='+repr('/home/choiceoh/stkernel-ep-onepass-0909-27')+'\nowner_pid='+repr(OWNER_PID)+'\nticket='+repr(TICKET)+'\nowner_start_tick='+repr(OWNER_START_TICK)+'\n'+REMOTE
