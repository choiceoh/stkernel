...
JOB = '/tmp/glm53-ep-onepass-0909-27'
...
if cfg['candidate']:
    ...
    need(not pass_records and b'[tp-sf6-q0-selftest] FAIL ' not in log,'baseline unexpectedly emitted TP Q0 canary')
...
def write(path,data):
    ...
    path.chmod(0o600)
def cpu24_source_binding():
    """Read original CPU24 proof and this job's reuse record; no new compile."""
    job=Path(cfg['job']); cpu_job=Path('/tmp/glm53-ep-decode-cpu-0909-24')
    expected_origin=dict(job=str(cpu_job),source='/home/choiceoh/stkernel-ep-onepass-0909-24',
        revision='82ac3c34173ae63b3dd0a42c49f8421097e96a1a',
        result_sha256='1d96612938f200bd86d078e1d8841cc92df808ba4ab802ca51306fe229b67506',
        submission_sha256='b99461061122fb83a7beb4106fa1733e5fbf21a06bae45b92ff83b625474ed6c',
        exit_sha256='ca36e7af240ffd23f4117192b13e4a6310ca53ffd5c3e9a4d695fde531c5ee56',
        archive_result='measurements/glm53_ep_local_20260908/decode24-cpu/result.json')
    def stable(path):
        a=path.lstat(); need(stat.S_ISREG(a.st_mode) and a.st_size<4*2**20,'unsafe CPU proof file')
        raw=path.read_bytes(); b=path.lstat()
        need((a.st_dev,a.st_ino,a.st_size,a.st_mtime_ns)==(b.st_dev,b.st_ino,b.st_size,b.st_mtime_ns),'CPU proof file changed')
        return raw
    raw=stable(job/'cpu24-reuse.json'); reuse=json.loads(raw)
    need(reuse.get('schema')==1 and reuse.get('mode')=='ORIGINAL_CPU24_EXACT_SOURCE_REUSE'
         and reuse.get('cpu_origin')==expected_origin and reuse.get('new_source')==cfg['source']
         and reuse.get('new_revision')==cfg['revision'] and reuse.get('new_cpu_compile') is False
         and reuse.get('mounted_and_contract_sources_equal') is True
         and reuse.get('artifact_and_runtime_validation') is True,'CPU24 reuse admission differs')
    receipts={}
    for name,key in [('submission.json','submission_sha256'),('exit.json','exit_sha256'),('evidence/result.json','result_sha256')]:
        data=stable(cpu_job/name); need(sha(data)==expected_origin[key],'original CPU24 receipt differs'); receipts[name]=data
    result=json.loads(receipts['evidence/result.json'])
    need(result.get('verdict')=='PASS' and result.get('phase')=='complete'
         and result.get('cuda_initialized') is False and result.get('binding_runtime_rechecked') is True
         and result.get('contracts')==dict(tests_run=165,failures=0,errors=0,skips=0),'original CPU24 incomplete')
    need(json.loads(receipts['exit.json'])['returncode']==0,'original CPU24 outer failed')
    source=Path(cfg['source']); found={}
    manifest=stable(source/'build/glm53/manifest.tsv')
    for line in manifest.decode().splitlines():
        if not line or line.startswith('#'): continue
        name,target,group=line.split('\t')
        if target in result['mounted_sources']:
            need('/' not in name and name not in ('.','..') and target not in found,'unsafe CPU mount entry')
            data=stable(source/'build/glm53'/name)
            need(data==run(['git','-C',str(source),'show',cfg['revision']+':build/glm53/'+name]),'mounted file differs from frozen git')
            found[target]=sha(data)
    need(found==result['mounted_sources'],'CPU24 mounted source mismatch')
    for relative,digest in result['contract_sources'].items():
        path=Path(relative); need(not path.is_absolute() and '..' not in path.parts,'unsafe contract source')
        data=stable(source/path)
        need(sha(data)==digest and data==run(['git','-C',str(source),'show',cfg['revision']+':'+relative]),'CPU24 contract source mismatch')
    return dict(mode='ORIGINAL_CPU24_EXACT_SOURCE_REUSE',reuse_path=str(job/'cpu24-reuse.json'),
        reuse_sha256=sha(raw),cpu_origin=expected_origin,new_source=cfg['source'],new_revision=cfg['revision'],
        new_cpu_compile=False,mounted_and_contract_sources_equal=True,
        scope='Read-only original CPU24 bytes and current frozen mounted/contract source equality; no new compile or GPU acceptance')

before_holder=holder(); source_check(); cpu24_before=cpu24_source_binding()
...
need(cpu24_source_binding()==cpu24_before,'CPU24 binding changed during capture')
...
identity={'schema':1,'arm':cfg['arm'],'label':cfg['label'],'revision':cfg['revision'],'source':cfg['source'],
          'session':cfg['session'],'ticket':cfg['ticket'],'owner_pid':cfg['owner_pid'],'owner_start_tick':cfg['owner_start_tick'],'cpu24_binding':cpu24_before,
...
def main():
    ...
    p.add_argument('--arm',choices=('B','A'),required=True)
    ...
    if args.session != 'eplocalonepass0909v27': p.error('wrong onepass27 session')
    ...
    if args.revision!=REV or not re.fullmatch('[a-f0-9]{40}',REV) or not TICKET.isdecimal() or not OWNER_PID.isdecimal() or int(OWNER_PID)<=0 or not OWNER_START_TICK.isdecimal():
        p.error('bind actual onepass27 revision, ticket, owner PID and start tick before capture')
    os.umask(0o077)
    local=Path('/tmp')/('glm53-onepass27-live-'+label)
