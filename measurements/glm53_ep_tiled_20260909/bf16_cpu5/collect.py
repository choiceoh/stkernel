#!/usr/bin/env python3
"""Read-only collection of completed EP native BF16 scatter CPU originals, not a test run."""
import datetime
import gzip
import hashlib
import io
...
OUT = REPO/'measurements/glm53_ep_tiled_20260909/bf16_cpu5'
REV = 'e88f5fd368ef8c895000496101fc7fcf8e7fb344'
RESULT = '8c39170913365b171f39f257ab10f4bbe0192900767c3de373d6f9f31bddbff9'
EVIDENCE = '/home/choiceoh/glm53-ep-tiled-bf16-5-cpu-evidence'
LIMIT = 32*1024*1024

READ = r'''
import hashlib,io,json,pathlib,subprocess,sys,tarfile,time
root=pathlib.Path('/home/choiceoh/glm53-ep-tiled-bf16-5-cpu-evidence')
def inventory():
 result={}
...
for result in (r,c):
    ...
    assert result['contracts']==dict(tests_run=109,failures=0,errors=0,skips=0)
    assert sum(result['selected_test_counts'].values())==109 and len(result['selected_test_counts'])==11
...
log=Path('/tmp/glm53-ep-bf16-cpu5.log').read_bytes()
assert b'Ran 109 tests' in log and b'\nOK\n' in log
raw['fleet.log']=log
execution_raw=Path('/tmp/glm53-ep-bf16-cpu5-execution.json').read_bytes()
execution=json.loads(execution_raw)
source='/home/choiceoh/stkernel-ep-tiled-0909-ring'
assert execution['revision']==REV and execution['source']==source
assert execution['session']=='eptiledcpu0910bf165' and execution['output']==EVIDENCE
...
command=['env','REPO='+source,'bash',source+'/bench/fleet.sh','run','--cpu','eptiledcpu0910bf165','5',
 'EP SF6 native BF16 scatter: six real lowerings and source-bound CPU contracts; no GPU devices','--',
...
raw['source-verification.json']=(json.dumps(dict(source=REV,verified=source_checks,
    scope='all mounted and contract source hashes equal the exact immutable git CPU source e88f5fd3; no source rebind required'),indent=2)+'\n').encode()
...
readme=f'''# EP tiled native BF16 scatter CPU evidence

Normal CPU fleet session: `eptiledcpu0910bf165`, worker `10.10.10.4` (srv4).
Original output: `{EVIDENCE}`. Collection performed only read-only archive operations, no tests or GPU work.
...
Exact compiled source: `{REV}`. All **22 mounted sources and 23 contracts** match immutable committed bytes. No source rebind was needed. Original result SHA: `{RESULT}`.

All **20 original worker files**, before/after inventories, exact execution metadata and CPU stdout are preserved. `execution.json` was written by the process executing the normal fleet command, with rc0 and start/end times; elapsed time was {execution["finished"]-execution["started"]:.3f} seconds. Its stdout hash is bound by `verification.json`. Logs/PTX/cubins use deterministic gzip; original and stored hashes/sizes are in `manifest.json`. Raw JSON remains unchanged. Resource numbers describe compiled artifacts, not runtime timing or occupancy.
