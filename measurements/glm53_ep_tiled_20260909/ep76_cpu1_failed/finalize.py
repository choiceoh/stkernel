...
ROOT=Path('/tmp/glm53-ep76-cpu1-failed-archive')
REPO=Path('/Users/ost/.worktrees/stkernel2/glm53-prefill-moe-pipeline-20260908')
REV='680a899d7de0c823905b89eed1e27cfc375feaf8'
...
for result in (r,c):
 ...
 assert result['contracts']==dict(tests_run=181,failures=0,errors=4,skips=0)
 ...
 assert 'binding_runtime_rechecked' not in result
...
assert ex['revision']==REV and ex['returncode']==1 and b'Ran 181 tests' in log and b'FAILED (errors=4)' in log
assert log.count(b"NameError: name '_report_decode_opt' is not defined")==4
...
summary=dict(verdict='FAILED_CPU_ORIGINALS_VERIFIED',source=REV,result_sha256=sha(raw['result.json']),contracts_sha256=sha(raw['contracts.json']),
 ...
 cuda_initialized=False,runtime_post_recheck_recorded=False,elapsed_seconds=ex['finished']-ex['started'],resources=resources,
 first_error="NameError: name '_report_decode_opt' is not defined in the four existing checkpoint AST fixture tests",artifact_validator='frozen source pure validator replayed over original files; no tests/compiler/GPU',
 source_binding='original receipt compared against immutable git 680a899d; current worker HEAD is captured separately and is not used as original admission',gpu_numerics_acceptance=False,performance_acceptance=False)
...
(ROOT/'README.md').write_text(f'''# EP76 CPU1 failed prerequisite

Source `{REV}`, normal fleet CPU session `epdecode76cpu0910v1`, worker srv4. Terminal rc1 after {summary['elapsed_seconds']:.3f}s. This is a **failed CPU gate**: 181 tests, zero failures, four errors, zero skips. All four errors are missing `_report_decode_opt` in the existing checkpoint test AST namespace. The canonical failure and raw traceback are preserved; later fixes do not change this verdict.
...
All71 original worker files are preserved unchanged after decompression. Original and stored hashes are listed in `manifest.json`; inventories before/after copy match. The original22 mounts and43 contracts were compared against immutable source `{REV}`. Actual imported BF16 helper identity and initial capsule receipt validate. CUDA is recorded uninitialized, but **post-runtime recheck is absent** because the gate failed; no successful final recheck is claimed. Current worker HEAD in capture files may be a later CPU2 source and does not rebind CPU1.
...
(ROOT/'collect.py').write_bytes(Path('/tmp/glm53_archive_ep76_cpu1_failed.py').read_bytes())
(ROOT/'collector-reference.py').write_bytes(Path('/tmp/glm53_archive_prep_cpu10.py').read_bytes())
(ROOT/'verify.py').write_bytes(Path(__file__).read_bytes())
entries={}
