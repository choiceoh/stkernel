#!/usr/bin/env python3
"""Private, read-only collection of terminal CPU2 pass, never a test invocation."""
...
old.READ=old.READ.replace("roots={n:pathlib.Path('/home/choiceoh/glm53-ep-prep-%d-cpu-evidence'%n) for n in (10,)}", "roots={2:pathlib.Path('/home/choiceoh/glm53-ep76-2-cpu-evidence')}").replace("prefix='cpu10/'", "prefix=''")
OUT=Path('/tmp/glm53-ep76-cpu2-archive')
def sha(b):return hashlib.sha256(b).hexdigest()
assert not OUT.exists()
ex=Path('/tmp/glm53-ep76-cpu2-execution.json').read_bytes();e=json.loads(ex)
assert e['revision']=='b41d0da24059379d41c079626cc67c3e83cd14e3' and e['returncode']==0 and e['finished']>=e['started']
...
(OUT/'fleet.log.gz').write_bytes(gzip.compress(Path('/tmp/glm53-ep76-cpu2.log').read_bytes(),mtime=0))
