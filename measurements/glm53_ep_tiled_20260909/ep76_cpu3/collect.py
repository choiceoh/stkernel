#!/usr/bin/env python3
"""Private, read-only collection of terminal CPU3 pass, never a test invocation."""
...
old.READ=old.READ.replace("roots={n:pathlib.Path('/home/choiceoh/glm53-ep-prep-%d-cpu-evidence'%n) for n in (10,)}", "roots={3:pathlib.Path('/home/choiceoh/glm53-ep76-3-cpu-evidence')}").replace("prefix='cpu10/'", "prefix=''")
OUT=Path('/tmp/glm53-ep76-cpu3-archive')
def sha(b):return hashlib.sha256(b).hexdigest()
assert not OUT.exists()
ex=Path('/tmp/glm53-ep76-cpu3-execution.json').read_bytes();e=json.loads(ex)
assert e['revision']=='29daef8b95f3dd098ba24b2fae61b322d70ecb38' and e['returncode']==0 and e['finished']>=e['started']
...
(OUT/'fleet.log.gz').write_bytes(gzip.compress(Path('/tmp/glm53-ep76-cpu3.log').read_bytes(),mtime=0))
