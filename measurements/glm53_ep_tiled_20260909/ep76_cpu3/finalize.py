...
ROOT=Path('/tmp/glm53-ep76-cpu3-archive')
REPO=Path('/Users/ost/.worktrees/stkernel2/glm53-prefill-moe-pipeline-20260908')
REV='29daef8b95f3dd098ba24b2fae61b322d70ecb38'
...
load('probes/glm53_ep_tiled_compile.py',('static_specialization','global_static_specialization','opt_static_specialization','opt_shared_capacity','opt_register_layout'),('STATIC_ROWS','DYNAMIC_ROWS','GLOBAL_STATIC_CASES','OPT_STATIC_CASES','CPU_TESTS','CPU_TEST_COUNTS','EXPECTED_CPU_TESTS','CONTRACT_PATHS'))
...
summary=dict(verdict='PASSED_CPU_ORIGINALS_VERIFIED',source=REV,result_sha256=sha(raw['result.json']),contracts_sha256=sha(raw['contracts.json']),
 ...
 source_binding='original receipt compared against immutable git 29daef8b; worker before/after HEAD and clean state independently retained',gpu_numerics_acceptance=False,performance_acceptance=False)
...
(ROOT/'README.md').write_text(f'''# EP76 CPU3 PASS originals

Source `{REV}`, normal fleet CPU session `epdecode76cpu0910v3`, worker srv4. Terminal rc0 after {summary['elapsed_seconds']:.3f}s. The original CPU gate reports **181 tests, zero failures/errors/skips**, isolated CPU contracts and **23 actual CuTe lowerings**: baseline seven local, ten global, two dynamic and four optimized static.
...
(ROOT/'collect.py').write_bytes(Path('/tmp/glm53_archive_ep76_cpu3.py').read_bytes())
