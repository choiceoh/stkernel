#!/usr/bin/env python3
import ast,gzip,hashlib,json,re,subprocess,types
from pathlib import Path
ROOT=Path('/tmp/glm53-ep76-cpu4-archive')
REPO=Path('/Users/ost/.worktrees/stkernel2/glm53-prefill-moe-pipeline-20260908')
REV='3fab1ce81a79936b3b76fa4d87b447d9f55bacba'
...
load('probes/glm53_ep_tiled_compile.py',('static_specialization','global_static_specialization','opt_static_specialization','opt_shared_capacity','opt_register_layout','validate_register_layout'),('STATIC_ROWS','DYNAMIC_ROWS','GLOBAL_STATIC_CASES','OPT_STATIC_CASES','CPU_TESTS','CPU_TEST_COUNTS','EXPECTED_CPU_TESTS','CONTRACT_PATHS'))
...
assert len(r['mounted_sources'])==22 and len(r['contract_sources'])==44
...
for group in ('static','global_static','opt_static','dynamic'):
 for p in r[group+'_passes']:
  ...
  resources.append(dict(arm=p['arm'],**met,**({'capacity':p['shared_capacity'],'register_layout':p['register_layout']} if group=='opt_static' else {})))
...
for x in resources:
 if x['arm'].startswith('opt-static/'):
  assert {k:x[k] for k in ('REG','STACK','LOCAL','SHARED')}==dict(REG=96,STACK=0,LOCAL=0,SHARED=1024)
  assert x['capacity']==dict(dynamic_bytes=98304,static_bytes=1024,total_bytes=99328,block_limit_bytes=101376)
  witness=x['register_layout']
  assert witness==ns['opt_register_layout'](types.SimpleNamespace(ep_sf1_register_layout_proven=True,ep_sf1_register_layout_receipt=witness))
  assert witness['copy_shape']=='((1, (1, 4)), 4, 4)' and witness['words_per_thread']==[16]
...
summary=dict(verdict='PASSED_CPU_ORIGINALS_VERIFIED',source=REV,result_sha256=sha(raw['result.json']),contracts_sha256=sha(raw['contracts.json']),
 original_files=71,original_bytes=sum(map(len,raw.values())),lowerings=23,contracts=r['contracts'],mounted_sources=22,contract_sources=44,
 ...
 source_binding='original receipt compared against immutable git 3fab1ce8; worker before/after HEAD and clean state independently retained',gpu_numerics_acceptance=False,performance_acceptance=False)
...
(ROOT/'README.md').write_text(f'''# EP76 CPU4 PASS originals

Source `{REV}`, normal fleet CPU session `epdecode76cpu0910v4`, worker srv4. Terminal rc0 after {summary['elapsed_seconds']:.3f}s. The original CPU gate reports **181 tests, zero failures/errors/skips**, isolated CPU contracts and **23 actual CuTe lowerings**: baseline seven local, ten global, two dynamic and four optimized static.
...
(ROOT/'collect.py').write_bytes(Path('/tmp/glm53_archive_ep76_cpu4.py').read_bytes())
