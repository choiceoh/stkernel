#!/usr/bin/env python3
"""Private, read-only collection of terminal CPU5 failure, never a test invocation."""
...
old.READ=old.READ.replace("roots={n:pathlib.Path('/home/choiceoh/glm53-ep-prep-%d-cpu-evidence'%n) for n in (10,)}", "roots={5:pathlib.Path('/home/choiceoh/glm53-ep76-5-cpu-evidence')}").replace("prefix='cpu10/'", "prefix=''")
...
OUT=Path('/tmp/glm53-ep76-cpu5-failed-archive')
def sha(b):return hashlib.sha256(b).hexdigest()
assert not OUT.exists()
ex=Path('/tmp/glm53-ep76-cpu5-execution.json').read_bytes();e=json.loads(ex)
assert e['revision']=='48df51174e0f76a726ca87cc7652dc57f2ad8724' and e['returncode']==1 and e['finished']>=e['started']
...
(OUT/'fleet.log.gz').write_bytes(gzip.compress(Path('/tmp/glm53-ep76-cpu5.log').read_bytes(),mtime=0))
