#!/usr/bin/env python3
"""Verify and freeze CPU5 failure originals with actual Q1 lowering evidence."""
import ast,gzip,hashlib,json,re,subprocess
from pathlib import Path
ROOT=Path('/tmp/glm53-ep76-cpu5-failed-archive')
REPO=Path('/Users/ost/.worktrees/stkernel2/glm53-prefill-moe-pipeline-20260908')
REV='48df51174e0f76a726ca87cc7652dc57f2ad8724'
...
for result in (r,c):
 ...
 assert result['contracts']==dict(tests_run=183,failures=2,errors=0,skips=0)
...
assert ex['revision']==REV and ex['returncode']==1 and b'Ran 183 tests' in log and b'FAILED (failures=2)' in log
assert IMAGE in ex['command'][-1] and CAPSULE in ex['command'][-1]
assert b'assert len(branches)==5' in log
...
summary=dict(verdict='FAILED_CPU_ORIGINALS_VERIFIED',source=REV,result_sha256=sha(raw['result.json']),contracts_sha256=sha(raw['contracts.json']),
 ...
 source_binding='original receipt compared against immutable git 48df5117; worker before/after HEAD and clean state independently retained',
 image=dict(id=IMAGE,evidence='original submission command; no Docker re-inspection during collection'),
 failure='two old baseline AST extractors require five retired FC1 opt branches; full gate remains FAIL',
...
(ROOT/'README.md').write_text(f'''# EP76 CPU5 failure originals

Source `{REV}`, normal fleet CPU session `epdecode76cpu0910v5`, worker srv4. Terminal rc1 after {summary['elapsed_seconds']:.3f}s. **183 tests: two failures, zero errors/skips. Full CPU gate FAIL.** Both failures reach the retired `len(branches)==5` baseline AST extractor. Preserve the original failed result and contract receipts; successful lowerings do not change that verdict.
...
''')
(ROOT/'collect.py').write_bytes(Path('/tmp/glm53_archive_ep76_cpu5.py').read_bytes())
