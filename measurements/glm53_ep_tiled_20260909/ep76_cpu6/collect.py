#!/usr/bin/env python3
"""Private, read-only collection of terminal CPU6 pass, never a test invocation."""
...
old.READ=old.READ.replace("roots={n:pathlib.Path('/home/choiceoh/glm53-ep-prep-%d-cpu-evidence'%n) for n in (10,)}", "roots={6:pathlib.Path('/home/choiceoh/glm53-ep76-6-cpu-evidence')}").replace("prefix='cpu10/'", "prefix=''")
...
OUT=Path('/tmp/glm53-ep76-cpu6-archive')
def sha(b):return hashlib.sha256(b).hexdigest()
assert not OUT.exists()
ex=Path('/tmp/glm53-ep76-cpu6-execution.json').read_bytes();e=json.loads(ex)
assert e['revision']=='4618859c90131b33c5d9ebd85a67f1537497a357' and e['returncode']==0 and e['finished']>=e['started']
...
(OUT/'fleet.log.gz').write_bytes(gzip.compress(Path('/tmp/glm53-ep76-cpu6.log').read_bytes(),mtime=0))
