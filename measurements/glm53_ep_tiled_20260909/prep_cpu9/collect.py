#!/usr/bin/env python3
"""Read-only CPU9 original evidence collection; writes only private /tmp staging."""
import ast
import gzip
import hashlib
import io
...
OUT=Path('/tmp/glm53-ep-prep-cpu9-archive')
REV='96a8cab45a1eb049298315a77ed881ee74f04f9b'
REVISIONS={7:'66809847c78ac5338965c11a1164dde5849e8515',8:'4e93663900b6ced9b22c6b8abc0a877260d50e47',9:REV}
...
roots={n:pathlib.Path('/home/choiceoh/glm53-ep-prep-%d-cpu-evidence'%n) for n in (7,8,9)}
def inventory():
 files={};raw={};presence={}
 for n,root in roots.items():
  ...
  selected=names if n==9 else {name:p for name,p in names.items() if name in ('result.json','contracts.json')}
  for name,p in selected.items():
   prefix='cpu9/' if n==9 else 'failed-prerequisites%d/'%n
...
def main():
 ...
 execution_raw=Path('/tmp/glm53-ep-prep-cpu9-execution.json').read_bytes()
 execution=json.loads(execution_raw)
 assert execution.get('returncode')==0 and execution.get('finished',0)>=execution['started'],'CPU9 not terminal PASS'
 ...
 worker={k.removeprefix('cpu9/'):v for k,v in originals.items() if k.startswith('cpu9/')}
 ...
 for receipt in (r,c):
  ...
  assert receipt['contracts']==dict(tests_run=154,failures=0,errors=0,skips=0)
 ...
 assert r['selected_test_counts']==VALIDATOR['CPU_TEST_COUNTS'] and VALIDATOR['EXPECTED_CPU_TESTS']==154
 ...
 assert len(r['mounted_sources'])==22 and len(r['contract_sources'])==38
 ...
 failures=[]
 for n in (7,8,9):
  prefix='' if n==9 else 'failed-prerequisites'+str(n)+'/'
  ex=Path('/tmp/glm53-ep-prep-cpu%d-execution.json'%n).read_bytes();meta=json.loads(ex)
  log=Path('/tmp/glm53-ep-prep-cpu%d.log'%n).read_bytes()
  assert meta['revision']==REVISIONS[n] and meta['source']==SOURCE and meta['output']=='/home/choiceoh/glm53-ep-prep-%d-cpu-evidence'%n
  assert meta['session']=='eptiledcpu0910prep%d'%n and meta['finished']>=meta['started']
  assert meta['returncode']==(0 if n==9 else 1)
  ...
  raw[prefix+'execution.json']=ex;raw[prefix+'fleet.log']=log
  if n==9:
   assert b'Ran 154 tests' in log and b'\nOK\n' in log
   continue
  for filename in ('result.json','contracts.json'):
   key=prefix+filename
   if key in originals:raw[key]=originals[key]
  failure=json.loads(raw[prefix+'result.json']);assert failure['verdict']=='FAIL'
  for target,digest in failure.get('mounted_sources',{}).items():
   assert sha(git_bytes('build/glm53/'+targets[target],REVISIONS[n]))==digest
  for path,digest in failure.get('contract_sources',{}).items():assert sha(git_bytes(path,REVISIONS[n]))==digest
  if n==7:assert failure['phase']=='static-M4' and 'UNSUP_EARLY_EXIT' in failure['error']
  if n==8:
   assert failure['phase']=='cpu-contracts'
   assert sum(len(failure[k]) for k in ('static_passes','global_static_passes','dynamic_passes'))==19
   failedcontracts=json.loads(raw[prefix+'contracts.json']);assert failedcontracts['verdict']=='FAIL'
  failures.append(dict(cpu=n,source=REVISIONS[n],verdict='FAIL',phase=failure['phase'],error=failure.get('error'),
    presence=before['presence'][str(n)],contracts_acceptance=False,gpu_acceptance=False))
 ...
 summary=dict(verdict='ORIGINAL_CPU_EVIDENCE_VERIFIED',source=REV,result_sha256=sha(worker['result.json']),
  ...
  mounted_sources=22,contract_sources=38,cuda_initialized=False,binding_runtime_rechecked=True,
  scatter_helper=r['scatter_helper'],resources=resources,failed_prerequisites=failures,
 ...
 readme=f'''# EP native route fusion + PREP_FUSED CPU9 originals
...
'''
 ...
 for n in (7,8):
  failure=next(x for x in failures if x['cpu']==n)
  store('failed-prerequisites%d/README.md'%n,(f"# CPU{n} original failure\n\nSource `{REVISIONS[n]}`; phase `{failure['phase']}`, original verdict FAIL and execution rc1. Presence: `{failure['presence']}`. No successful CPU/GPU acceptance is inferred. See unchanged result and execution/stdout originals.\n").encode())
