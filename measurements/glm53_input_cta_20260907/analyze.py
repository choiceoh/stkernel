...
NAMES=('ICTAB1','ICTAA1','ICTAA2','ICTAB2')
...
def channel_summary(record):
    ...
    spec=importlib.util.spec_from_file_location('_cta_korean_scan',repo/'bench/korean-corruption.py')
    scan=importlib.util.module_from_spec(spec);spec.loader.exec_module(scan)
    rows=[json.loads(line) for line in (ROOT/f"channels-{record['name']}.jsonl").read_text().splitlines() if line.strip()]
    assert len(rows)==len(record['requests'])
    result={key:{'dirty_responses':0,'characters':0,'kinds':{}} for key in ('content','reasoning_content','reasoning')}
    for row,request in zip(rows,record['requests']):
        assert all(row[k]==request[k] for k in ('request_sha256','output_sha256'))
        for key,stats in result.items():
            text=row['channels'][key]
            hits={k:v for k,v in scan.scan(text,truncated=row['finish_reason']=='length').items() if k not in scan.INFORMATIONAL}
            stats['characters']+=len(text)
            stats['dirty_responses']+=int(any(hits.values()))
            for kind,count in hits.items():stats['kinds'][kind]=stats['kinds'].get(kind,0)+count
    # Informational attribution only: the unchanged combined gate stays authoritative.
    return result
...
def summarize(rows, *, incomplete=False):
    ...
    knobs={'VLLM_GLM53_MK_INPUT_CTA':str(mode)}
    ...
    production=json.loads((ROOT/'result.json').read_text())
    assert production['status']=='PASS'
    assert production['source_sha256']==selection['source_sha256']
    ...
    for record in rows:
        ...
        for node in (1,2,3,4):
            ...
            assert proof['knobs']['VLLM_GLM53_MK_INPUT_REUSE']=='1'
            assert proof['knobs']['VLLM_GLM53_MK_INPUT_CTA']==(str(mode) if arm=='A' else '0')
            assert proof['source_sha256']['glm53_megakernel.cu']==production['source_sha256']
            assert proof['mode']==(mode if arm=='A' else 0)
            assert bool(proof['markers'])==(arm=='A')
            if arm=='A': assert any(f'M=6 N=6416 K=4096 mode={mode} split=8' in s for s in proof['markers'])
            if node==2:
                assert proof['boot_id']==record['boot_id']
            receipts.append(proof)
        boot_log=(ROOT/f'boot-{name}.log').read_text(errors='replace')
        assert '[megakernel] input-reuse CAPTURED M=6 N=6416 K=4096 split=8' in boot_log
        assert (f'[megakernel] input-cta CAPTURED M=6 N=6416 K=4096 mode={mode} split=8' in boot_log)==(arm=='A')
        ...
        assert len(requests)==3 and all(q['completion_tokens']==2048 for q in requests)
        ...
        channels=channel_summary(record)
        out.append({'name':name,'arm':arm,'windows':len(windows),'step_s':rate,
                    ...
                    'quality':q,'korean':k,'channels':channels,'quality_pass':q['ok']==q['total'] and k['dirty']==0})
    ...
    return {'source_commit':source,'mode':mode,'per_boot':out,'arms':stats,'paired':paired,
