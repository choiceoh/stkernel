...
def analyze():
    ...
    events=json.loads((ROOT/'job/owned-container-events.json').read_text())
    ended=[r for r in events if r['action']=='die' and '-reuse-' in r['name']]
    assert [r['exit_code'] for r in ended]==['0','15']
    result['process_exits']=ended
    result['memcheck_termination_cause']='unknown; no Python traceback or retained final container resource state'
