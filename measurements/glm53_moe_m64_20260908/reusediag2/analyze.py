...
def analyze():
    ...
    assert process['observed_memory_peak_bytes']==2909007872 and not process['sampler_errors']
    ...
    result['process']={k:process[k] for k in ('exit_code','samples','observed_memory_peak_bytes','last_resource','final_container','sampler_errors')}
    result['driver_allocation_events']=json.loads((ROOT/'job/kernel-allocation-events.json').read_text())
    assert all('NV_ERR_NO_MEMORY' in v[0] for v in result['driver_allocation_events'].values())
    result['memcheck_termination_cause']='NVIDIA driver allocation failure immediately precedes both exits; no container cgroup OOM; specific driver allocation limit remains unresolved'
