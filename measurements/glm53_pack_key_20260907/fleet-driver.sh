#!/usr/bin/env bash
set -euo pipefail
cd /home/choiceoh/glm53-pack-key-20260907
export REPO=$PWD
export STARTUP_CACHE_EVIDENCE=/home/choiceoh/glm53-logs/pack-key-20260907
export STARTUP_CACHE_PREFIX=PACKKEY
export STARTUP_CACHE_MODE=pack-key
...
docker run --rm --gpus all --network none --ipc host \
  ...
  -v "$REPO/build/glm53/glm53_megakernel.py":/usr/local/lib/python3.12/dist-packages/vllm/model_executor/layers/glm53_megakernel.py:ro \
  -v "$REPO/build/glm53/glm53_startup_cache.py":/usr/local/lib/python3.12/dist-packages/vllm/model_executor/layers/glm53_startup_cache.py:ro \
  --entrypoint python3 "$screen_image" /candidate/probes/glm53_pack_key_check.py \
  --out /proof/screen.json > "$STARTUP_CACHE_EVIDENCE/screen.log" 2>&1
python3 - "$STARTUP_CACHE_EVIDENCE/screen.json" <<'GATE'
import json, sys
result = json.load(open(sys.argv[1]))
assert result['ok'] and result['checks'] == 36, result
GATE
