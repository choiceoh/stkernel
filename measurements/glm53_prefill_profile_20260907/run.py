...
def main():
    # Check the live image's HTTP contract before spending a boot. A cache
    # salt is supported without exposing the development-only reset API.
    with urllib.request.urlopen('http://127.0.0.1:8000/openapi.json', timeout=10) as r:
        api = json.load(r)
    for route in ['/start_profile', '/stop_profile']:
        if route not in api['paths']:
            raise RuntimeError('profiler API missing: ' + route)
    if 'cache_salt' not in api['components']['schemas']['ChatCompletionRequest']['properties']:
        raise RuntimeError('request-level cache isolation unsupported')
    before = snapshot()
    save('production-before.json', before)
    reference = (json.loads(Path(os.environ['PROFILE_REFERENCE']).read_text())
                 if os.environ.get('PROFILE_REFERENCE') else before)
    verify(reference, before, 'reference')
    head = reference['local']
    expected = {}
    ...
    # GMU was read after the launcher's graph-budget correction, so do not
    # subtract that correction for a second time when reproducing it.
    env['CG_UTIL_DELTA'] = '0'
