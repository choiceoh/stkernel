...
export HEAD_URL=http://127.0.0.1:18000
changed=0
finish() {
  rc=$?
  trap - EXIT
  if [ "$changed" = 1 ] && bash "$FLEET" restore-needed "$FLEET_SESSION" >/dev/null 2>&1; then
    ...
    export HEAD_URL=http://10.10.10.2:8000
    LEGS=none bash "$LEVER" SPFR30907PROD "" > "$RETRY_JOB/production-restore.log" 2>&1 || { restore_rc=$?; [ "$rc" != 0 ] || rc=$restore_rc; }
...
for host in local choiceoh@10.10.10.1 choiceoh@10.10.10.3 choiceoh@10.10.10.4; do
  ...
  [[ "$available" =~ ^[0-9]+$ ]] && [ "$available" -ge 134217728 ] || { echo "REFUSED: $host has less than 128 GiB spare disk before deploy"; exit 3; }
done
changed=1
bash "$FLEET" deploy "$FLEET_SESSION" "$RETRY_REV"
[ "$(git -C "$REPO" rev-parse HEAD)" = "$RETRY_REV" ]
changed=1
bash "$REPO/bench/chain.sh" SPFR30907B1="" \
 SPFR30907A="VLLM_GLM53_PREFILL_SP_FUSE_MHC=1 VLLM_GLM53_PREFILL_SP_FP8_AG_MIN_TOKENS=2048 VLLM_GLM53_PREFILL_SP_FP8_RS_MIN_TOKENS=4096" \
 SPFR30907B2="" \
 --after SPFR30907B1 "python3 '$RETRY_JOB/attest.py' SPFR30907B1" \
 --after SPFR30907A "python3 '$RETRY_JOB/attest.py' SPFR30907A" \
 --after SPFR30907B2 "python3 '$RETRY_JOB/attest.py' SPFR30907B2"
