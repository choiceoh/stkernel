...
helper=repo/'overlay/modules/glm53_runtime/glm53_prefill_collectives.py';helper_sha=hashlib.sha256(helper.read_bytes()).hexdigest()
for ip,n in nodes.items():
 ...
 helpers=[v['sha256'] for k,v in n['mounts'].items() if k.endswith('/glm53_prefill_collectives.py')]
 if helpers!=[helper_sha]:issues.append(ip+': helper fingerprint mismatch')
 if n['stamp'][:12]!=r['overlay']:issues.append(ip+': overlay stamp mismatch')
