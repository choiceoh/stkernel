...
finish() {
  ...
  if [ "$changed" = 1 ] && bash "$FLEET" restore-needed "$FLEET_SESSION" >/dev/null 2>&1; then
    ...
    LEGS=none bash "$LEVER" SPFRT0907PROD "" > "$RETRY_JOB/production-restore.log" 2>&1 || { restore_rc=$?; [ "$rc" != 0 ] || rc=$restore_rc; }
...
changed=1
bash "$REPO/bench/chain.sh" SPFRT0907B1="" \
 SPFRT0907A="VLLM_GLM53_PREFILL_SP_FUSE_MHC=1 VLLM_GLM53_PREFILL_SP_FP8_AG_MIN_TOKENS=2048 VLLM_GLM53_PREFILL_SP_FP8_RS_MIN_TOKENS=4096" \
 SPFRT0907B2="" \
 --after SPFRT0907B1 "python3 '$RETRY_JOB/attest.py' SPFRT0907B1" \
 --after SPFRT0907A "python3 '$RETRY_JOB/attest.py' SPFRT0907A" \
 --after SPFRT0907B2 "python3 '$RETRY_JOB/attest.py' SPFRT0907B2"
