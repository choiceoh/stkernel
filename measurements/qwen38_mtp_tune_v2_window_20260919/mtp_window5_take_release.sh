#!/usr/bin/env bash
# Window 5's third takeover (2026-09-19): the operator chose to go to self-distribution data in the next window, so
# the original head's arm is dropped -- the T=1 block arm runs, then release. 5c's variables and functions taken whole.
set -uo pipefail
...
# == take-release: the block arm, then release (operator 20:5x: 블록 팔까지, 2번은 다음 창) ======================================
...
[ -n "$running" ] || { stamp "take-release: no arm's requests running: leaving the takeover alone"; released=1; exit 1; }
pkill -9 -f "bash mtp_window5_take_orig.sh"
sleep 2
mine || { stamp "take-release: the lease is not ours: $(lease read)"; released=1; exit 1; }
( while sleep 120; do lease renew --owner "$OWNER" >/dev/null 2>&1; done ) &
BEAT=$!
stamp "take-release: took the window during $running's requests (the original head's arm dropped: self-distribution data comes first)"
