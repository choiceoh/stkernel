#!/usr/bin/env python3
"""Window 6's arms, paired by prompt: the fifth and third heads against the original, greedy and T=1 (block verification), over the
...
for line in open(sys.argv[2] if len(sys.argv) > 2 else d / "prompts.jsonl"):
    ...
    if p.get("split") == "eval":
        ...
        if not p.get("category") and p.get("kind") and not p["id"].startswith(("v2-", "d-", "r-", "m-")):
            p["category"] = "v1_" + p["kind"]
        prompts[p["id"]] = p
PAIRS = (("orig-off", "tuned5-off"), ("orig-t1", "tuned5-t1"), ("orig-off", "tuned3-off"), ("tuned3-off", "tuned5-off"))


def suite(i):
    return ("v2" if i.startswith("v2-") else "crafted" if i.startswith(("s-", "s2-")) else "deneb" if i.startswith("d-")
            else "records" if i.startswith("r-") else "mail" if i.startswith("m-") else "v1")
