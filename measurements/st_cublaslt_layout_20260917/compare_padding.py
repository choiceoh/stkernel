...
def main():
    ...
    ap.add_argument('--rows', type=int, nargs='+', default=[8, 16, 32, 64])
    ap.add_argument('--parts', type=int, nargs='+', default=[5])
    ap.add_argument('--row-padding', action='store_true')
    ap.add_argument('--paddings', type=int, nargs='+', default=[0,128,256,384,512,1024])
    ...
    paths = ('engine/kernels/dense/cublaslt.cpp', 'engine/kernels/dense/cublaslt_split.py',
             'engine/kernels/dense/cublaslt_serving.py', 'probes/engine_cublaslt_padding_check.py')
        ...
        for m in args.rows:
            ...
            for parts in args.parts:
                ...
                for padding in args.paddings:
                    w = torch.nn.functional.pad(original_view, (0,0,0,padding) if args.row_padding else (0,padding)).contiguous()
                    plan = native.Plan(context, m, n, kp, 0, query, ws, parts, True, True, 0 if args.row_padding else padding, padding if args.row_padding else 0)
                    cell = dict(shape=[m, n, k], parts=parts, padding=padding, row_padding=args.row_padding, search=plan.statistics())
                    if not plan.candidates():
                        ...
                        with torch.cuda.stream(stream):
                            trial()
                            _measure(base, None, repeats=2); _measure(trial, None, repeats=2)
                        ...
                        for scale in (.5, 2.):
                            ...
                            torch.testing.assert_close(out, ref, rtol=.01, atol=.001)
                        cell.update(status='PASS', choice=choice, extra_weight_bytes=ws.numel()+w.numel())
