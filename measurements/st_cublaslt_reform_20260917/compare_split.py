...
def main():
    ...
    source=Path(__file__).with_name('split_plan.cpp')
    flags,links=['-O2','-std=c++17'],['-lcublasLt']
    key,directory,sources=prepare_cuda_sources(build_root('cublaslt-split'),[source],(flags,links,torch.__version__,torch.version.cuda))
    split_native=load(name='st_cublaslt_split_'+key,sources=list(sources),extra_cflags=flags,extra_ldflags=links,with_cuda=True,build_directory=str(directory))
    ...
    try:
        for m in args.rows:
            ...
            for parts in args.parts:
                ...
                ws=[mxfp8.pack_weight_scales(scales[p],n,kp) for p in range(parts)]
                ...
                pplan=split_native.Plan(split_context,m,n,kp,WORKSPACE_LIMIT,sparts[0],ws[0])
                ...
                ranked=[]; runners={}
                for c in candidates:
                    workspaces=[torch.empty(c['workspace'],device='cuda',dtype=torch.uint8) for _ in streams]
                    bindings=[pplan.bind(c['index'],qparts[p],w[p],sparts[p],ws[p],partial[p],workspaces[p]) for p in range(parts)]
                    def run(bindings=bindings):
                        parent=torch.cuda.current_stream()
                        produce(); ready.record(parent)
                        for stream,end,binding in zip(streams,done,bindings):
                            with torch.cuda.stream(stream):
                                stream.wait_event(ready); binding.run(); end.record(stream)
                        for end in done: parent.wait_event(end)
