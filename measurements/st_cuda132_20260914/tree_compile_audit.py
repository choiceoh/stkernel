# Derived from probes/engine_tree_dataflow_compile.py; collect every variant instead of stopping at the first unsupported compiler path.
...
def compile_variants(output):
    ...
    report = {"scope": "offline SM121 compilation audit; failures remain explicit, no GPU execution", "gpu_used": False,
    ...
    for name, fn, signature, constants in variants:
        ...
        try:
            ...
            report["variants"].append({"name": name, "status": "FAIL", "error": str(error)})
    report["status"] = "FAIL" if any(v.get("status") == "FAIL" for v in report["variants"]) else "PASS"
...
if __name__ == "__main__":
    ...
    raise SystemExit(0 if compile_variants(args.output)["status"] == "PASS" else 1)
