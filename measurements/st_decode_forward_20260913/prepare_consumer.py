...
def rank_layout(rank, host):
    # Each Spark owns only its rank file; homes are not shared.
    script = ("import json\n"
              f"with open({str(RANKS / f'rank{rank}of4.safetensors')!r}, 'rb') as f:\n"
              " h=json.loads(f.read(int.from_bytes(f.read(8), 'little')))\n"
              "print(h['__metadata__']['weight_layout'])\n")
...
def prepare_metadata():
    ...
    for rank, host in enumerate(NODES):
        assert rank_layout(rank, host) == F.weight_layout, (rank, host)
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
        receipts = list(pool.map(prepare, NODES))
