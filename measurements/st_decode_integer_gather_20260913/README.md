...
The named prefill-path fix from `a03fe14b5` is integrated, retaining required gather/reduce-scatter and selected tiled-projection checks. Main through PR #834 is also integrated, including PR #833 CUDA 13 graph attribution and PR #832 sampling keys. Source `05e7a8c2` passes 65 focused CPU tests, with 4 GPU-only skips (`cpu/integration-830.log`). The replacement candidate-only ticket is `st-decode-native-consumer0913v3`; previous source records remain separate.

The first v3 preparation was refused before GPU admission when main advanced to PR #835. Its common-kernel relocation is now integrated, and the native gather GPU oracle imports the relocated candidate module. The resulting CPU suite passes 82 tests, with 12 GPU-only skips (`cpu/common-integration-830.log`); this preparation attempt did not boot or measure a model.
