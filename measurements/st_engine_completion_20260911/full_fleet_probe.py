...
name='st-full-completion-9391'
source='/tmp/st-engine-completion'
env={
    'MASTER_ADDR':nodes[0], 'MASTER_PORT':'29693','WORLD_SIZE':'4','LOCAL_RANK':'0',
...
if sys.argv[1]=='start':
    for rank,ip in enumerate(nodes):
        ...
        run(ip,['mkdir','-p',cache,'/home/choiceoh/.cache/st-completion-tier'])
        ...
        for src,dst,ro in [(source,'/repo',True),('/home/choiceoh/models/st-glm53-9391-up-gate-full','/ranks',True),('/home/choiceoh/models/GLM-5.3-Flash-DFlash2','/draft',True),('/home/choiceoh/.cache/st-completion-tier','/tiers',False),(cache,'/cache',False)]:
            args+=['--mount',f'type=bind,src={src},dst={dst}'+(',readonly' if ro else '')]
        command='source /repo/launchers/lib/common-tp4.sh; eval "$CT_GID_PRELUDE"; exec '+shlex.join(['python3','-u','/repo/probes/engine_full_check.py','--ranks','/ranks','--ckpt-meta','/repo/meta','--drafter-dir','/draft','--tier-dir','/tiers','--port','29692'])
    ...
    for rank,ip in enumerate(nodes):
        with open(f'{source}/full-fleet-rank{rank}.log','w') as out:
