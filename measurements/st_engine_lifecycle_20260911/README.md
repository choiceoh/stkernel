...
Base: PR #535 merge `3c9622b7d5eb8b326322c8933a20668381015c2f`.
...
|---|---|---|
| CUDA regression suite on srv1 | 71 passed, zero skips | `engine-tests.log` |
| Same suite without PyTorch | 54 passed, 17 skipped | `cpu-tests.log` |
...
| Four logical ranks through LocalTP | identical admission/reuse order, results and stop; 12 requests per rank | regression suite |
...
The 71-test suite was repeated after the final large-integer input validation
change. The unchanged I/O probe ran immediately before that validation-only
change. No serving containers, production checkpoint files or other fleet
