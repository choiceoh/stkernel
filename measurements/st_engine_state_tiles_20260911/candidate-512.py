...
def kda_history(conv, rec, slot, context, history):
    ...
    width = state.numel()
    block = min(512, triton.next_power_of_2(width))
    ...
    _read_rec[(triton.cdiv(width, block),)](
        rec, slot, context, state, rec.stride(0), rec.stride(1), rec.shape[1], width, block)
    return hist, state
...
def write_ring(src, dst, slot, context):
    ...
    block = min(512, triton.next_power_of_2(width))
    _write_ring[(length-first, triton.cdiv(width, block))](
        flat, dst, slot, context, flat.stride(0), dst.stride(0), dst.stride(1),
        width, dst.shape[1], first, block)
