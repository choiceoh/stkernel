...
@triton.jit
def _single_conv_parallel(X, W, S, Y, F, T: tl.constexpr, C: tl.constexpr,
                         XS: tl.constexpr, XC: tl.constexpr, WS: tl.constexpr, WC: tl.constexpr,
                         SS: tl.constexpr, SC: tl.constexpr, K: tl.constexpr,
                         HAS_STATE: tl.constexpr, BC: tl.constexpr, BT: tl.constexpr):
    c = tl.program_id(0) * BC + tl.arange(0, BC)
    start = tl.program_id(1) * BT
    token = start + tl.arange(0, BT)
    acc = tl.zeros((BT, BC), tl.float32)
    for j in tl.static_range(K):
        pos = token - (K - 1) + j
        value = tl.load(X + pos[:, None] * XS + c[None, :] * XC,
                        (c[None, :] < C) & (pos[:, None] >= 0) & (token[:, None] < T), other=0.)
        if HAS_STATE:
            old = tl.load(S + c[None, :] * SS + (pos[:, None] + K - 1) * SC,
                          (c[None, :] < C) & (pos[:, None] < 0) & (token[:, None] < T), other=0.)
            value = tl.where(pos[:, None] < 0, old.to(X.dtype.element_ty), value)
        w = tl.load(W + c * WS + j * WC, c < C, other=0.)
        acc += value * w[None, :]
    acc = acc / (1 + tl.exp(-acc))
    tl.store(Y + token[:, None] * C + c[None, :], acc, (c[None, :] < C) & (token[:, None] < T))
    if start + BT >= T:
        for j in tl.static_range(K - 1):
            final_pos = T - (K - 1) + j
            final_value = tl.load(X + final_pos * XS + c * XC, (c < C) & (final_pos >= 0), other=0.)
            if HAS_STATE:
                final_old = tl.load(S + c * SS + (final_pos + K - 1) * SC, (c < C) & (final_pos < 0), other=0.)
                final_value = tl.where(final_pos < 0, final_old.to(X.dtype.element_ty), final_value)
            tl.store(F + c * (K - 1) + j, final_value, c < C)
