...
state-budget tests passed in 4.314 seconds. Thirty SM121 commit configurations
...
64/128 registers. All have zero local memory, stack and shared-memory usage.
The latter is a resource cost, not an assumed speedup. `compile.json` and
`cpu.log` preserve these results.

The short GPU gate compares five implementations at identical addresses,
FP32 factors and initial state: flat, the first tiled implementation, and
hoisted 1024/2048/4096-cell tiles. It covers C=1/C=4, accepted counts 1/3/8,
