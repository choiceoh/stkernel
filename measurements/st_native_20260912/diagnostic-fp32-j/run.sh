...
set -euo pipefail
ROOT=/home/choiceoh/st-native-long-fp32-j
CANDIDATE=/home/choiceoh/st-releases/perf-f4d7-20260912j
...
changed=0
ingress_closed=0
drain_rule=(-p tcp --dport 8000 '!' -i lo -m conntrack --ctstate NEW -m comment --comment st-native-long-fp32-j-drain -j REJECT --reject-with tcp-reset)
reopen_ingress() {
...
restore() {
  ...
  reopen_ingress
  sudo -n systemctl stop st-native-long-fp32-j-drain-cleanup.timer
...
sudo -n systemd-run --quiet --unit=st-native-long-fp32-j-drain-cleanup --on-active=60m \
