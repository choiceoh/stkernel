...
try:
 ...
 variants=[('native-marks',True,True)]
 for label,use_sp,use_nv in variants:
  ...
  try:
   for ctx in range(0,len(ids),6912):
    part=ids[ctx:ctx+6912]
    marks=tuple((rel,j) for j,rel in enumerate(range(768,len(part),768)))
    step=Step.prefill(torch.tensor(part,device='cuda',dtype=torch.int64),ctx,0,slot,marks=marks)
 ...
 comm.barrier()
 directory=Path('/diag/commands');directory.mkdir(exist_ok=True)
 consumed=set();deadline=time.monotonic()+1200
 ...
 while time.monotonic()<deadline:
  ...
  if comm.rank==0:
   ...
   if pending:
    p=pending[0];command=(p.name,p.read_text());consumed.add(p.name)
  command=comm.broadcast_object(command)
  if command:
   name,source=command
   if source.strip()=='STOP':break
   print('DIAGNOSTIC_COMMAND '+name,flush=True)
   exec(compile(source,name,'exec'),globals())
   torch.cuda.synchronize();comm.barrier();print('DIAGNOSTIC_DONE '+name,flush=True)
  else:time.sleep(1)
finally:
