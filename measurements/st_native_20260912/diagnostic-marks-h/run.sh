...
set -euo pipefail
ROOT=/home/choiceoh/st-native-long-marks-h
...
changed=0
ingress_closed=0
drain_rule=(-p tcp --dport 8000 '!' -i lo -m conntrack --ctstate NEW -m comment --comment st-native-long-marks-h-drain -j REJECT --reject-with tcp-reset)
reopen_ingress() {
...
restore() {
  ...
  reopen_ingress
  sudo -n systemctl stop st-native-long-marks-h-drain-cleanup.timer
...
sudo -n systemd-run --quiet --unit=st-native-long-marks-h-drain-cleanup --on-active=60m \
