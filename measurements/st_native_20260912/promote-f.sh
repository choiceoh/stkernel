...
drain_rule=(-p tcp --dport 8000 '!' -i lo -m conntrack --ctstate NEW -m comment --comment st-native-f-promotion -j REJECT --reject-with tcp-reset)
changed=0
closed=0
adopted=0
finish() {
  ...
  sudo -n systemctl stop st-native-f-promotion-cleanup.timer
  printf '%s\n' "$rc" > exit-code.txt
  exit "$rc"
}
trap finish EXIT
...
closed=1
sudo -n systemd-run --quiet --unit=st-native-f-promotion-cleanup --on-active=60m \
...
PY
for ip in 10.10.10.2 10.10.10.1 10.10.10.3 10.10.10.4; do
  node_sh "$ip" "docker tag st-engine:perf-f4d7-f $IMAGE"
done
changed=1
...
ST_TIER_DIR=/home/choiceoh/glm53-logs/st-tier-native-e24ea2a11529
ST_DUMP_DIR=/home/choiceoh/glm53-logs/st-dumps-native-e24ea2a11529
PORT=8000
EOF
