...
set -euo pipefail
ROOT=/home/choiceoh/st-native-f4d7-20260912d
CANDIDATE=/home/choiceoh/st-releases/perf-f4d7-20260912d
...
collect() {
  ...
  for ip in 10.10.10.2 10.10.10.1 10.10.10.3 10.10.10.4; do
    ...
    node_sh "$ip" "cat /home/choiceoh/glm53-logs/st-native-f4d7-d-dumps/memory-rank$r.json" > "$ROOT/$phase/memory-rank$r.json" 2>/dev/null || true
    r=$((r+1))
  done
}
wait_door() {
...
export ST_ENGINE_DIR="$CANDIDATE" ST_IMAGE=st-engine:perf-f4d7-d ST_PRODUCTION=1 PORT=8001
export CKPT="$CANDIDATE/st-glm53-meta"
unset STK_execution STK_moe_static
export ST_TIER_DIR=/home/choiceoh/glm53-logs/st-native-f4d7-d-tier
export ST_DUMP_DIR=/home/choiceoh/glm53-logs/st-native-f4d7-d-dumps
...
GLM53_API_PORT=8001 BENCH_MODEL=glm-5.3-flash SPEC_K=5 timeout --signal=TERM --kill-after=15 1200 \
  python3 -u run_onepass_st.py --name ST-NATIVE-F4D7-20260912D \
