...
- 먼 끝 패스: 29.52–30.57 s(표).
- 대신 도는 이어 쓰기: 약 1.6–2 s(추정). 원장의 `prefill/1024/0` 이 1.54–1.66 s 이고, 문맥 32,256 의 선택이 조금 더한다.
- 키 계산: 약 0.3 s(추정). 엔진 트리 6.4 MiB, 헤더 셋, tokenizer.json 20 MiB.
- **합: 재부팅 한 번에 약 −28 s (추정, GPU 부팅 미측정).**
...
결과: 147 테스트 OK(12 스킵). `tests/test_engine_prefill_record.py` 는 24 테스트이고, 다룬 것은 이렇다.
