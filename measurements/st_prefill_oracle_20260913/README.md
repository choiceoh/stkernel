...
`sum-pack-r2`는 소스 `eb58d244`에서 **49개 검증 중 42 통과·7 생략**, 9개 변형 컴파일 통과
...
최대 절대 오차 0.58984375가 재현됐다. 당시에는 원인을 확정하지 못했다.
...
`run_cpu.py`의 기본 전체 검증에는 해당 테스트를 그대로 유지했다. `sum-pack-r2`는 `--tests`로 새 후보와
영향 범위를 명시한 집중 검증이며 전체 상태 검증 통과나 GPU 품질 통과를 뜻하지 않는다.

머지 전 CPU 추적에서 첫 차이는 동일 입력에 대한 L0 mHC의 FP32 계수(최대 1.19e-7)였다.
전체 행과 분할 행의 ARM BLAS reduction 순서 차이가 뒤 BF16·attention 연산에서 증폭됐다.
상태 테스트에서 같은 실제 mHC 기준 함수를 양쪽 모두 행별로 실행하도록 고쳤다. 출력·aux·KDA·KV·
경계 snapshot·후속 decode의 `rtol=0, atol=0` 조건은 유지했다. 실행용 커널은 변경하지 않았다.

최신 main을 결합한 `a112efa7`의 `merge-r2`는 기본 검증 전체 **50개 중 43 통과·7 생략**,
9개 실제 커널 컴파일 통과(36.85초, CUDA 미초기화)다. 앞선 `merge-r1` 실패 로그도 남겼다.
M64와 scale expansion의 이전 실험 자동 선택도 해제해 모든 새 후보가 명시 opt-in이 됐다.
별도 opt-in/오라클 검증은 80개 중 78 통과·2 생략이다. GPU 품질·성능 검증은 여전히 미실행이다.
