...
|---|---|---|---|
| Indexer tail (`index_kpool_always_select_tail`) | #1158/#1159 pin the pool that just completed above the top-k on rows whose length is a whole number of pools | `append_visible_tail` appends the `seq % 4` newest tokens and nothing else; every complete pool competes on its score (sglang's `append_tail_to_topk` and upstream vLLM agree) | Kept (operator, 2026-09-18): the pin was in place for the #1174 replays, which still failed, and the same-build controls in [`st_telemachus_tail_reference_20260918`](../st_telemachus_tail_reference_20260918/README.md) (#1178) found the unpinned T=1 replay substantially worse; a deviation from the model on one pool of 512 on every fourth position, retained on that evidence |
| Sampling defaults for a request that omits `top_p` | the served meta's `generation_config.json` (cut from the quantised repository) names only temperature 1.0, so the door sampled the whole tail | zai-org/GLM-5.3-Flash ships temperature 1.0, **top_p 0.95** | **Restored** by #1176 (`boot.generation_defaults` supplies 0.95 only when the meta omits it; explicit checkpoint or request values win; the raw `/v1/engine/completions` dialect stays caller-controlled). Deneb's GLM profile carries no sampling override, so its requests get the model's nucleus once production boots a source with #1176; the incident's own record ran at top_p 1, and #1176's replay with top_p 0.95 on an earlier build still failed, so this is a tail guard, not the repair |
...
head, and it shapes the sampled distribution directly), the restored nucleus (#1176) or a
min-p for clients that omit them, or context hygiene (Deneb's R1'/R2, 2026-09-18).
