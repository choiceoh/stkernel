"""Run the official CLI with performance and original console output, then release the hold."""
...
command = [str(Path.home()/'.local/bin/uv'), 'tool', 'run', '--from',
           ...
           '--output-dir', str(root/'runs'), '--no-live',
...
# Keep CLI console output intact: --json-file implies --json and suppresses
# the original Rich tables. The completed official SQLite row is copied below.
if args.protocol != 'reference_t1' or args.diagnostics_only:
    raise ValueError('The performance workflow requires the full reference T=1 protocol')
benchy_args = shlex.join([
    '--exact-tg', '--extra-body', 'temperature=1', '--extra-body', 'top_p=0.95',
    '--extra-body', 'seed=42', '--extra-body', 'retain=false',
    '--extra-body', 'chat_template_kwargs={"thinking":true}',
])
command += ['--perf', '--pp', '2048', '--tg', '512', '--depth', '0,4096,8192',
            '--concurrency', '1,2', '--benchy-runs', '3',
            '--tokenizer', '/home/choiceoh/st-releases/'+args.sha[:12]+'/st-glm53-meta/tokenizer.json',
            '--benchy-args', benchy_args]
command[command.index('--label')+1] = f'ST {args.sha[:8]} full88 T1 plus official perf C1 C2'
manifest.update(command=command, performance={'pp':2048, 'tg':512, 'depths':[0,4096,8192],
    'concurrency':[1,2], 'runs':3, 'exact_tg':True, 'temperature':1, 'top_p':.95,
    'seed':42, 'thinking':True, 'retain':False}, original_console=True)

def export_completed_database():
    db=sqlite3.connect('file:'+str(root/'data/benchmarks.sqlite')+'?mode=ro',uri=True)
    db.row_factory=sqlite3.Row
    rows=db.execute("SELECT * FROM scenario_runs WHERE run_type='tool_eval' AND status='completed'").fetchall()
    if len(rows)!=1:
        raise RuntimeError(f'Expected one completed official quality run, found {len(rows)}')
    row=dict(rows[0]);scores=json.loads(row['scores_json']);config=json.loads(row['config_json'])
    metadata=json.loads(row['metadata_json'])
    result={'source':'Unmodified completed official SQLite row; no rescoring',
        'tool_eval_bench_version':metadata['tool_version'],'run_id':row['run_id'],
        'status':row['status'],'config':config,'scores':scores,'metadata':metadata,
        'report_path':row['report_path'],'final_score':scores['final_score'],
        'rating':scores['rating'],'safety_warnings':scores.get('safety_warnings',[]),
        'deployability':scores.get('deployability'),'responsiveness':scores.get('responsiveness'),
        'total_scenarios':len(scores['scenario_results'])}
    (root/'result.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
    (root/'official-database-row.json').write_text(json.dumps(row,ensure_ascii=False,indent=2)+'\n')
