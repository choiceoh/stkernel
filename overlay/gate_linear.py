...
# deneb fork: port of upstream issue #49921 — the specialized-router gate
# excludes consumer Blackwell (family 120, GB10 = SM121), so DeepSeek-V4's
# fp32 router falls to the Tier-4 F.linear path with fp32-stored weights.
# Default OFF (launcher knob RGEMM=1 -> DENEB_ROUTER_SPECIALIZED=1): enabling
# changes the router weight dtype at load (bf16 instead of fp32) and the
# kernel tier, and the specialized kernels are only validated on family(100)
# upstream — it must clear check-quality + bench-dec before promotion.
_DENEB_ROUTER_SPECIALIZED = os.environ.get("DENEB_ROUTER_SPECIALIZED") == "1"
...
class GateLinear(ReplicatedLinear):
    ...
    def __init__(
        ...
        # deneb fork (#49921): family(100) upstream; env-gated family(120)
        # extension for GB10. With the env unset this is byte-identical to
        # the image's behavior.
        is_blackwell = current_platform.is_device_capability_family(100) or (
            _DENEB_ROUTER_SPECIALIZED
