...
class DeepSeekV3ReasoningParser(ReasoningParser):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizerBase, *args, **kwargs):
        ...
        # The deepseek_v4 template wrapper renders reasoning_effort="none" as
        # chat mode (prompt pre-closes with </think>), so the stream never
        # contains a </think> marker. Mirror that here — otherwise the R1
        # parser classifies the entire response as reasoning and content
        # stays empty for such requests.
        if chat_kwargs.get("reasoning_effort") == "none":
            thinking = False
