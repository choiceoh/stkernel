...
# deneb fork: the fused small-M MoE router gate lives in its own overlay
# module -- the tier-dispatch gap it works around belongs to sm_121, not to
# this model. See overlay/modules/moe_gate_sm121/.
...
class DeepseekV4MoE(nn.Module):
    def __init__(
        ...
        # deneb fork: DenebGateLinear adds the fused small-M decode tier
        # (VLLM_DSV4_GATE_FUSED, default off); with the knob off it is
        # byte-identical to stock GateLinear dispatch.
...
class DeepseekV4ForCausalLM(
    ...
    def _maybe_fp8_lm_head_logits(
        self, hidden_states: torch.Tensor
    ) -> torch.Tensor | None:
        """VLLM_DSV4_TARGET_LM_HEAD_FP8 (default off): compute VERIFICATION
        logits through an fp8-deepgemm copy of the target lm_head.

        The bf16 vocab shard (32,320 x 4096 = 265MB/rank) is streamed on every
        decode step (measured: the grid=[8,253,1] wmma population, 1.16 ms/step
        = 2.7% of the step). fp8 halves those bytes. UNLIKE the draft-head/
        markov fp8 moves this is OUTSIDE the rejection-sampling losslessness:
        verification logits pick accept/reject AND the sampled token, so this
        knob ships default-OFF and adoption requires the greedy-divergence
        bracket gate (see KERNEL_CAMPAIGN.md) on top of 9/9.
        ...
        local = _fp8_gemm(
            ...
            self._tgt_head_fp8_w,
            self._tgt_head_fp8_ws,
        )
    ...
    def compute_logits(
        ...
        fp8_logits = self._maybe_fp8_lm_head_logits(hidden_states)
        if fp8_logits is not None:
            return fp8_logits
