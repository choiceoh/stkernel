...
# adds nothing. Restored verbatim from the checkpoint's reference implementation
# (model repo encoding/encoding_dsv4.py); the previous overlay-base collapsed
# this to a single "max" level that carried the reference *high* text, leaving
# reasoning_effort=high a silent no-op and the reference max text unreachable.
...
def encode_arguments_to_dsml(tool_call: Dict[str, Any]) -> str:
    ...
    if isinstance(tool_call["arguments"], str):
        try:
            ...
            # Reference behavior: histories resent by clients can carry
            # arguments that are not valid JSON (e.g. "" for no-arg tools);
            # degrade to a wrapper parameter instead of failing the encode.
            arguments = {"arguments": tool_call["arguments"]}
    else:
        arguments = tool_call["arguments"]
    if not isinstance(arguments, dict):
        # json.loads can legally yield non-dicts ("null", "[]", "3") — those
        # have no parameter names to map, so wrap them the same way.
        arguments = {
            "arguments": tool_call["arguments"]
            if isinstance(tool_call["arguments"], str)
            else to_json(arguments)
        }
...
def render_message(index: int, messages: List[Dict[str, Any]], thinking_mode: str, drop_thinking: bool = True, reasoning_effort: Optional[str] = None) -> str:
    ...
    if reasoning is None:
        # DeepSeek's public API returns reasoning under "reasoning_content";
        # accept it on input so such clients can round-trip their histories.
