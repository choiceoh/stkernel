...
def gumbel_noised_argmax(
    ...
    if temp != 0.0:
        if IS_DRAFTING:
            # deneb fork (vLLM #54282): keep the draft's noise stream disjoint
            # from the target's, or the residual a rejected proposal is
            # resampled from shares that proposal's noise vector.
