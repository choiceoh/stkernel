...
# deneb fork (vLLM #54282): offset salt keeping the draft's Gumbel noise
# disjoint from the target's. Verification is a probability-ratio test, not a
# Gumbel coupling, so a proposal and the residual it is resampled from must not
# share a noise vector. Positions are int64 and never approach 2**30, so the
...
def gumbel_block_argmax(
    ...
    if temp != 0.0:
        ...
        if IS_DRAFTING:
            # deneb fork (vLLM #54282): without this the noise that produced a
            # draft token and the noise that resamples it after rejection come
            # from the same Philox offset, so the residual under-weights exactly
            # the tokens the draft ranked highest and the output distribution is
            # no longer the target's. probabilistic drafting only.
