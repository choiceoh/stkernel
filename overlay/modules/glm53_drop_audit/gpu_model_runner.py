...
_DENEB_DROP_AUDIT = os.environ.get("VLLM_DENEB_DROP_AUDIT") == "1"


def _deneb_audit_discards(mask, seq_lens, num_tokens, req_ids, requests):
    """Report a sampled token thrown away after its prompt was done.

    get_output() clears a request's tokens for the step when this mask is set.
    That is right while the prompt is still being consumed and wrong once the
    request is generating -- and in the non-speculative path it is the only way
    a generated token can fail to reach the client.
    """
    for i in np.nonzero(mask)[0]:
        req = requests.get(req_ids[i])
        if req is None:
            continue
        if int(seq_lens[i]) >= int(req.num_prompt_tokens):
            logger.warning(
                "deneb drop-audit: discarded sampled token for %s past prefill "
                "(seq_len=%d num_tokens=%d prompt=%d)",
                req_ids[i], int(seq_lens[i]), int(num_tokens[i]),
                int(req.num_prompt_tokens),
            )
...
class GPUModelRunner(
    ...
    def _prepare_inputs(
        ...
        if _DENEB_DROP_AUDIT:
            _deneb_audit_discards(
                self.discard_request_mask.np[:num_reqs],
