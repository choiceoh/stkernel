...
def _deneb_audit_contiguity(valid_mask, ids):
    """Check the prefix assumption three separate places depend on.

    A valid entry after a placeholder makes the accepted-token counts wrong and
    makes parse_output's boolean mask close the hole instead of reporting it.
    """
    bad = np.nonzero(((np.cumsum(~valid_mask, axis=1) > 0) & valid_mask).any(axis=1))[0]
    for r in bad:
        logger.warning(
            "deneb drop-audit: non-contiguous sampled tokens in row %d: "
            "mask=%s ids=%s",
            int(r), valid_mask[r].tolist(), ids[r].tolist(),
        )
...
class RejectionSampler(nn.Module):
    ...
    def parse_output(
        ...
        if _DENEB_DROP_AUDIT:
            _deneb_audit_contiguity(valid_mask, output_token_ids_np)
