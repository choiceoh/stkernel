...
class CudaGraphManager:
    ...
    def _init_candidates(self) -> None:
        ...
        if (
            ...
            # deneb fork (vLLM #54801): the sequence-length schedule picks K
            # the same way and its entries have the same
            # (range_start, range_end, num_speculative_tokens) shape, so the
            # decode query lengths to capture are the same union. This image's
            # copy reads the schedule directly instead of going through
            # build_dynamic_sd_schedule_lookup, so the upstream hunk is applied
            # here by hand.
            num_spec_per_batch_size = (
                speculative_config.num_speculative_tokens_per_batch_size
                or speculative_config.num_speculative_tokens_per_seq_len
            )
            # uses_dynamic_speculative_decoding() guarantees one is set.
