...
class SpeculativeConfig:
    ...
    """Sequence-length schedule used to dynamically choose speculative-token
    count (vLLM #54801). Same entry shape, but the range is over the batch's
    longest sequence: draft verification cost scales with context length, so
    K can be wound down (to 0) on long inputs while short ones keep the full
    draft. Ignored when the batch-size schedule is also set.
    """
