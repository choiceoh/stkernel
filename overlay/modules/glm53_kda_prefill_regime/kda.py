...
_GLM53_KDA_PREFILL_REGIME_ENV = "VLLM_GLM53_KDA_PREFILL_REGIME"
...
def _glm53_kda_prefill_regime_gate(
    *,
    enabled: bool,
    ...
    """Fail closed to the stock autotune cache on any production drift."""
    if len(q_shape) != 4:
        return False
    ...
    return (
        ...
        and key_dim == 128
        and k_shape == q_shape
        and v_shape == (batch, tokens, heads, 128)
        and raw_g_shape == q_shape
        and beta_shape == (batch, tokens, heads)
        and q_dtype == "torch.bfloat16"
        and k_dtype == "torch.bfloat16"
        and v_dtype == "torch.bfloat16"
        and raw_g_dtype == "torch.bfloat16"
        # GLM's _cast_sigmoid deliberately supplies fp32 beta to this path.
        and beta_dtype == "torch.float32"
        and initial_state_shape == (num_sequences, heads, key_dim, 128)
        and initial_state_dtype == "torch.float32"
        and output_final_state is True
        and safe_gate is True
        and lower_bound == -5.0
        and is_varlen
        and num_sequences > 0
        # Shape-only packed-average test: T / nseq >= 1024 without
        # reading cu_seqlens values or keying autotune directly on raw T.
        and tokens >= 1024 * num_sequences
    )


def _glm53_kda_prefill_runtime_contract(
    capability,
    vllm_config,
) -> tuple[tuple[int, int], str | None, int | None, int | None] | None:
    """Read drift-prone vLLM fields without turning drift into a boot failure."""
    try:
        if capability is None or vllm_config is None:
            return None
        ...
        return (
            ...
            getattr(vllm_config.parallel_config, "tensor_parallel_size", None),
            getattr(
...
def _glm53_kda_prefill_capture_runtime_contract(
    *, hidden_size: int, activation: str
    ...
    """Latch the exact GLM contract while model-init config is available.

    vLLM exposes its current config only inside ``set_current_vllm_config``;
    the global is restored before model forwards begin.  FusedRMSNormGated is
    constructed by the GLM KDA layer inside that initialization context, so
    capture once there and keep all forward-time dispatch shape-only.
    """
    global _GLM53_KDA_PREFILL_RUNTIME_CONTRACT
    if (
        not _GLM53_KDA_PREFILL_REGIME_ENABLED
        or _GLM53_KDA_PREFILL_RUNTIME_CONTRACT is not None
        or hidden_size != 128
    ...
    try:
        ...
        return
    contract = _glm53_kda_prefill_runtime_contract(capability, vllm_config)
    if contract == ((12, 1), "glm5_next_text", 4, 8192):
        _GLM53_KDA_PREFILL_RUNTIME_CONTRACT = contract


def _glm53_kda_prefill_autotune_regime(
    *,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    """Return stock/short=0 or exact-gated long packed prefill=1."""
    if not _GLM53_KDA_PREFILL_REGIME_ENABLED:
        return 0
    runtime_contract = _GLM53_KDA_PREFILL_RUNTIME_CONTRACT
    if runtime_contract is None:
        return 0
    if cu_seqlens is None:
        return 0
    num_sequences = cu_seqlens.numel() - 1
    if num_sequences <= 0 or q.shape[1] < 1024 * num_sequences:
        return 0
    capability_tuple, model_type, tensor_parallel_size, max_tokens = (
        runtime_contract
    )
    initial_shape = tuple(initial_state.shape) if initial_state is not None else None
    return int(
        _glm53_kda_prefill_regime_gate(
            enabled=_GLM53_KDA_PREFILL_REGIME_ENABLED,
            ...
            raw_g_shape=tuple(raw_g.shape),
            beta_shape=tuple(beta.shape),
            q_dtype=str(q.dtype),
            k_dtype=str(k.dtype),
            v_dtype=str(v.dtype),
            raw_g_dtype=str(raw_g.dtype),
            beta_dtype=str(beta.dtype),
            initial_state_shape=initial_shape,
            initial_state_dtype=(
                str(initial_state.dtype) if initial_state is not None else None
            ),
...
def chunk_kda_with_fused_gate_fwd(
    ...
    # Resolve the request's bucket exactly once. Every downstream core autotuner
    # receives this same scalar; no wrapper derives a bucket from its own grid.
    autotune_regime = _glm53_kda_prefill_autotune_regime(
        q=q,
        k=k,
        v=v,
        raw_g=raw_g,
