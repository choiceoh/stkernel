...
def fused_recurrent_gated_delta_rule_fwd_kernel(
    ...
    # deneb fork (vLLM #55736): per-token strides for the inputs.  The decode
    # caller hands this kernel column slices of a merged q|k|v conv output and
    # of the fused qkvbfg_a projection, which are token-strided but contiguous
    ...
    if USE_INITIAL_STATE:
        if IS_CONTINUOUS_BATCHING:
            if IS_SPEC_DECODING:
                # deneb fork (vLLM #51508): num_accepted_tokens is 0 for a
                # stale row whose sampled tokens were discarded, which would
                # index the request's state slots at -1.  The metadata builder
                # nulls such a row's slots (gdn_attn.py) so this kernel returns
                # below without reading or writing state; this clamp is the
                # in-kernel half of that contract.
                i_t = tl.maximum(
...
def fused_recurrent_gated_delta_rule_fwd(
    ...
    fused_recurrent_gated_delta_rule_fwd_kernel[grid](
        ...
        # This wrapper's callers are contiguous; these reproduce the addresses
        # the kernel computed before the stride parameters existed.
        stride_q_token=H * K,
        stride_k_token=H * K,
        stride_v_token=HV * V,
        stride_g_token=HV,
        stride_beta_token=HV * (V if beta.ndim == v.ndim else 1),
