...
# ---------------------------------------------------------------------------
# deneb fork: mhc_onepass_tilelang -- the decode pair (mhc_fused_tilelang +
# mhc_pre_big_fuse_with_norm_tilelang) in ONE launch. Grid = (num_tokens),
# one CTA per token, threads=n_thr; the single tile spans all n_out and
# split_k = 1, so one CTA holds everything the two-kernel pipeline passes
# through global memory (gemm_out_mul / gemm_out_sqrsum disappear).
#
# Phase map (line-by-line from the two stock kernels above):
#   phase 1   = mhc_fused_tilelang body with i_nt collapsed (tile_n = n_out)
#               and i_ks dropped (split_k = 1): post-map + prenorm FMA,
#               residual_out written, per-thread acc[n_out] + sqr.
#   reduce    = mhc_fused's s_warp cross-warp reduction (sync_threads barrier).
#   mixes     = big_fuse's split-sum replaced by the in-CTA reduction; every
#               thread recomputes mixes[] redundantly (same pattern as
#               big_fuse's redundant per-thread split-sum), then T.copy to
#               shared -- big_fuse's exact handoff.
#   warp 0    = big_fuse's <32 branch: post_mix_out, comb sinkhorn, verbatim.
#   all thr   = big_fuse's norm branch: pre_mix_shared, weighted sum over
#               residual_out (the post-mapped streams, read back through L2),
#               bf16 stash matching RMSNorm rounding, rsqrt, norm_weight.
#
# Contract (checked by the dispatcher before calling): n_out == hc*(hc+2)
# and n_out <= 32, hidden % n_thr == 0, norm_weight present.
@tilelang.jit(
    pass_configs=pass_configs,
)
def mhc_onepass_tilelang(
    comb_mix,
    residual_in,
    post_mix,
    x_in,
    ...
    """One launch: post-map + prenorm GEMM + mixes + sinkhorn + norm."""
    m = T.dynamic("num_tokens")
    h = hidden
    ...
    with T.Kernel(m, threads=n_thr) as i_n:
        ...
        # ---- phase 1: post-map + prenorm FMA (mhc_fused_tilelang, collapsed)
        s_post = T.alloc_shared((hc,), T.float32)
        s_comb = T.alloc_shared((hc, hc), T.float32)
        pm = T.alloc_local((hc,), T.float32)
        cm = T.alloc_local((hc, hc), T.float32)
        ...
        for it in T.serial(h_iters):
            ...
            for n in T.unroll(n_out):
                for j in T.unroll(hc):
                    acc[n] += weight_t[n, j, h_idx] * new_r[j]
        ...
        if tid < 32:
            # ---- post + comb sinkhorn: big_fuse's <32 branch, verbatim
        ...
        # ---- pre + norm: big_fuse's norm branch on all threads. pre_mix is
        # recomputed per thread from s_mixes exactly like big_fuse recomputes
        # it from mixes_shared in its >=32 branch.
