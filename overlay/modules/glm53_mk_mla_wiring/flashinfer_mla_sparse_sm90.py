...
# deneb fork (glm53_mk_mla_wiring): route the decode shapes to MK_SEG_MLA,
# the megakernel's own sparse MLA decode kernel (bf16 mma, fp32 acc -- the
...
def _mk_mla_route(impl, num_tokens: int) -> bool:
    ...
    if m is None or num_tokens <= 0 or num_tokens > _MK_MLA_MAX_T:
        return False
    if (impl.qk_rope_head_dim != 0 or impl.kv_lora_rank != m.MLA_D
            or impl.num_heads != m.MLA_H or not impl.use_fp8_kv_cache
            or impl.head_size != m.MLA_D):
        return False
    m.maybe_arm()
    return bool(m._ARMED.get("mla"))


def _sm90_wrapper_run(impl, q_nope, q_pe, kv_c_and_k_pe_cache, topk_slots, layer):
    """The stock tail of forward_mqa, callable on its own (shadow compare)."""
    state = _SM90_STATE
    ...
    ).reshape(-1, 1, impl.head_size)
    ckv = flat[..., : impl.kv_lora_rank]
    kpe = flat[..., impl.kv_lora_rank :]
    scale_kwargs = (
        {"ckv_scale": float(layer._k_scale_float or 1.0), "kpe_scale": 1.0}
        if impl.use_fp8_kv_cache
        else {}
    )
    return state.wrapper.run(q_nope, q_pe, ckv, kpe, **scale_kwargs)
...
class FlashInferMLASparseSM90Impl(SparseMLACommonImpl[FlashInferMLASparseMetadata]):
    ...
    def forward_mqa(
        ...
        # deneb fork (glm53_mk_mla_wiring): decode shapes -> MK_SEG_MLA
        if _mk_mla_route(self, num_tokens):
            mk_out = _mk_mla_run(self, q_nope, q_pe, kv_c_and_k_pe_cache,
                                 topk_slots, valid_counts, layer)
