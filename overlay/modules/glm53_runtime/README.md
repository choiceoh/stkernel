...
- GLM-5.3-Flash의 `reasoning_effort`는 `low`, `high`를 사용하며 `max`는 `high`로 정규화한다.
  최상위 필드와 `chat_template_kwargs`를 정규화한 뒤 값이 충돌하거나 지원하지 않는 값을 지정하면 400이다.
  `null`/생략은 서버 기본값이며, 이 저장소의 GLM 템플릿은 `high`를 기본으로 쓴다.
  `max`를 보내던 클라이언트도 오류 없이 `high`로 처리한다.
