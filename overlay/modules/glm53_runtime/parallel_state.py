...
_GLM53_PREFILL_SP = os.environ.get("VLLM_GLM53_PREFILL_SP") == "1"
...
class GroupCoordinator:
    ...
    def all_reduce(self, input_: torch.Tensor) -> torch.Tensor:
        ...
        # Defer the exact GLM eager prefill output reduction BEFORE entering
        # the custom op: vllm.all_reduce promises a fresh non-aliasing output.
        # A rank partial is the original tensor and may only be returned here.
        # Compiled/drafter callers retain the original traceable dispatch.
        if _GLM53_PREFILL_SP and not torch.compiler.is_compiling():
            from .device_communicators.glm53_prefill_collectives import (
                maybe_partial_all_reduce,
            )

            partial = maybe_partial_all_reduce(self.device_communicator, input_)
