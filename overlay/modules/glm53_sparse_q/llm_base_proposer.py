...
# deneb fork: 1 = return gathered q(token) from probabilistic draft sampling
# (sparse through the whole rejection pipeline); 0 = legacy dense probs.
...
class SpecDecodeBaseProposer:
    ...
    def _sample_from_logits(
        ...
        # deneb fork (gathered-q): the rejection ratio test reads the draft
        # distribution at exactly ONE index per position -- the sampled token
        # -- and nothing else in the accept path touches q. Gathering that
        # one float per row here means the dense [tokens, vocab] probs never
        # escapes this function: no [reqs, k, vocab] persistent cache, no
        # staging copies, and the recovery kernel drops its full-vocab q
        # sweep (it falls back to the target-only law this stack served
        # before probabilistic mode -- acceptance stays EXACT, only the
