...
class GDNAttentionMetadataBuilder(AttentionMetadataBuilder[GDNAttentionMetadata]):
    ...
    def build(  # type: ignore[override]
        ...
        if spec_sequence_masks is None:
            ...
            # deneb fork (vLLM #51508): a row can report 0 accepted tokens when
            # its sampled tokens were discarded (a stale async-scheduling step;
            # update_num_computed_tokens_for_batch_change writes
            # valid_sampled_token_count straight into num_accepted_tokens) while
            # its drafts were still scheduled. Every consumer indexes the
            # request's state slots with num_accepted_tokens - 1, so a 0 becomes
            # -1: an out-of-bounds load in the Triton kernels and, on the CPU
            # align-mode paths, a *valid* Python index that silently reads from
            # the end of the state. Such a row has no valid spec state to resume
            # from and its recurrent state must not advance this step: null its
            # slots so the kernels skip both the initial-state read and the
            # final-state write, and clamp the count so the slot index stays in
            # bounds.
