...
def get_conv_copy_spec(
    ...
    # deneb fork (vLLM #51508): num_accepted_tokens is 0 for a stale row whose
    # sampled tokens were discarded. -1 is a *valid* Python index here, so an
    # unclamped offset slices from the end of the state with no error at all.
...
def get_temporal_copy_spec(
    ...
    # deneb fork (vLLM #51508): see get_conv_copy_spec -- a 0 count would pick
    # a block from the end of the list instead of raising.
