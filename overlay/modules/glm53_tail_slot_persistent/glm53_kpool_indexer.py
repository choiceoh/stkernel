...
def compute_kpool_tail_slot_mapping(
    ...
    Pure torch (no Triton, no device sync).

    ``out`` must be supplied whenever this metadata can be read from inside a
    captured cudagraph. The original code returned a fresh ``clone()`` each
...
class KpoolTailMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(
        ...
        # One buffer for the life of the builder. The rebuilt mapping is read
    ...
    def build(
        ...
        if positions is not None:
            ...
            slot_mapping = compute_kpool_tail_slot_mapping(
                ...
                out=self._ensure_tail_slot_buf(slot_mapping),
            )
