...
# --- Deneb: keep the data-dependent PLE lookup out of dynamo -----------------
# forward_impl reads query_start_loc.numel() into Python and derives every
# n-gram index from it; dynamo guards that symbolic size and the guard breaks
# during vLLM's memory-profiling run. Excluding just this function lets the rest
# of the model compile. Needs cudagraph_mode=PIECEWISE (this adds a graph break).
_DENEB_PLE_NOCOMPILE = os.environ.get("DENEB_PLE_NOCOMPILE", "1") == "1"

try:
    from torch._dynamo import disable as _dynamo_disable
except Exception:  # pragma: no cover - torch without dynamo
    def _dynamo_disable(fn=None, **_kw):
        return fn if fn is not None else (lambda f: f)


def _deneb_no_compile(fn):
    """Mark fn as a dynamo boundary when DENEB_PLE_NOCOMPILE is on."""
    if not _DENEB_PLE_NOCOMPILE:
        return fn
    return _dynamo_disable(fn)
# --- end Deneb ---------------------------------------------------------------


# --- Deneb: NVMe-backed PLE lookup ------------------------------------------
_DENEB_PLE_NVME = os.environ.get("DENEB_PLE_NVME", "0") == "1"
_DENEB_PLE_NVME_VERIFY = os.environ.get("DENEB_PLE_NVME_VERIFY", "0") == "1"
_DENEB_PLE_MODEL_DIR = os.environ.get(
    "DENEB_PLE_MODEL_DIR", "/home/choiceoh/models/qwen38-flash-next-nvfp4"
)

_deneb_store = None


def _deneb_row_store():
    """Process-wide row store; opening 10 fds per layer would be wasteful."""
    global _deneb_store
    if _deneb_store is None:
        import sys

        sys.path.insert(0, "/home/choiceoh/q38-overlay")
        from ple_store4 import PleRowStore

        _deneb_store = PleRowStore(_DENEB_PLE_MODEL_DIR)
    return _deneb_store


def _deneb_nvme_gather(ngram_ids, out_2d, staging_holder):
    """out_2d[i] <- table[ngram_ids_flat[i]] , reading rows from NVMe.

    staging_holder is a one-element list owned by the calling layer so the
    pinned buffer survives across steps instead of being reallocated.
    """
    import numpy as np
    ...
    buf = staging_holder[0]
    if buf is None or buf.shape[0] < n:
        buf = torch.empty((n, rows), dtype=torch.uint8, pin_memory=True)
        staging_holder[0] = buf
    view = buf[:n]

    # One D2H for the ids (they are data-dependent, so this sync is unavoidable).
    ids_np = flat.to("cpu", non_blocking=False).numpy().astype(np.int64, copy=False)
    _deneb_row_store().gather(ids_np, view.numpy())
    # One H2D for the rows.
    out_2d.copy_(view.view(out_2d.dtype), non_blocking=True)
# --- end Deneb ---------------------------------------------------------------
...
def _get_ple_embedding_quant_method(
    ...
    # DENEB TP=4 EXPERIMENT (2026-08-27, opt-in via DENEB_PLE_FORCE_FP8_EMBED=1).
    #
    # Upstream only builds the FP8 embedding method for an Fp8Config checkpoint.
    # The NVFP4 build of Qwen3.8-Flash-Next excludes "*.ple.*" from NVFP4 and
    # ships the PLE table separately FP8-scaled, so its top-level config is
    # NVFP4 and this returns None -> a plain VocabParallelEmbedding with no
    # weight_scale -> "no module or parameter named ngram_embedding.weight_scale".
    #
    # That only bites when VLLM_PLE_CPU_OFFLOAD is off, and the offload path is
    # hard-gated to nnodes=1 (gpu_worker._validate_ple_offload_config) — which
    # is exactly the TP>1-across-Sparks case, since a Spark has one GPU. The
    ...
    # The ignored-layers interrogation below is Fp8Config-shaped
    # (ModelOptNvFp4Config has no .ignored_layers). Under the forced path the
    # question it answers is already settled: the NVFP4 checkpoint EXCLUDES
    # "*.ple.*" from NVFP4 and ships the PLE separately FP8-scaled, so the PLE
    # being outside the top-level quant scheme is the very reason we are here.
    if not isinstance(quant_config, Fp8Config):
...
class Qwen3_8FlashNextNGramEmbedding(PleOffloadLayer):
    ...
    def forward_impl(  # type: ignore[override]
        ...
        if is_offload_process():
            ...
            # DENEB PATCH (2026-08-28): bound the hash-chain width by the actual
            # token count instead of the static maximum (max_num_batched_tokens,
            # 16384 here). Everything after this point — the EOS fill, the
            # concat, both shift passes, the XOR mixing, and the [reqs, width,
            # heads] remainder broadcast — runs over this width, while the final
            # gather reads only each request's current columns. On a decode step
            # a request contributes 1+num_speculative_tokens columns, so the
            # static width does ~4000x the needed work and materializes
            # multi-MB intermediates per step. num_tokens is a Python int fixed
            # per CUDA-graph capture size, so shapes stay static within every
            # captured graph: decode graphs shrink to width<=capture size,
            # prefill keeps the full width. columns is already clamped to
            # packed.shape[1]-1, and adjusted_columns' maximum
            # (num_tokens-1 + ngram_size-1) equals the new context width - 1.
            max_seq_len = min(self.padded_buffer.shape[1], num_tokens)
        ...
        if output_buffer is not None:
            ...
            _out2d = output.reshape(-1, self.head_dim)
            if _DENEB_PLE_NVME:
                if not hasattr(self, "_deneb_staging"):
                    self._deneb_staging = [None]
                if _DENEB_PLE_NVME_VERIFY:
                    _ref = torch.empty_like(_out2d)
                    torch.index_select(
                        self.ngram_embedding.weight, 0,
                        ngram_ids.reshape(-1), out=_ref,
                    )
                _deneb_nvme_gather(ngram_ids, _out2d, self._deneb_staging)
                if _DENEB_PLE_NVME_VERIFY:
                    if not torch.equal(_ref.view(torch.uint8), _out2d.view(torch.uint8)):
                        raise RuntimeError(
                            "DENEB PLE NVMe gather disagrees with the resident "
                            "table -- the row mapping is wrong"
                        )
            else:
                torch.index_select(
