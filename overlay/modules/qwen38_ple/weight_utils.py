...
def fastsafetensors_weights_iterator(
    ...
    # DENEB PATCH (2026-08-28, opt-in via DENEB_FST_LOCAL=1).
    #
    # With distributed initialized this passes the WORLD group, so
    # fastsafetensors partitions the file reads across ranks and redistributes
    # tensors over its own NCCL context. That design assumes single-node
    # multi-GPU; on a 4-node RoCE TP it (a) ships weights across the network
    # that every node already has on local NVMe, and (b) dies in
    # ncclSystemError inside parallel_loader.iterate_weights. SingleGroup makes
    # every rank read locally with direct-IO — semantically identical to the
    # default loader (vLLM's weight_loader still does the TP slicing), just
    # faster file IO and no loader-side collectives.
    if os.environ.get("DENEB_FST_LOCAL", "") == "1":
        pg = SingleGroup()
    ...
    # DENEB (FST_LOCAL): load to HOST memory, not the device. With device=cuda
    # the pipelined loader keeps several shards resident on the GPU at once,
    # and this checkpoint's tail is ~10 PLE FP8 shards of ~4.7 GB each — on
    # unified memory that OOM-killed the head at shard 196/206 (kernel
    # oom-kill, shmem-rss 39 GB). CPU tensors keep the direct-IO speed win;
    # vLLM's weight_loader copies each rank's slice to the GPU as it consumes.
    if os.environ.get("DENEB_FST_LOCAL", "") == "1":
    ...
    nogds = pg.size() > 1
    # DENEB (FST_LOCAL): the SingleGroup override above shrinks pg to size 1,
    # which flips this heuristic to nogds=False — and GB10 has no GPUDirect
    # Storage, so the GDS copier then kills the worker natively (no Python
    # stack; only "GDS is not supported in this platform" in the log). Local
    # mode always means plain direct-IO, never GDS.
    if os.environ.get("DENEB_FST_LOCAL", "") == "1":
        nogds = True
