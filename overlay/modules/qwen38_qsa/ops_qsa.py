...
def qsa_sparse_paged_attention(
    ...
    #
    # DENEB PATCH (2026-08-28): those split counts assume a GB300-class SM
    # budget (~180 SMs). On GB10 (sm_121, 48 SMs) the decode branch's 64-way
    # split-K yields hundreds of tiny programs plus a 64-partial FP32 merge,
    # where the merge overhead can exceed the parallelism it buys. The env var
    # caps target_splits so the profile can be tuned per device without
    # touching the kernel; unset keeps upstream behavior exactly.
    _split_cap = int(os.environ.get("DENEB_QSA_MAX_SPLITS", "0") or 0)
    ...
    if _split_cap > 0:
        target_splits = min(target_splits, _split_cap)
