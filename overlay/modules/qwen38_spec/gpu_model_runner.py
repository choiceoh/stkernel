...
class GPUModelRunner(
    ...
    def execute_model(
        ...
        with (
            ...
            # DENEB PATCH (2026-08-28) — reported on vLLM PR #53896, unfixed there.
            #
            # _prepare_ngram_context reads the CPU token mirror
            # (input_batch.token_ids_cpu) to build the PLE n-gram context, but
            # under async scheduling that mirror still holds the optimistic -1
            # placeholders async spec decode writes before acceptance is known.
            # The mirror is only repaired when deferred_state_corrections_fn
            # runs, which normally happens AFTER the n-gram context is built —
            # so every decode step feeds the embedding -1 (a sentinel) or 0 (a
            # VALID token id from never-written buffer rows, so it fails
            # silently). The PR's reviewer measured 154 decode steps with zero
            # correct contexts, and --no-async-scheduling made 16/16 runs
            # bit-identical.
            #
            # The mamba path already does exactly this, for the same reason,
            # a few hundred lines below ("preprocess_mamba reads
            # req_state.num_computed_tokens (CPU) ... apply deferred
            # corrections before it runs"). This is that fix for the n-gram
            # reader, so async scheduling and MTP can be used together.
            if (
                getattr(self, "uses_ngram_embedding", False)
                and deferred_state_corrections_fn
            ):
