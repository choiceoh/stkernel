...
class SpecDecodeBaseProposer:
    ...
    # ---- DENEB adaptive speculative length (opt-in) -------------------
    # K per step from an EMA of the accepted draft fraction. The rejected
    # count arrives one step late through a pinned async copy, so no GPU
    # sync is added. num_speculative_tokens is swapped for the duration of
    # one propose() call and restored in finally, so every internal loop
    # and view sees a consistent K; the config maximum (slot reservation,
    # init-time buffers) is untouched.
    _DENEB_ADAPTIVE = os.environ.get("DENEB_ADAPTIVE_SPEC", "") == "1"
    _DENEB_K_HI = int(os.environ.get("DENEB_SPEC_K_HI", "8"))
    _DENEB_K_MID = int(os.environ.get("DENEB_SPEC_K_MID", "3"))
    _DENEB_K_LO = int(os.environ.get("DENEB_SPEC_K_LO", "2"))

    def _deneb_pick_k(self, num_rejected_tokens_gpu):
        st = getattr(self, "_deneb_spec_state", None)
        if st is None:
            st = self._deneb_spec_state = {
                "ema": 0.5,
                "prev_k": self.num_speculative_tokens,
                "buf": torch.zeros(1, dtype=torch.float32, pin_memory=True),
                "pending": False,
            }
        if st["pending"]:
            rejected = float(st["buf"][0])
            frac = 1.0 - rejected / max(1, st["prev_k"])
            st["ema"] = 0.6 * st["ema"] + 0.4 * max(0.0, min(1.0, frac))
            st["pending"] = False
        if (num_rejected_tokens_gpu is not None
                and num_rejected_tokens_gpu.numel() > 0):
            st["buf"].copy_(
                num_rejected_tokens_gpu.detach().float().mean().reshape(1),
                non_blocking=True,
            )
            st["pending"] = True
        if st["ema"] >= 0.8:
            ...
            k = self._DENEB_K_LO
        k = max(1, min(k, self.num_speculative_tokens))
        st["prev_k"] = k
        return k

    def propose(self, *args, **kwargs):
        if not self._DENEB_ADAPTIVE:
            return self._deneb_propose_inner(*args, **kwargs)
        k_max = self.num_speculative_tokens
        self.num_speculative_tokens = self._deneb_pick_k(
            kwargs.get("num_rejected_tokens_gpu")
        )
        try:
            return self._deneb_propose_inner(*args, **kwargs)
        finally:
            self.num_speculative_tokens = k_max

    def _deneb_propose_inner(
        self,
