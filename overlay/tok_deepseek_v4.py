        ...
        def apply_chat_template(
            ...
            **kwargs,
            ...
            if tools is not None and len(tools) > 0:
                # The checkpoint's reference encoding (model repo encoding/,
                # test_input_1) carries tools on the conversation's leading
                # system message: BOS + system content + "\n\n## Tools". A
                # separate carrier message would flip that order for every
                # request that brings its own system prompt.
                if messages and messages[0].get("role") == "system":
                    messages[0] = {**messages[0], "tools": tools}  # type: ignore[typeddict-unknown-key]
                else:
            ...
            if not isinstance(reasoning_effort, str):
                ...
                # minimal/low/medium and unknown values map to the reference
                # default "low" (renders nothing); "high"/"max" select the
                # reference reasoning-effort preambles in the encoding.
                reasoning_effort = None
