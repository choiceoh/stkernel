...
class DeepSeekV32ToolParser(ToolParser):
    ...
    def _add_tool_call_delta(
        ...
        *,
        ...
        if index not in tool_call_deltas:
            # Only pass fields that carry data: an explicit None still marks
            # the field as set, and the exclude_unset stream serialisation
            # then emits "id"/"type"/"name": null on every continuation
            # chunk — keys the OpenAI wire format omits entirely.
            ...
            if call_id is not None:
                tool_call_kwargs["id"] = call_id
            if call_type is not None:
                tool_call_kwargs["type"] = call_type
            tool_call_deltas[index] = DeltaToolCall(**tool_call_kwargs)
            return
    ...
    def extract_tool_calls_streaming(
        ...
        if content_parts or tool_call_deltas:
            # Pass only populated fields: content=None marks the field set
            # and serialises as "content": null on tool-call chunks, and an
            # explicit empty tool_calls list relies on the DeltaMessage
            # serializer to strip it. Neither key exists on the OpenAI wire.
            delta_kwargs: dict[str, Any] = {}
            if content_parts:
                delta_kwargs["content"] = "".join(content_parts)
            if tool_call_deltas:
                delta_kwargs["tool_calls"] = list(tool_call_deltas.values())
            return DeltaMessage(**delta_kwargs)
