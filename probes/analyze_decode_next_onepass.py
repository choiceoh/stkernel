...
def summarize(root, candidate, baseline, *, canonical=False):
    ...
    for mode, name in (("candidate", candidate), ("baseline", baseline)):
        ...
        for host in HOSTS:
            def verify_host():
                ...
                issues = runtime_proof.validate_report(before, expected) + runtime_proof.validate_report(after, expected)
                issues += runtime_proof.compare_snapshots(before, after)
    ...
    check("matched runtime arms", lambda: require(not (issues := runtime_proof.compare_arms(snapshots.get("baseline"), snapshots.get("candidate"))), "; ".join(issues)))
