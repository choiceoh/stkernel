#!/usr/bin/env python3
"""Summarize the integrated A-B onepass evidence; windows are not independent boots."""
import argparse
import json
import math
...
def summarize(root):
    ...
    assert [r['name'] for r in records] == ['MOERFA1', 'MOERFB1']
    assert len({r['boot_id'] for r in records}) == 2
    ...
    for record in records:
        name = record['name']
        mode = 't,r' if name == 'MOERFA1' else 't'
    ...
    a,b = rows
    base = {key:b[key] for key in ('step_s','output_tok_s','median_step_s')}
    prefill = []
