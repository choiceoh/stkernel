...
def main():
    ...
    ap.add_argument("--arm", choices=("stock", "n128", "stream"), required=True)
    ap.add_argument("--layout", choices=("rowmajor", "tiled"), required=True)
    ap.add_argument("--m", type=int, default=6912)
    ap.add_argument("--output", type=Path, required=True)
    args = ap.parse_args()
    if args.arm == "n128" and args.layout == "tiled":
        ap.error("the previous N128 candidate does not support tiled weights")
    ...
    os.environ.update(CUTE_DSL_ARCH="sm_121a", CUTE_DSL_KEEP="ptx,cubin",
                      ...
                      VLLM_GLM53_B12X_PREFILL_FC1_N128=str(int(args.arm == "n128")),
                      VLLM_GLM53_B12X_PREFILL_STREAM_FC2=str(int(args.arm == "stream")),
    ...
    md._get_dynamic_kernel(
        288, args.m, 4096, 512, 8, args.m,
        activation="swigluoai_uninterleave", swiglu_alpha=1.0,
        swiglu_beta=0.0, swiglu_limit=10.0, tiled=args.layout == "tiled")
    keys = list(md._DYNAMIC_KERNEL_CACHE)
    assert len(keys) == 1, keys
    tag = {"stream": "glm53_prefill_stream_fc2_v1",
           "n128": "glm53_prefill_fc1_n128_v1"}.get(args.arm)
    assert (keys[0][-1] == tag) if tag else not any("prefill_" in str(x) for x in keys[0])
    ...
    evidence = dict(arm=args.arm, layout=args.layout, m=args.m,
                    ...
                    sources={str(p): hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in map(Path, md._kernel_source_files())})
    (args.output / "result.json").write_text(json.dumps(evidence, indent=2, default=str) + "\n")
    print(f"COMPILE PASS arm={args.arm} layout={args.layout} m={args.m}", flush=True)
