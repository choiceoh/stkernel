...
def main():
    ...
    from engine.kernels.dense.mxfp8 import _quantize, _pack_weights
    from engine.kernels.prefill_collectives.consumer import _quantize_gather_mx
    started = time.monotonic()
    native = _build()
    kernels = []
    variants = [
        (f'quantize-k{k}-tiled{int(tiled)}', _quantize, dict(X='*bf16', Q='*fp8e4nv', S='*i32', M='i32'), dict(K=k, G=k//128, TILED=tiled))
        for k in (128, 384, 4096, 20480) for tiled in (False, True)]
    variants += [('weight-scales', _pack_weights, dict(S='*fp32', Out='*i32'), {}),
                 ('packet-mx', _quantize_gather_mx,
                  dict(Packed='*fp8e4nv', Scales='*fp32', Q='*fp8e4nv', S='*i32', M='i32',
                       LOCAL_N='i32', PAYLOAD_BYTES='i32'), dict(K=4096, G=32, PACK_BLOCK=2048, TILED=True))]
    for name, fn, signature, constants in variants:
        ...
        kernels.append(dict(name=name, shared_bytes=kernel.metadata.shared,
