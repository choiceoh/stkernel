...
# -- U13: GDN prefill ------------------------------------------------------------------------------------------------------
def l2norm_heads(x):
    """q or k normalised per head over its width in fp32, back in the input dtype: what the served kernel does inside
    (use_qk_l2norm_in_kernel) and what the image's FlashInfer does not -- flashinfer#5255 (open, 2026-09-17): the native
    prefill paths ignore the flag, and unnormalised q/k make the recurrence grow to NaN (sm121-gdndiag-0919d: every
    element of every variant)."""
    import torch
    f = x.float()
    return (f * torch.rsqrt((f * f).sum(-1, keepdim=True) + 1e-6)).to(x.dtype)


def flashinfer_gdn_args(args, *, normalize=False) -> dict:
    ...
    q, k = args["q"][0], args["k"][0]
    if normalize:
        q, k = l2norm_heads(q), l2norm_heads(k)
    return dict(q=q.contiguous(), k=k.contiguous(), v=args["v"][0].contiguous(),
                ...
                use_qk_l2norm_in_kernel=not normalize, output_state=torch.empty_like(initial))
...
def run_gdn(output=None) -> dict:
    ...
    with torch.inference_mode():
        for t in GDN_TOKENS:
            ...
            if chunk_gated_delta_rule is not None:
                try:
                    fn = flashinfer_gdn_args(args, normalize=True)
                    o_n, st_n = chunk_gated_delta_rule(**fn)
                    st_n = st_n[0]
                    row["normalized_first"] = _time(lambda: chunk_gated_delta_rule(**flashinfer_gdn_args(args, normalize=True)))
                    row["normalized_first_kernel_only"] = _time(lambda: chunk_gated_delta_rule(**fn))
                    row["normalized_vs_served"] = {"o": _error(o_n, o_s), "state": _error(st_n, st_s),
                                                   "nan": int(torch.isnan(o_n).sum())}
                    if t in ORACLE_TOKENS:
                        o_r, st_r = gdn_oracle(args)
                        row["normalized_vs_oracle"] = {"o": _error(o_n, o_r), "state": _error(st_n, st_r)}
                    row["normalized_speedup"] = round(row["served"]["median_us"] / row["normalized_first"]["median_us"], 3)
                except Exception as exc:                            # noqa: BLE001
                    report["unavailable"][f"flashinfer normalized {t}"] = f"{type(exc).__name__}: {exc}"[:300]
