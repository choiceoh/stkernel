...
def run(output=None):
    ...
    paths = ('engine/kernels/common/vocab_merge.py', 'engine/kernels/common/vocab_candidates.py',
             'engine/modules/vocab.py', 'engine/profiles/glm53/drafter.py',
             'engine/kernels/draft_sample.py', 'tests/test_engine_vocab_merge.py',
    ...
    report['default_compact'] = compact_supported(14, 154880, 64, 16, 'cuda')
    if not report['default_compact']:
        raise AssertionError('qualified GB10 runtime must select the compact serving default')
