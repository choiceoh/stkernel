...
# ---------------------------------------------------------------------------
# deneb probe (measurement build only): stage-level CUDA-event timing for the
# sparse-MLA prefill path. Env-gated (DENEB_ATTN_PROF=1); flushes in batches to
# keep synchronization off the steady-state path; never records during graph
# capture.
# ---------------------------------------------------------------------------
...
_DENEB_STAGES: dict[str, dict] = {}


def _deneb_stage(name: str) -> dict:
    b = _DENEB_STAGES.get(name)
    if b is None:
        b = {"ms": 0.0, "n": 0, "pending": []}
        _DENEB_STAGES[name] = b
    return b


def _deneb_mark() -> "torch.cuda.Event | None":
...
def _deneb_span(name: str, ev0, ev1) -> None:
    if ev0 is None or ev1 is None:
        return
    b = _deneb_stage(name)
    b["pending"].append((ev0, ev1))
    if len(b["pending"]) >= _DENEB_PROF_FLUSH:
        torch.cuda.synchronize()
        for e0, e1 in b["pending"]:
            b["ms"] += e0.elapsed_time(e1)
            b["n"] += 1
        b["pending"].clear()
        _deneb_logger.info(
            "[deneb-fi] %s calls=%d total=%.1fms avg=%.3fms",
...
class DeepseekV4FlashInferMLAAttention(DeepseekV4Attention):
    ...
    def _build_sparse_index_metadata(
        ...
        # deneb fork: port of upstream PR #51042. Under DSpark the decode SWA
        # buffer is noncausal_index_width wide, not window_size wide; take the
        # row stride from the metadata that allocated it.
        ...
        if cached_sparse is None:
            sparse_indices, sparse_topk_lens = build_flashinfer_mixed_sparse_indices(
                ...
                swa_metadata.decode_swa_width,  # deneb fork: PR #51042
                self.compress_ratio,
                top_k,
...
class DeepseekV4FlashInferSM120Attention(DeepseekV4Attention):
    ...
    def _forward_prefill(
        ...
        _ev1 = _deneb_mark()
        _deneb_span("pf.topk_globalize", _ev0, _ev1)
        q = self._prepare_query(q, output)
        _ev2 = _deneb_mark()
        ...
        _ev3 = _deneb_mark()
        _deneb_span("pf.kernel_loop", _ev2, _ev3)
