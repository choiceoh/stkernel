...
and captures use identical model inputs, with distinct request cache salts.
...
def main():
    ...
    # This image exposes cache resets only in server dev mode. Its supported
    # request-level salt isolates cache identity without changing model tokens.
    record["cache_salt"] = f"prefill-attribution:{args.name}:{time.time_ns()}"
    ...
    def instrumented_open(request, *a, **kw):
        ...
        if url == base + "/v1/chat/completions":
            original = request.data
            payload = json.loads(original)
            payload['cache_salt'] = record['cache_salt']
            wire = json.dumps(payload).encode()
            record['unsalted_request_sha256'] = hashlib.sha256(original).hexdigest()
            record['wire_request_sha256'] = hashlib.sha256(wire).hexdigest()
