...
# ---------------------------------------------------------------------------
# deneb probe (measurement build only): CUDA-event timing of the MoE custom-op
# bodies. These are the ONLY per-layer python that still executes at runtime
# under AOT compile (opaque custom ops), so this is where the FFN share of the
# step is observable. Env-gated; batched flush; never records during capture.
# ---------------------------------------------------------------------------
...
def _deneb_span_close(handle, rows: int) -> None:
    if handle is None:
        return
    name, ev0 = handle
    ...
    if len(b["pending"]) >= _DENEB_PROF_FLUSH:
        ...
        b["pending"].clear()
        logger.info(
            "[deneb-moe] %s calls=%d rows=%d total=%.1fms avg=%.3fms",
            name, b["n"], b["rows"], b["ms"], b["ms"] / b["n"],
        )
...
def _moe_forward(
    ...
    _h = _deneb_span_open(
        "moe.prefill" if hidden_states.shape[0] > 64 else "moe.decode"
    )
    ...
    _deneb_span_close(_h, hidden_states.shape[0])
    return out
