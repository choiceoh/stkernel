#!/usr/bin/env python3
"""Integrated decode tile reform vs same-build t and independent stock kernels.
...
def main():
    ...
    ap.add_argument('--sanitizer-smoke', action='store_true', help='M6/U40 graph mutation and zeroing only')
    ...
    info = dict(variant='t,r', source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),
                files=[str(source)], tile_m=16, fc1_n=128, fc1_k=256, fc2_n=256)
    ...
    def select(arm):
        ...
        if arm == 'stock':
            ...
            cfg = md._parse_glm53_static_v2('t,r' if arm == 'candidate' else 't', probe=True)
            md._STATIC_V2_OVERRIDE = cfg

    # M=1/2 force the static path for correctness; timing targets M=6/8.
    cases = ((1,8), (2,8), (2,16), (6,8), (6,40), (6,48), (8,8), (8,40), (8,64), (16,8), (16,40), (32,8), (32,40))
    for tokens, unique in (((6,40),) if args.sanitizer_smoke else cases):
        fixture.T = tokens
    ...
    print('VERDICT: PASS (integrated MoE GPU correctness/replay; serving follows)',flush=True)
    return 0
