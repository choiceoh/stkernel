...
collect() {
  ...
  python3 "$REPO/probes/moe_warp_runtime_proof.py" "$mode" "$v4_sha" "$dispatch_sha" > "$out/$prefix-srv2.json" || status=1
  for node in 1 3 4; do
    ssh -o BatchMode=yes "choiceoh@10.10.10.$node" python3 - "$mode" "$v4_sha" "$dispatch_sha" \
      < "$REPO/probes/moe_warp_runtime_proof.py" > "$out/$prefix-srv$node.json" || status=1
  done
