...
def snapshot_errors(mode, expected, head_before, head_after, ranks, phase,
                    record_before=None, record_after=None, prepared=None):
    ...
    if (not head_before or not head_after or not head_before.get("boot_id")
            or head_before != head_after or mode_from_metadata(head_before) != mode):
    ...
    for host in HOSTS:
        ...
        errors.extend(host + ": " + error for error in proof.validate_report(report, expected))
        ...
        if prepared:
            errors.extend(host + ": " + error for error in
                          proof.compare_snapshots(prepared.get(host), report))
    if phase == "prepared":
...
class Reader:
    ...
    def __init__(self, repo, port, session, fleet_dir):
...
report = ns["collect_report"](MODE, EXPECTED)
...
errors = ns["validate_report"](report, EXPECTED)
...
class Observer:
    def __init__(self, out, candidate, baseline, revision, expected, reader,
        ...
        self.state = dict(schema=1, status="RUNNING", source_commit=revision,
                          candidate=candidate, baseline=baseline, errors=[], arms={})
...
def main(argv=None):
    ...
    try:
        ...
        print("READY " + json.dumps(dict(out=str(args.out), source_commit=revision, targets=len(expected), session=session,
                                         candidate=args.candidate, baseline=args.baseline, port=args.port)), flush=True)
