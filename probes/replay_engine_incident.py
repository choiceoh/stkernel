...
def main():
    ...
    parser.add_argument('--diagnostic-mode', type=int, choices=tuple(range(10)), default=0,
                        help='Private diagnostic arm only: 1 host block, 2 host target, 3 host token rejection, 4 device reference, 5 single-position target, 6 device target; 7 FP8 dense scalar, 8 BF16 transport scalar, 9 both')
    ...
    if args.diagnostic_mode in (1, 2, 3, 5, 7, 8, 9) and delta['st:async_decode_steps_total'] != 0:
        raise SystemExit('host control was bypassed by asynchronous decoding')
    if args.diagnostic_mode in (2, 5, 6, 7, 8, 9) and delta['vllm:spec_decode_num_accepted_tokens_total'] != 0:
    ...
    if args.diagnostic_mode in (5, 7, 8, 9) and delta['vllm:spec_decode_num_draft_tokens_total'] != 0:
