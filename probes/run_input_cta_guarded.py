...
def main():
    ...
    try:
        ...
        script='gemm_input_cta3.py' if args.three_slice else 'gemm_input_cta_next.py'
        kernel='mk_gemm_input_cta3_kernel' if args.three_slice else 'mk_gemm_input_next_kernel'
        ...
        for tool in ('probe','racecheck','memcheck'):
            assert not issues,issues
            command=(common+['--entrypoint','python3',IMAGE,f'/repo/probes/{script}'] if tool=='probe'
                     else common+['--entrypoint','/san/compute-sanitizer',IMAGE,'--tool',tool,
            ...
            assert rc==0,(tool,rc)
            assert json.loads((out/('result.json' if tool=='probe' else tool+'.json')).read_text())['status']=='PASS'
    ...
    assert not issues,issues
    print('PASS numerical checks, graphs, sanitizers and continuous resource guard')
