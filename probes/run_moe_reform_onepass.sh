...
[[ $resume_supervisor =~ ^[0-9]+$ ]] || exit 2
[[ ${MOE_RESUME_RESTORE:-0} == 0 ]] || { echo "session recovery resume is disabled"; exit 2; }
cleanup() {
  local rc=$?
  trap - EXIT INT TERM
...
# Complete both arms in the same hold; the central idle controller owns recovery.
