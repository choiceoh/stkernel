...
cd "$(dirname "$0")/.."
export REPO=$PWD
RESTORE_REPO=/home/choiceoh/stkernel-moe-onepass-restore-20260908
out=${MOE_ONEPASS_OUT:-/home/choiceoh/glm53-logs/MOEONEPASS0908}
...
cleanup() {
  local rc=$?
  trap - EXIT INT TERM
  docker stop -t 2 "moedirect-$session" >/dev/null 2>&1 || true
  if [[ $touched == 1 ]]; then
    if (
      cd "$RESTORE_REPO"
...
python3 probes/run_moe_direct_guarded.py --maintenance --variants native --out "$out/numerics"
bash launchers/deploy-overlays.sh glm53 > "$out/deploy.log" 2>&1
export MOE_ONEPASS_OUT=$out
export FLEET=/home/choiceoh/stkernel/bench/fleet.sh LEVER=$REPO/probes/moe_warp_lever.sh
...
bash bench/chain.sh 'MOEWSB1=' 'MOEWSA1=VLLM_GLM53_B12X_STATIC_V2=t,ws' 'MOEWSB2='
