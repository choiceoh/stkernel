...
| `glm53_moe` | **묶음(34차)**: b12x 공유 워크스페이스·EP 마이크로커널 레인·직접 출력 (옛 `b12x_shared_workspace`·`b12x_zero_weight_micro`·`glm53_b12x_out`) + 정적(디코드) MoE 커널 v2/v3/v4/v5(35·38·39차, `moe_static_kernel_v2.py`·`moe_static_kernel_v3.py`·`moe_static_kernel_v4.py`·`moe_static_kernel_v5.py`(타일 우선 가중치 `t`) + 그 배치를 읽는 gated 동적(프리필) 커널 오버레이 `moe_dynamic_gated.py`, 프로필 기본값 `u` = v4) | 9 | — | · | ● | · |
