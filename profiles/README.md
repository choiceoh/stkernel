...
| | | | | | | |
| `glm53_model` | **묶음(34차)**: 모델·어텐션·KDA·MLA 파일 접수 + 밀집 GEMM fp8/W4 패스 + KDA 원패스 + 순수 프리필 SP/NVFP4 후보 + 영상 자리표시 수정(39차) | 12 | 일부 | · | ● | · |
...
| `glm53_runtime` | **묶음(34차)**: prep-fused, 드래프터 학습 덤프, 샘플러 가드, 부팅 스탬프, 개발 랩, one-shot AR 배선 및 순수 프리필 collectives, 디코드 우선 스케줄러(39차, 기본값 `DECODE_FIRST=1` v3.2 순차 모드(디코더 뒤 대기 ≤20 s, 그 뒤 번갈아)), 채팅 옵션 검증 | 12 | 일부 | · | ● | · |
