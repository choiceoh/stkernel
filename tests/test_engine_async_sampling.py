...
class SpeculativePickBatchTests(unittest.TestCase):
    def test_identical_distributions_accept_every_draft_and_draw_the_bonus_from_the_target(self):
        ...
        accepted, tokens, count = speculative_pick_batch(probs, drafts, probs[:, :K], g)
    ...
    def test_disjoint_supports_reject_the_first_draft_and_recover_from_the_target(self):
        ...
        accepted, tokens, count = speculative_pick_batch(target, drafts, draft, g)
    ...
    def test_the_same_seed_gives_the_same_picks_on_every_rank(self):
        V, K, n = 64, 5, 4
        ...
        a = speculative_pick_batch(probs, drafts, draft, torch.Generator().manual_seed(9))
        b = speculative_pick_batch(probs, drafts, draft, torch.Generator().manual_seed(9))
        for x, y in zip(a, b):
