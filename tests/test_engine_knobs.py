...
class KnobDeclarationTests(unittest.TestCase):
    ...
    def test_execution_experiments_are_off_and_production_refuses_overrides(self):
        ...
        for production in (False, True):
            ...
            self.assertEqual([cfg[k] for k in ("execution_overlap", "early_observe", "prefill_tiles", "direct_mhc", "prefill_project_tiles", "nvme_mapped_staging")], [0, 0, 1, 0, 0, 0])
        for key in ("execution_overlap", "early_observe", "prefill_tiles", "direct_mhc", "prefill_project_tiles", "nvme_mapped_staging"):
