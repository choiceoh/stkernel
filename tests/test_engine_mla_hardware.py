...
class MlaHardwareTests(unittest.TestCase):
    ...
    def test_retained_queries_are_bound_to_ordinary_k7_cells(self):
        ...
        self.mla._ensure_workspace = lambda device: {name: Tensor((8,), 'i32', 144 + i * 32) for i, name in enumerate(('barrier_mla', 'barrier_mla_cell4', 'barrier_mla_cell6', 'barrier_mla_cell8', 'barrier_mla_cell10', 'barrier_mla_cell12'))}
    ...
    def test_decode_experiments_never_capture_tree_or_probe_dispatch(self):
        ...
        torch = SimpleNamespace(int32='i32', bfloat16='bf16', float8_e4m3fn='fp8',
                                cuda=SimpleNamespace(is_current_stream_capturing=lambda: False))
        for experiment in ('sync', 'bf16', 'direct'):
            materialize = experiment == 'bf16'
            self.mla.ENABLE_MLA_DIRECT_CVT = experiment == 'direct'
            ...
            for rows in (1, 7, 8, 16, 24):
                ...
                for mode in ('ordinary', 'tree', 'probe'):
                    ...
                    expected = ((10 if rows == 8 else 12) if experiment == 'direct' else 8 if materialize else 4 if rows == 8 else 6) if rows in (8, 16) and mode == 'ordinary' else 0
                    self.assertEqual(cell, expected, (materialize, rows, mode))
                    self.assertEqual(self.calls[-1][0][7],
                                     {0: 144, 4: 176, 6: 208, 8: 240, 10: 272, 12: 304}[expected])
