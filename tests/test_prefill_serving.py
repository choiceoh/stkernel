    ...
    def test_gpu_gate_requires_both_transports_all_cases_source_and_recovery(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(m.CANDIDATES,{'moe-m64':candidate}),patch.object(m,'pinned'):
                ...
                for case in ('transport','coverage','numerics','source','routing','marker','recovery','outer_failure','diagnostic_report','diagnostic_command','missing_rank','control','graph','reuse','nonfinite'):
